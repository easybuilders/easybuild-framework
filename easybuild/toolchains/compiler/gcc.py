...
class Gcc(Compiler):
    ...
    COMPILER_OPTIMAL_ARCHITECTURE_OPTION = {
        (
            systemtools.AARCH32,
            systemtools.ARM,
        ): "mcpu=native",  # implies -march=native and -mtune=native
        (
            systemtools.AARCH64,
            systemtools.ARM,
        ): "mcpu=native",  # since GCC 6; implies -march=native and -mtune=native
        (
            systemtools.POWER,
            systemtools.POWER,
        ): "mcpu=native",  # no support for -march on POWER; implies -mtune=native
        (
            systemtools.POWER,
    ...
    COMPILER_GENERIC_OPTION = {
        (
            systemtools.AARCH32,
            systemtools.ARM,
        ): "mcpu=generic-armv7",  # implies -march=armv7 and -mtune=generic-armv7
        (
            systemtools.AARCH64,
            systemtools.ARM,
        ): "mcpu=generic",  # implies -march=armv8-a and -mtune=generic
        (
            systemtools.POWER,
            systemtools.POWER,
        ...
        (
            systemtools.POWER,
            systemtools.POWER_LE,
        ): "mcpu=powerpc64le",  # no support for -march on POWER
    ...
    def _set_compiler_vars(self):
        ...
        if self.options.get("32bit", None):
            raise EasyBuildError(
                "_set_compiler_vars: 32bit set, but no support yet for 32bit GCC in EasyBuild"
            )
    ...
    def _guess_aarch64_default_optarch(self):
        ...
        if cpu_vendor == systemtools.ARM and cpu_model.startswith("ARM "):
            self.log.debug(
                "Determining architecture-specific optimization flag for ARM (model: %s)",
                cpu_model,
            )
            core_types = []
            for core_type in [ct.strip().lower() for ct in cpu_model[4:].split("+")]:
                ...
                res = re.search(
                    "\d+$", core_type
                )  # note: numeric ID is expected at the end
                if res:
                    core_id = int(res.group(0))
                    core_types.append((core_id, core_type))
                    self.log.debug(
                        "Extracted numeric ID for ARM core type '%s': %s",
                        core_type,
                        core_id,
                    )
                else:
                    ...
                    core_types = None
                    self.log.debug(
                        "Failed to extract numeric ID for ARM core type '%s', bailing out",
                        core_type,
                    )
                    break
            if core_types:
                ...
                default_optarch = "mcpu=%s" % ".".join(
                    [ct[1] for ct in sorted(core_types, reverse=True)]
                )
                self.log.debug(
                    "Using architecture-specific compiler optimization flag '%s'",
                    default_optarch,
                )
