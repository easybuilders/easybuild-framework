...
from easybuild.toolchains.compiler.inteliccifort import (
...
class CrayPECompiler(Compiler):
    ...
    COMPILER_UNIQUE_OPTS = {
        ...
        "mpich-mt": (
            False,
            "Directs the driver to link in an alternate version of the Cray-MPICH library which \
    ...
    def _set_optimal_architecture(self):
        ...
        if optarch is None:
            raise EasyBuildError(
                "Don't know which 'craype' module to load, 'optarch' build option is unspecified."
            )
        else:
            ...
            if self.modules_tool.exist([craype_mod_name], skip_avail=True)[0]:
                ...
                raise EasyBuildError(
                    "Necessary craype module with name '%s' is not available (optarch: '%s')",
                    craype_mod_name,
                    optarch,
                )
    ...
    def prepare(self, *args, **kwargs):
        ...
        if self.options["dynamic"] or self.options["shared"]:
            self.log.debug(
                "Enabling building of shared libs/dynamically linked executables via $CRAYPE_LINK_TYPE"
            )
...
class CrayPEGCC(CrayPECompiler):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for precflag in self.COMPILER_PREC_FLAGS:
            self.COMPILER_UNIQUE_OPTION_MAP[precflag] = Gcc.COMPILER_UNIQUE_OPTION_MAP[
                precflag
            ]
...
class CrayPEIntel(CrayPECompiler):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for precflag in self.COMPILER_PREC_FLAGS:
            self.COMPILER_UNIQUE_OPTION_MAP[
                precflag
            ] = IntelIccIfort.COMPILER_UNIQUE_OPTION_MAP[precflag]
...
class CrayPEPGI(CrayPECompiler):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for precflag in self.COMPILER_PREC_FLAGS:
            self.COMPILER_UNIQUE_OPTION_MAP[precflag] = Pgi.COMPILER_UNIQUE_OPTION_MAP[
                precflag
            ]
