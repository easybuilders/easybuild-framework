    ...
    * required Extension functions
    ...
    def extra_options(extra_vars=None):
        ...
        if not isinstance(extra_vars, dict):
            _log.nosupport(
                "Obtained value of type '%s' for extra_vars, should be 'dict'"
                % type(extra_vars),
                "2.0",
            )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if isinstance(args[0], EasyBlock):
            ...
            )  # we need this for Extension.sanity_check_step
    ...
    def run(self, unpack_src=False):
        ...
        if unpack_src:
            targetdir = os.path.join(
                self.master.builddir, remove_unwanted_chars(self.name)
            )
            self.ext_dir = extract_file(
                "%s" % self.src, targetdir, extra_options=self.unpack_options
            )
    ...
    def sanity_check_step(
        self, exts_filter=None, custom_paths=None, custom_commands=None
    ):
        """
        ...
        self.log.debug(
            "starting sanity check for extension with filter %s",
            self.cfg.get_ref("exts_filter"),
        )
        ...
        if self.cfg["multi_deps"] and not self.is_extension:
            ...
            lists_of_extra_modules = [
                [d["short_mod_name"] for d in deps] for deps in multi_deps
            ]
        else:
        ...
        for extra_modules in lists_of_extra_modules:
            ...
            if not (self.is_extension or self.dry_run):
                ...
                if extra_modules:
                    info_msg = (
                        "Running extension sanity check with extra modules: %s"
                        % ", ".join(extra_modules)
                    )
        ...
        if custom_paths or custom_commands or not self.is_extension:
            super(ExtensionEasyBlock, self).sanity_check_step(
        ...
        if sanity_check_ok:
            ...
            if not self.is_extension:
                msg = "Sanity check for %s failed: %s" % (
                    self.name,
