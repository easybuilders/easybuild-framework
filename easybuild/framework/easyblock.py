...
from easybuild.tools.config import (
    FORCE_DOWNLOAD_ALL,
    FORCE_DOWNLOAD_PATCHES,
    FORCE_DOWNLOAD_SOURCES,
)
from easybuild.tools.config import (
    build_option,
    build_path,
...
from easybuild.tools.modules import (
    ROOT_ENV_VAR_NAME_PREFIX,
    VERSION_ENV_VAR_NAME_PREFIX,
    DEVEL_ENV_VAR_NAME_PREFIX,
)
from easybuild.tools.modules import (
    Lmod,
    curr_module_paths,
...
class EasyBlock(object):
    ...
    def extra_options(extra=None):
        ...
        if not isinstance(extra, dict):
            _log.nosupport(
                "Found 'extra' value of type '%s' in extra_options, should be 'dict'"
                % type(extra),
                "2.0",
            )
    ...
    def __init__(self, ec):
        ...
        self.module_extra_extensions = (
            ""  # extra stuff for module file required by extensions
        )
        ...
        if isinstance(ec, EasyConfig):
            ...
            raise EasyBuildError(
                "Value of incorrect type passed to EasyBlock constructor: %s ('%s')",
                type(ec),
                ec,
            )
        ...
        if group_spec is not None:
            if isinstance(group_spec, tuple):
                if len(group_spec) == 2:
                    ...
                    raise EasyBuildError(
                        "Found group spec in tuple format that is not a 2-tuple: %s",
                        str(group_spec),
                    )
            self.log.warning(
                "Group spec '%s' is overriding config group '%s'."
                % (group_spec, group_name)
            )
        ...
        self.log.info(
            "Init completed for application name %s version %s"
    ...
    def _init_log(self):
        ...
        self.log.info(
            "This is easyblock %s from module %s (%s)",
            eb_class,
            eb_mod_name,
            eb_mod_loc,
        )
    ...
    def close_log(self):
        ...
        """
        self.log.info(
            "Closing log for application name %s version %s" % (self.name, self.version)
        )
    ...
    def init_dry_run(self):
        ...
        self.installdir = os.path.join(
            tmp_root_dir, self.installdir.lstrip(os.path.sep)
        )
        self.installdir_mod = os.path.join(
            tmp_root_dir, self.installdir_mod.lstrip(os.path.sep)
        )

        # register fake build/install dirs so the original values can be printed during dry run
        dry_run_set_dirs(
            tmp_root_dir, self.builddir, self.installdir, self.installdir_mod
        )
    ...
    def get_checksum_for(self, checksums, filename=None, index=None):
        ...
        if isinstance(checksums, (list, tuple)):
            if (
                index is not None
                and index < len(checksums)
                and (index >= 0 or abs(index) <= len(checksums))
            ):
            ...
            raise EasyBuildError(
                "Invalid type for checksums (%s), should be list, tuple or None.",
                type(checksums),
            )
    ...
    def fetch_sources(self, sources=None, checksums=None):
        ...
        for index, source in enumerate(sources):
            extract_cmd, download_filename, source_urls, git_config = (
                None,
                None,
                None,
                None,
            ...
            if isinstance(source, string_type):
                ...
                if source:
                    raise EasyBuildError(
                        "Found one or more unexpected keys in 'sources' specification: %s",
                        source,
                    )

            elif isinstance(source, (list, tuple)) and len(source) == 2:
                self.log.deprecated(
                    "Using a 2-element list/tuple to specify sources is deprecated, "
                    "use a dictionary with 'filename', 'extract_cmd' keys instead",
                    "4.0",
                )
                filename, extract_cmd = source
            else:
                raise EasyBuildError(
                    "Unexpected source spec, not a string or dict: %s", source
                )

            # check if the sources can be located
            force_download = build_option("force_download") in [
                FORCE_DOWNLOAD_ALL,
                FORCE_DOWNLOAD_SOURCES,
            ]
            path = self.obtain_file(
                filename,
                ...
                self.src.append(
                    {
                        ...
                        "checksum": self.get_checksum_for(
                            checksums, filename=filename, index=index
                        ),
        ...
        for index, patch_spec in enumerate(patch_specs):
            ...
            if isinstance(patch_spec, (list, tuple)):
                if not len(patch_spec) == 2:
                    raise EasyBuildError(
                        "Unknown patch specification '%s', only 2-element lists/tuples are supported!",
                        str(patch_spec),
                    )
                ...
                if type(patch_spec[1]) == int:
                    ...
                    raise EasyBuildError(
                        "Wrong patch spec '%s', only int/string are supported as 2nd element",
                        str(patch_spec),
                    )
            else:
            ...
            path = self.obtain_file(
                patch_file, extension=extension, force_download=force_download
            )
            if path:
                ...
                patchspec = {
                    ...
                    "path": path,
                    "checksum": self.get_checksum_for(
                        checksums, filename=patch_file, index=index
                    ),
                }
                if suff:
                    if copy_file:
        ...
        for ext in exts_list:
            if (isinstance(ext, list) or isinstance(ext, tuple)) and ext:
                ...
                if len(ext) == 1:
                    ...
                    ext_options = copy.deepcopy(
                        self.cfg.get_ref("exts_default_options")
                    )
                    ...
                    if len(ext) == 3:
                        if isinstance(ext_options, dict):
                            ...
                            raise EasyBuildError(
                                "Unexpected type (non-dict) for 3rd element of %s", ext
                            )
                    elif len(ext) > 3:
                        raise EasyBuildError(
                            "Extension specified in unknown format (list/tuple too long)"
                        )
                    ...
                    default_source_tmpl = resolve_template(
                        "%(name)s-%(version)s.tar.gz", template_values
                    )
                        ...
                        if src_fn:
                            ...
                            if not skip_checksums:
                                ...
                                for checksum_type in (
                                    ...
                                    src_checksum = compute_checksum(
                                        src_fn, checksum_type=checksum_type
                                    )
                                    self.log.info(
                                        "%s checksum for %s: %s",
                                        checksum_type,
                                ...
                                self.log.debug(
                                    "Verifying checksums for extension source..."
                                )
                                fn_checksum = self.get_checksum_for(
                                    checksums, filename=src_fn, index=0
                                )
                                if verify_checksum(src_fn, fn_checksum):
                                    self.log.info(
                                        "Checksum for extension source %s verified", fn
                                    )
                                elif build_option("ignore_checksums"):
                                    print_warning(
                                        "Ignoring failing checksum verification for %s"
                                        % fn
                                    )
                                else:
                                    raise EasyBuildError(
                                        "Checksum verification for extension source %s failed",
                                        fn,
                                    )

                            ext_patches = self.fetch_patches(
                                patch_specs=ext_options.get("patches", []),
                                extension=True,
                            )
                            if ext_patches:
                                self.log.debug(
                                    "Found patches for extension %s: %s"
                                    % (ext_name, ext_patches)
                                )
                                ...
                                if not skip_checksums:
                                    for patch in ext_patches:
                                        ...
                                        for checksum_type in (
                                            ...
                                            checksum = compute_checksum(
                                                patch, checksum_type=checksum_type
                                            )
                                            self.log.info(
                                                "%s checksum for %s: %s",
                                                checksum_type,
                                    ...
                                    # verify checksum (if provided)
                                    self.log.debug(
                                        "Verifying checksums for extension patches..."
                                    )
                                    for idx, patch in enumerate(ext_patches):
                                        patch = patch["path"]
                                        checksum = self.get_checksum_for(
                                            checksums[1:], filename=patch, index=idx
                                        )
                                        if verify_checksum(patch, checksum):
                                            self.log.info(
                                                "Checksum for extension patch %s verified",
                                                patch,
                                            )
                                        elif build_option("ignore_checksums"):
                                            print_warning(
                                            ...
                                            raise EasyBuildError(
                                                "Checksum for extension patch %s failed",
                                                patch,
                                            )
                            else:
                                self.log.debug(
                                    "No patches found for extension %s." % ext_name
                                )
                            ...
                            raise EasyBuildError(
                                "Source for extension %s not found.", ext
                            )
                ...
                raise EasyBuildError(
                    "Extension specified in unknown format (not a string/list/tuple)"
                )
    ...
    def obtain_file(
        ...
        if re.match(r"^(https?|ftp)://", filename):
            ...
            try:
                ...
                if os.path.exists(fullpath):
                    if force_download:
                        ...
                        self.log.info(
                            "Found file %s at %s, no need to download it",
                            filename,
                            filepath,
                        )
                        return fullpath
            ...
            common_filepaths.extend(
                get_paths_for(subdir=EASYCONFIGS_PKG_SUBDIR, robot_path=self.robot_path)
            )
            ...
            if foundfile:
                ...
                for url in source_urls:
                    ...
                    if isinstance(url, string_type):
                        ...
                        self.log.warning(
                            "Source URL %s is of unknown type, so ignoring it." % url
                        )
                        continue
                    ...
                    if PYPI_PKG_URL_PATTERN in fullurl and not is_alt_pypi_url(fullurl):
                        ...
                        if alt_url:
                            _log.debug(
                                "Using alternate PyPI URL for %s: %s", fullurl, alt_url
                            )
                            fullurl = alt_url
                        else:
                            _log.debug(
                                "Failed to derive alternate PyPI URL for %s, so retaining the original",
                                fullurl,
                            )

                    if self.dry_run:
                        self.dry_run_msg(
                            "  * %s will be downloaded to %s", filename, targetpath
                        )
                        ...
                        self.log.debug(
                            "Trying to download file %s from %s to %s ..."
                            % (filename, fullurl, targetpath)
                        )
                        downloaded = False
                        try:
                    ...
                    if downloaded:
                        ...
                        self.log.info(
                            "Successfully downloaded source file %s from %s"
                            % (filename, fullurl)
                        )
                        return targetpath
                    else:
    ...
    def moduleGenerator(self):
        ...
        """
        self.log.nosupport(
            "self.moduleGenerator is replaced by self.module_generator", "2.0"
        )
    ...
    def gen_builddir(self):
        ...
        builddir = os.path.join(
            os.path.abspath(build_path()), clean_name, self.version, lastdir
        )
    ...
    def make_builddir(self):
        ...
        if not self.build_in_installdir:
            ...
            if not self.builddir:
                raise EasyBuildError(
                    "self.builddir not set, make sure gen_builddir() is called first!"
                )
            self.log.debug(
                "Creating the build directory %s (cleanup: %s)",
                self.builddir,
                self.cfg["cleanupoldbuild"],
            )
        else:
            ...
            self.log.info(
                "Overriding 'cleanupoldinstall' (to False), 'cleanupoldbuild' (to True) "
            ...
            if build_option("module_only"):
                self.log.debug(
                    "Disabling cleanupoldbuild because we run as module-only"
                )
    ...
    def gen_installdir(self):
        ...
        if basepath:
            ...
            self.installdir = os.path.join(
                os.path.abspath(basepath), self.install_subdir
            )
            ...
            self.installdir_mod = os.path.join(
                os.path.abspath(mod_basepath), mod_path_suffix
            )
    ...
    def make_installdir(self, dontcreate=None):
        ...
        """
        self.log.debug(
            "Creating the installation directory %s (cleanup: %s)"
            % (self.installdir, self.cfg["cleanupoldinstall"])
        )
        if self.build_in_installdir:
            self.cfg["keeppreviousinstall"] = True
        dontcreate = (
            dontcreate is None and self.cfg["dontcreateinstalldir"]
        ) or dontcreate
        self.make_dir(
            self.installdir,
            self.cfg["cleanupoldinstall"],
            dontcreateinstalldir=dontcreate,
        )
    ...
    def make_dir(self, dir_name, clean, dontcreateinstalldir=False):
        ...
        if os.path.exists(dir_name):
            ...
            if self.cfg["keeppreviousinstall"]:
                self.log.info(
                    "Keeping old directory %s (hopefully you know what you are doing)",
                    dir_name,
                )
                return
            elif build_option("module_only"):
                self.log.info(
                    "Not touching existing directory %s in module-only mode...",
                    dir_name,
                )
            elif clean:
                remove_dir(dir_name)
                ...
                self.log.info(
                    "Moving existing directory %s out of the way...", dir_name
                )
        ...
        if dontcreateinstalldir:
            ...
            self.log.info(
                "Cleaning only, no actual creation of %s, only verification/defining of dirname %s",
                olddir,
                dir_name,
            )
            if os.path.exists(dir_name):
    ...
    def make_devel_module(self, create_in_builddir=False):
            ...
            if key.startswith(DEVEL_ENV_VAR_NAME_PREFIX):
                if not key.endswith(convert_name(self.name, upper=True)):
                    ...
                    if os.path.isfile(path):
                        ...
                        load_statement = self.module_generator.load_module(
                            mod_name,
                ...
                self.log.nosupport(
                    "Environment variable SOFTDEVEL* being relied on", "2.0"
                )
        ...
        filename = os.path.join(
            output_dir, ActiveMNS().det_devel_module_filename(self.cfg)
        )
    ...
    def make_module_dep(self, unload_info=None):
        ...
        if not self.toolchain.is_system_toolchain():
            ...
            self.log.debug(
                "Toolchain to load in generated module (before excluding any deps): %s",
                tc_mod,
            )
        ...
        if mns.expand_toolchain_load(ec=self.cfg):
            ...
            self.log.debug(
                "Toolchain components to load in generated module (before excluding any): %s",
                tc_dep_mods,
            )
        ...
        deps = []
        self.log.debug(
            "List of deps considered to load in generated module: %s",
        ...
        for dep in self.toolchain.dependencies:
            if dep["build_only"]:
                ...
                deps.append(modname)
        self.log.debug(
            "List of deps to load in generated module (before excluding any): %s", deps
        )
        ...
        top_paths = [self.installdir_mod] + [
            os.path.join(self.installdir_mod, p) for p in init_modpaths
        ]

        all_deps = [d for d in [tc_mod] + (tc_dep_mods or []) + deps if d is not None]
        excluded_deps = self.modules_tool.path_to_top_of_module_tree(
            top_paths, self.cfg.short_mod_name, full_mod_subdir, all_deps
        )
        ...
        for excluded_dep in excluded_deps:
            ...
            self.log.debug(
                "List of dependencies for excluded dependency %s: %s"
                % (excluded_dep, excluded_dep_deps)
            )
        ...
        for dep in deps:
            ...
            loads.append(
                self.module_generator.load_module(
                    dep,
                    recursive_unload=recursive_unload,
                    depends_on=depends_on,
        ...
        if self.cfg["multi_deps_load_default"]:
            ...
            for depname, depmods in sorted(multi_dep_mod_names.items()):
                stmt = self.module_generator.load_module(
                    depmods[0],
                    multi_dep_mods=depmods,
                    recursive_unload=recursive_unload,
                    depends_on=depends_on,
                )
    ...
    def make_module_extra(self, altroot=None, altversion=None):
        ...
        if altroot:
            set_root_envvar = self.module_generator.set_environment(
                root_envvar, altroot
            )
        else:
            set_root_envvar = self.module_generator.set_environment(
                root_envvar, "", relpath=True
            )
        ...
        lines.append(
            self.module_generator.set_environment(
                version_envvar, altversion or self.version
            )
        )

        # $EBDEVEL<NAME>
        devel_path = os.path.join(
            log_path(ec=self.cfg), ActiveMNS().det_devel_module_filename(self.cfg)
        )
        devel_path_envvar = DEVEL_ENV_VAR_NAME_PREFIX + env_name
        lines.append(
            self.module_generator.set_environment(
                devel_path_envvar, devel_path, relpath=True
            )
        )
        ...
        for (key, value) in self.cfg["modextrapaths"].items():
            if isinstance(value, string_type):
                ...
                raise EasyBuildError(
                    "modextrapaths dict value %s (type: %s) is not a list or tuple",
                    value,
                    type(value),
                )
            lines.append(
                self.module_generator.prepend_paths(
                    key, value, allow_abs=self.cfg["allow_prepend_abs_path"]
                )
            )
    ...
    def make_module_extra_extensions(self):
        ...
        exts_list = ",".join(
            ["%s-%s" % (ext[0], ext[1]) for ext in self.cfg["exts_list"]]
        )
        env_var_name = convert_name(self.name, upper=True)
        lines.append(
            self.module_generator.set_environment(
                "EBEXTSLIST%s" % env_var_name, exts_list
            )
        )
    ...
    def make_module_footer(self):
        ...
        """
        footer = [
            self.module_generator.comment(
                "Built with EasyBuild version %s" % VERBOSE_VERSION
            )
        ]
        ...
        if self.modules_footer is not None:
            self.log.debug(
                "Including specified footer into module: '%s'" % self.modules_footer
            )
        ...
        if self.cfg["modtclfooter"]:
            if isinstance(self.module_generator, ModuleGeneratorTcl):
                self.log.debug(
                    "Including Tcl footer in module: %s", self.cfg["modtclfooter"]
                )
                footer.extend([self.cfg["modtclfooter"], "\n"])
            else:
                self.log.warning(
                    "Not including footer in Tcl syntax in non-Tcl module file: %s",
        ...
        if self.cfg["modluafooter"]:
            if isinstance(self.module_generator, ModuleGeneratorLua):
                self.log.debug(
                    "Including Lua footer in module: %s", self.cfg["modluafooter"]
                )
                footer.extend([self.cfg["modluafooter"], "\n"])
            else:
                self.log.warning(
                    "Not including footer in Lua syntax in non-Lua module file: %s",
    ...
    def make_module_extend_modpath(self):
        ...
        if self.cfg["include_modpath_extensions"]:
            ...
            self.log.debug(
                "Including module path extensions returned by module naming scheme: %s",
                modpath_exts,
            )
            full_path_modpath_extensions = [
                os.path.join(self.installdir_mod, ext) for ext in modpath_exts
            ]
            ...
            if user_modpath:
                ...
                self.log.debug(
                    "Including user module path extensions returned by naming scheme: %s",
                    user_modpath_exts,
                )
                txt += self.module_generator.use(
                    user_modpath_exts,
                    prefix=self.module_generator.getenv_cmd("HOME"),
                    guarded=True,
    ...
    def make_module_group_check(self):
        ...
        if self.group is not None:
            txt = self.module_generator.check_group(
                self.group[0], error_msg=group_error_msg
            )
        else:
            txt = ""
        ...
        if os.path.isdir(self.installdir):
            ...
            for key in sorted(requirements):
                ...
                if isinstance(reqs, string_type):
                    self.log.warning(
                        "Hoisting string value %s into a list before iterating over it",
                        reqs,
                    )
                    reqs = [reqs]
                    ...
                    if path and not self.dry_run:
                        ...
                        if paths and key in keys_requiring_files:
                            ...
                            retained_paths = [
                                ...
                                and dir_contains_files(
                                    os.path.join(self.installdir, path)
                                )
                            ]
                            self.log.info(
                                "Only retaining paths for %s that contain at least one file: %s -> %s",
                                key,
                                paths,
                                retained_paths,
                            )
    ...
    def make_module_req_guess(self):
        ...
        return {
            ...
            "PKG_CONFIG_PATH": [
                os.path.join(x, "pkgconfig") for x in lib_paths + ["share"]
            ],
    ...
    def load_module(self, mod_paths=None, purge=True, extra_modules=None):
        ...
        if self.full_mod_name is not None:
            ...
            self.modules_tool.load(
                mods,
                mod_paths=all_mod_paths,
                purge=purge,
                init_env=self.initial_environ,
            )
    ...
    def load_fake_module(self, purge=False, extra_modules=None):
        ...
        self.modules_tool.prepend_module_path(
            os.path.join(fake_mod_path, self.mod_subdir), priority=10000
        )
    ...
    def clean_up_fake_module(self, fake_mod_data):
        ...
        if fake_mod_path and self.short_mod_name is not None:
            try:
                ...
                self.modules_tool.remove_module_path(
                    os.path.join(fake_mod_path, self.mod_subdir)
                )
                rmtree2(os.path.dirname(fake_mod_path))
            except OSError as err:
                raise EasyBuildError(
                    "Failed to clean up fake module dir %s: %s", fake_mod_path, err
                )
        elif self.short_mod_name is None:
            self.log.warning(
                "Not unloading module, since self.short_mod_name is not set."
            )
    ...
    def load_dependency_modules(self):
        ...
        self.modules_tool.load(
            [ActiveMNS().det_full_module_name(dep) for dep in self.cfg.dependencies()]
        )
    ...
    def skip_extensions(self):
        ...
        if not exts_filter or len(exts_filter) == 0:
            raise EasyBuildError(
                "Skipping of extensions, but no exts_filter set in easyconfig"
            )

        res = []
        for ext in self.exts:
            cmd, stdin = resolve_exts_filter_template(exts_filter, ext)
            (cmdstdouterr, ec) = run_cmd(
                cmd, log_all=False, log_ok=False, simple=False, inp=stdin, regexp=False
            )
    ...
    def guess_start_dir(self):
        ...
        if not os.path.isabs(start_dir):
            ...
            if self.dry_run and os.path.samefile(topdir, self.builddir):
                topdir = os.path.join(
                    self.builddir, "%s-%s" % (self.name, self.version)
                )
                self.log.info(
                    "Modified parent directory of start dir in dry run mode to likely path %s",
                    topdir,
                )
            ...
            if topdir.endswith(start_dir) and not os.path.exists(abs_start_dir):
                ...
                if os.path.exists(abs_start_dir):
                    ...
                    raise EasyBuildError(
                        "Specified start dir %s does not exist", abs_start_dir
                    )
    ...
    def handle_iterate_opts(self):
        ...
        if self.iter_opts:
            print_msg(
                "starting iteration #%s ..." % self.iter_idx,
                log=self.log,
                silent=self.silent,
            )
    ...
    def post_iter_step(self):
        ...
        for opt in self.iter_opts:
            ...
            self.log.debug(
                "Restored value of '%s' that was iterated over: %s", opt, self.cfg[opt]
            )
    ...
    def det_iter_cnt(self):
        ...
        iter_opt_counts = [
            len(self.cfg.get_ref(opt))
            for opt in ITERATE_OPTIONS
            if opt not in ["builddependencies"]
            and isinstance(self.cfg.get_ref(opt), (list, tuple))
        ]
        ...
        self.log.info(
            "Number of iterations to perform for central part of installation procedure: %s",
            iter_cnt,
        )
    ...
    def set_parallel(self):
        ...
        if self.cfg["parallel"] is not None:
            if par is None:
                ...
                self.log.debug(
                    "Desired parallelism specified via 'parallel' easyconfig parameter: %s",
                    par,
                )
            else:
                par = min(int(par), int(self.cfg["parallel"]))
                self.log.debug(
                    "Desired parallelism: minimum of 'parallel' build option/easyconfig parameter: %s",
                    par,
                )
        else:
            self.log.debug(
                "Desired parallelism specified via 'parallel' build option: %s", par
            )
    ...
    def remove_module_file(self):
        ...
        if os.path.exists(self.mod_filepath):
            ...
            if old_installdir is None:
                warning_msg = (
                    "Failed to determine installation directory from module file %s"
                    % self.mod_filepath
                )
                warning_msg += (
                    ", can't clean up potential ghost installation for %s %s"
                    % (self.name, self.version)
                )
                print_warning(warning_msg)

            elif os.path.exists(old_installdir) and not os.path.samefile(
                old_installdir, self.installdir
            ):
                if build_option("remove_ghost_install_dirs"):
                    remove_dir(old_installdir)
                    self.log.info(
                        "Ghost installation directory %s removed", old_installdir
                    )
                    print_msg("Ghost installation directory %s removed", old_installdir)
                else:
                    print_warning(
                        "Likely ghost installation directory detected: %s",
                        old_installdir,
                    )
    ...
    def check_readiness_step(self):
        ...
        if root:
            raise EasyBuildError(
                "Module is already loaded (%s is set), installation cannot continue.",
                env_var,
            )
        ...
        if os.path.exists(self.mod_filepath) and build_option("backup_modules"):
            ...
            if isinstance(self.module_generator, ModuleGeneratorLua) and isinstance(
                self.modules_tool, Lmod
            ):
                hidden = LooseVersion(self.modules_tool.version) < LooseVersion("7.0.0")

            self.mod_file_backup = back_up_file(
                self.mod_filepath, hidden=hidden, strip_fn=strip_fn
            )
            print_msg(
                "backup of existing module file stored at %s" % self.mod_file_backup,
                log=self.log,
            )
        ...
        if self.cfg["skip"]:
            if self.modules_tool.exist([self.full_mod_name], skip_avail=True)[0]:
                ...
                self.log.info(
                    "Going to skip actual main build and potential existing extensions. Expert only."
                )
            else:
                self.log.info(
                    "No module %s found. Not skipping anything." % self.full_mod_name
                )
    ...
    def fetch_step(self, skip_checksums=False):
        ...
        if not easybuild_version:
            self.log.warning(
                "Easyconfig does not specify an EasyBuild-version (key 'easybuild_version')! "
                "Assuming the latest version"
            )
        else:
            if LooseVersion(easybuild_version) < VERSION:
                self.log.warning(
                    "EasyBuild-version %s is older than the currently running one. Proceed with caution!",
                    easybuild_version,
                )
            elif LooseVersion(easybuild_version) > VERSION:
                raise EasyBuildError(
                    "EasyBuild-version %s is newer than the currently running one. Aborting!",
                    easybuild_version,
                )
        ...
        if not (skip_checksums or self.dry_run):
            for fil in self.src + self.patches:
                ...
                for checksum_type in [CHECKSUM_TYPE_MD5, CHECKSUM_TYPE_SHA256]:
                    fil[checksum_type] = compute_checksum(
                        fil["path"], checksum_type=checksum_type
                    )
                    self.log.info(
                        "%s checksum for %s: %s",
                        checksum_type,
        ...
        for mod_symlink_path in mod_symlink_paths:
            pardirs.append(
                os.path.join(install_path("mod"), mod_symlink_path, mod_subdir)
            )
    ...
    def checksum_step(self):
        ...
        for fil in self.src + self.patches:
            if self.dry_run:
                ...
                self.dry_run_msg(
                    "* expected checksum for %s: %s", filename, expected_checksum
                )
            else:
                if verify_checksum(fil["path"], fil["checksum"]):
                    self.log.info(
                        "Checksum verification for %s using %s passed."
                        % (fil["path"], fil["checksum"])
                    )
                elif build_option("ignore_checksums"):
                    print_warning(
                        "Ignoring failing checksum verification for %s" % fil["name"]
                    )
                else:
                    raise EasyBuildError(
                        "Checksum verification for %s using %s failed.",
                        fil["path"],
                        fil["checksum"],
                    )
    ...
    def check_checksums(self):
        ...
        for ext in self.cfg["exts_list"]:
            ...
            if not isinstance(ext, string_type):
                ...
                res = self.check_checksums_for(
                    ext_opts, sub="of extension %s" % ext_name, source_cnt=1
                )
    ...
    def patch_step(self, beginpath=None):
        ...
        for patch in self.patches:
            ...
            self.log.debug(
                "Source index: %s; patch level: %s; source path suffix: %s; copy patch: %s",
                srcind,
                level,
                srcpathsuffix,
                copy,
            ...
            if beginpath is None:
                try:
                    ...
                    self.log.debug(
                        "Determine begin path for patch %s: %s"
                        % (patch["name"], beginpath)
                    )
                except IndexError as err:
                    raise EasyBuildError(
                        "Can't apply patch %s to source at index %s of list %s: %s",
                        patch["name"],
                        srcind,
                        self.src,
                        err,
                ...
                self.log.debug(
                    "Using specified begin path for patch %s: %s"
                    % (patch["name"], beginpath)
                )
    ...
    def prepare_step(self, start_dir=True, load_tc_deps_modules=True):
        ...
        self.cfg["unwanted_env_vars"] = env.unset_env_vars(
            self.cfg["unwanted_env_vars"]
        )
        ...
        for var in ["TMPDIR"]:
            if os.environ.get(var) != self.initial_environ.get(var):
                ...
                self.log.info(
                    "Found tweaked value for $%s: %s (was: %s)",
                    var,
                    self.tweaked_env_vars[var],
                    self.initial_environ[var],
                )
    ...
    def test_step(self):
        ...
        if self.cfg["runtest"]:
            ...
            self.log.debug(
                "Trying to execute %s as a command for running unit tests..."
            )
    ...
    def extensions_step(self, fetch=False):
        ...
        if not self.dry_run:
            ...
            build_dep_mods = [
                dep["short_mod_name"] for dep in self.cfg.dependencies(build_only=True)
            ]

            fake_mod_data = self.load_fake_module(
                purge=True, extra_modules=build_dep_mods
            )
        ...
        self.exts_all = self.exts[
            :
        ]  # retain a copy of all extensions, regardless of filtering/skipping
        ...
        if not exts_defaultclass and fake_mod_data:
            self.clean_up_fake_module(fake_mod_data)
            raise EasyBuildError(
                "ERROR: No default extension class set for %s", self.name
            )
        ...
        if isinstance(exts_defaultclass, string_type):
            ...
            raise EasyBuildError(
                "Improper default extension class specification, should be string."
            )
        ...
        for idx, ext in enumerate(self.exts):
            ...
            try:
                ...
                self.log.debug(
                    "Obtained class %s for extension %s" % (cls, ext["name"])
                )
                if cls is not None:
                    inst = cls(self, ext)
            except (ImportError, NameError) as err:
                self.log.debug(
                    "Failed to use extension-specific class for extension %s: %s"
                    % (ext["name"], err)
                )
            ...
            if inst is None and ext["name"] in exts_classmap:
                ...
                try:
                    ...
                    raise EasyBuildError(
                        "Failed to load specified class %s for extension %s: %s",
                        class_name,
                        ext["name"],
                        err,
                    )
            ...
            if inst is None:
                try:
                    ...
                    self.log.debug(
                        "Obtained class %s for installing extension %s"
                        % (cls, ext["name"])
                    )
                    inst = cls(self, ext)
                    self.log.debug(
                        "Installing extension %s with default class %s (from %s)",
                        ext["name"],
                        default_class,
                    ...
                    raise EasyBuildError(
                        "Also failed to use default class %s from %s for extension %s: %s, giving up",
                        default_class,
                        default_class_modpath,
                        ext["name"],
                        err,
                ...
                self.log.debug(
                    "Installing extension %s with class %s (from %s)"
                    % (ext["name"], class_name, mod_path)
                )
            ...
            if self.dry_run:
                self.dry_run_msg(
                    "defining build environment based on toolchain (options) and dependencies..."
                )
            else:
                ...
                inst.toolchain.prepare(
                    onlymod=self.cfg["onlytcmod"],
                    silent=True,
                    loadmod=False,
                    rpath_filter_dirs=self.rpath_filter_dirs,
                )
        ...
        if build_option("package"):
            ...
            for src_file in glob.glob(os.path.join(pkgdir_src, "*.%s" % pkgtype)):
                ...
                if os.path.exists(dest_file) and not opt_force:
                    raise EasyBuildError(
                        "Unable to copy package %s to %s (already exists).",
                        src_file,
                        dest_file,
                    )
                else:
        ...
        for lang in ["perl", "python"]:
            ...
            if fix_shebang_for:
                ...
                for glob_pattern in fix_shebang_for:
                    ...
                    self.log.info(
                        "Fixing '%s' shebang to '%s' for files that match '%s': %s",
                        lang,
                        shebang,
                        glob_pattern,
                        paths,
    ...
    def post_install_step(self):
        ...
        if self.cfg["postinstallcmds"] is not None:
            ...
            if not isinstance(self.cfg["postinstallcmds"], (list, tuple)):
                raise EasyBuildError(
                    "Invalid value for 'postinstallcmds', should be list or tuple of strings."
                )
            for cmd in self.cfg["postinstallcmds"]:
                if not isinstance(cmd, string_type):
                    raise EasyBuildError(
                        "Invalid element in 'postinstallcmds', not a string: %s", cmd
                    )
    ...
    def _sanity_check_step_multi_deps(self, *args, **kwargs):
        ...
        for iter_deps in self.cfg.multi_deps:
            ...
            extra_modules = common_extra_modules + [
                d["short_mod_name"] for d in iter_deps
            ]

            info_msg = "Running sanity check with extra modules: %s" % ", ".join(
                extra_modules
            )
            trace_msg(info_msg)
    ...
    def sanity_check_rpath(self, rpath_dirs=None):
        ...
        self.log.debug(
            "$LD_LIBRARY_PATH during RPATH sanity check: %s",
            os.getenv("LD_LIBRARY_PATH", "(empty)"),
        )
        self.log.debug("List of loaded modules: %s", self.modules_tool.list())
        ...
        if rpath_dirs is None:
            ...
            self.log.info(
                "Using default subdirs for binaries/libraries to verify RPATH linking: %s",
                rpath_dirs,
            )
        else:
            self.log.info(
                "Using specified subdirs for binaries/libraries to verify RPATH linking: %s",
                rpath_dirs,
            )
                ...
                for path in [os.path.join(dirpath, x) for x in os.listdir(dirpath)]:
                    ...
                    if "dynamically linked" in out:
                        ...
                        if ec:
                            ...
                            fail_msg = (
                                "One or more required libraries not found for %s: %s"
                                % (path, out)
                            )
                            self.log.warning(fail_msg)
                            fails.append(fail_msg)
                        ...
                        if ec:
                            ...
                            fail_msg = (
                                "No '(RPATH)' found in 'readelf -d' output for %s: %s"
                                % (path, out)
                            )
                            self.log.warning(fail_msg)
                            fails.append(fail_msg)
                            ...
                            self.log.debug(
                                "Output of 'readelf -d %s' checked, looks OK", path
                            )

                    else:
                        self.log.debug(
                            "%s is not dynamically linked, so skipping it in RPATH sanity check",
                            path,
                        )
            else:
                self.log.debug(
                    "Not sanity checking files in non-existing directory %s", dirpath
                )
    ...
    def _sanity_check_step_common(self, custom_paths, custom_commands):
        ...
        path_keys_and_check = {
            ...
            SANITY_CHECK_PATHS_FILES: (
                "file",
                lambda fp: os.path.exists(fp) and not os.path.isdir(fp),
            ),
            # directories must exist and be non-empty
            SANITY_CHECK_PATHS_DIRS: (
                "(non-empty) directory",
                lambda dp: os.path.isdir(dp) and os.listdir(dp),
            ),
        }
        ...
        if not ks == req_keys or sum(valnottypes) > 0 or sum(lenvals) == 0:
            raise EasyBuildError(
                "Incorrect format for sanity_check_paths (should (only) have %s keys, "
    ...
    def _sanity_check_step_dry_run(self, custom_paths=None, custom_commands=None, **_):
        ...
        :param custom_commands: custom sanity check commands to run
        """
        paths, path_keys_and_check, commands = self._sanity_check_step_common(
            custom_paths, custom_commands
        )
    ...
    def _sanity_check_step_extensions(self):
        ...
        for ext in self.ext_instances:
            ...
            if isinstance(res, tuple):
                if len(res) != 2:
                    raise EasyBuildError(
                        "Wrong sanity check result type for '%s' extension: %s",
                        ext.name,
                        res,
                    )
            ...
            if not success:
                fail_msg = "failing sanity check for '%s' extension: %s" % (
                    ext.name,
                    fail_msg,
                )
        ...
        if failed_exts:
            overall_fail_msg = (
                "extensions sanity check failed for %d extensions: " % len(failed_exts)
            )
            self.log.warning(overall_fail_msg)
            self.sanity_check_fail_msgs.append(
                overall_fail_msg + ", ".join(x[0] for x in failed_exts)
            )
            self.sanity_check_fail_msgs.extend(x[1] for x in failed_exts)

    def _sanity_check_step(
        self,
        custom_paths=None,
        ...
        def check_path(xs, typ, check_fn):
            ...
            for name in xs:
                ...
                if check_fn(path):
                    self.log.debug(
                        "Sanity check: found %s %s in %s" % (typ, name, self.installdir)
                    )
                    found = True
                    break
                else:
                    self.log.debug(
                        "Could not find %s %s in %s" % (typ, name, self.installdir)
                    )
        ...
        for key in [SANITY_CHECK_PATHS_FILES, SANITY_CHECK_PATHS_DIRS]:
            ...
            for xs in paths[key]:
                if isinstance(xs, string_type):
                    ...
                    raise EasyBuildError(
                        "Unsupported type %s encountered in '%s', not a string or tuple",
                        type(xs),
                        key,
                    )
                ...
                if not found and build_option("lib64_fallback_sanity_check"):
                    ...
                    if all(x.startswith("lib/") or x == "lib" for x in xs):
                        xs_alt = [
                            os.path.join("lib64", *x.split(os.path.sep)[1:]) for x in xs
                        ]
                    elif all(x.startswith("lib64/") or x == "lib64" for x in xs):
                        xs_alt = [
                            os.path.join("lib", *x.split(os.path.sep)[1:]) for x in xs
                        ]

                    if xs_alt:
                        self.log.info(
                            "%s not found at %s in %s, consider fallback locations: %s",
                            typ,
                            xs2str(xs),
                            self.installdir,
                            xs2str(xs_alt),
                ...
                if not found:
                    sanity_check_fail_msg = "no %s found at %s in %s" % (
                        typ,
                        xs2str(xs),
                        self.installdir,
                    )
                ...
                trace_msg(
                    "%s %s found: %s" % (typ, xs2str(xs), ("FAILED", "OK")[found])
                )
        ...
        if not (extension or self.dry_run):
            try:
                ...
                fake_mod_data = self.load_fake_module(
                    purge=True, extra_modules=extra_modules
                )
            except EasyBuildError as err:
                self.sanity_check_fail_msgs.append(
                    "loading fake module failed: %s" % err
                )
                self.log.warning("Sanity check: %s" % self.sanity_check_fail_msgs[-1])

            if extra_modules:
                self.log.info(
                    "Loading extra modules for sanity check: %s",
                    ", ".join(extra_modules),
                )
        ...
        for command in commands:
            out, ec = run_cmd(
                command, simple=False, log_ok=False, log_all=False, trace=False
            )
            if ec != 0:
                fail_msg = (
                    "sanity check command %s exited with code %s (output: %s)"
                    % (command, ec, out)
                )
                ...
                self.log.info(
                    "sanity check command %s ran successfully! (output: %s)"
                    % (command, out)
                )
    ...
    def _set_module_as_default(self, fake=False):
        ...
        if self.dry_run:
            ...
            mod_dir_path = os.path.dirname(
                self.module_generator.get_module_filepath(fake=fake)
            )
            if fake:
                mod_symlink_paths = []
            else:
                mod_symlink_paths = ActiveMNS().det_module_symlink_paths(self.cfg)
            self.module_generator.set_as_default(
                mod_dir_path, version, mod_symlink_paths=mod_symlink_paths
            )
    ...
    def cleanup_step(self):
        ...
        if not self.build_in_installdir and build_option("cleanup_builddir"):
            ...
            try:
                ...
                while len(os.listdir(base)) == 0 and not os.path.samefile(
                    base, build_path()
                ):
                    os.rmdir(base)
                ...
                raise EasyBuildError(
                    "Cleaning up builddir %s failed: %s", self.builddir, err
                )
    ...
    def make_module_step(self, fake=False):
        ...
        if self.dry_run:
            ...
            if not fake:
                self.dry_run_msg(
                    "Generating module file %s, with contents:\n", mod_filepath
                )
            ...
            if self.mod_file_backup and not fake:
                diff_msg = (
                    "comparing module file with backup %s; " % self.mod_file_backup
                )
    ...
    def permissions_step(self):
        ...
        if self.group is not None:
            ...
            try:
                ...
                raise EasyBuildError(
                    "Unable to change group permissions of file(s): %s", err
                )
            self.log.info(
                "Successfully made software only available for group %s (gid %s)"
                % self.group
            )
        ...
        if build_option("read_only_installdir"):
            ...
            self.log.info(
                "Successfully removed write permissions recursively for *EVERYONE* on install dir."
            )
            ...
            self.log.info(
                "Successfully enabled write permissions recursively for group on install dir."
            )
            ...
            self.log.info(
                "Successfully removed write permissions recursively for group/other on install dir."
            )
        ...
        self.log.debug(
            "Ensuring read permissions for user/group on install dir (recursively)"
        )
        ...
        if self.group is None:
            ...
            self.log.debug(
                "Also ensuring read permissions for others on install dir (no group specified)"
            )
        ...
        if umask is not None:
            ...
            self.log.debug(
                "Taking umask '%s' into account when ensuring read permissions to install dir",
                umask,
            )

        self.log.debug(
            "Adding file read permissions in %s using '%s'", self.installdir, oct(perms)
        )
        adjust_permissions(
            self.installdir,
            perms,
        ...
        self.log.debug(
            "Adding directory search permissions in %s using '%s'",
            self.installdir,
            oct(dir_perms),
        )
        adjust_permissions(
        ...
        self.log.info(
            "Successfully added read permissions recursively on install dir %s",
            self.installdir,
        )
    ...
    def _skip_step(self, step, skippable):
        ...
        if skippable and (cli_skip or step in self.cfg["skipsteps"]):
            self.log.info(
                "Skipping %s step (skip: %s, skipsteps: %s)",
                step,
                self.skip,
                self.cfg["skipsteps"],
            )
        ...
        # allow skipping sanity check too when only generating module and force is used
            ...
            self.log.debug(
                "Not skipping %s step (skippable: %s, skip: %s, skipsteps: %s, module_only: %s, force: %s",
                step,
                skippable,
                self.skip,
                self.cfg["skipsteps"],
    ...
    def run_step(self, step, step_methods):
        ...
        for step_method in step_methods:
            self.log.info(
                "Running method %s part of step %s"
                % (extract_method_name(step_method), step)
            )
    ...
    def get_steps(run_test_cases=True, iteration_count=1):
        ...
        def get_step(tag, descr, substeps, skippable, initial=True):
            ...
            substeps = [
                substep
                for (always_include, substep) in substeps
                if (initial or always_include)
            ]
        ...
        def source_step_spec(initial):
            ...
            return get_step(
                SOURCE_STEP, "unpacking", source_substeps, True, initial=initial
            )
        ...
        def install_step_spec(initial):
            ...
            return get_step(
                INSTALL_STEP, "installing", install_substeps, True, initial=initial
            )
        ...
        prepare_step_spec = (
            PREPARE_STEP,
            "preparing",
            [lambda x: x.prepare_step],
            False,
        )
        configure_step_spec = (
            CONFIGURE_STEP,
            "configuring",
            [lambda x: x.configure_step],
            True,
        )
        ...
        if run_test_cases:
            steps.append(
                (
                    TESTCASES_STEP,
                    "running test cases",
                    [lambda x: x.load_module, lambda x: x.test_cases_step,],
                    False,
                )
            )
    ...
    def run_all_steps(self, run_test_cases):
        ...
        steps = self.get_steps(
            run_test_cases=run_test_cases, iteration_count=self.det_iter_cnt()
        )

        print_msg(
            "building and installing %s..." % self.full_mod_name,
            log=self.log,
            silent=self.silent,
        )
...
def print_dry_run_note(loc, silent=True):
    ...
    msg = "\n".join(
        [
            ...
            "(slightly) from what is outlined %s, due to conditions in the easyblock which are"
            % loc,
            "incorrectly handled in a dry run.",
            "Any errors that may occur are ignored and reported as warnings, on a per-step basis.",
            "Please be aware of this, and only use the information %s for quick debugging purposes."
            % loc,
            "",
        ]
    )
...
def build_and_install_one(ecdict, init_env):
    ...
    try:
        ...
        _log.info(
            "Obtained application instance of for %s (easyblock: %s)"
            % (name, easyblock)
        )
    except EasyBuildError as err:
        print_error(
            "Failed to get application instance for %s (easyblock: %s): %s"
            % (name, easyblock, err.msg),
            silent=silent,
        )
        ...
        if app.cfg["stop"]:
            ...
            if build_option("read_only_installdir"):
                ...
                if os.path.exists(new_log_dir):
                    adjust_permissions(
                        new_log_dir, stat.S_IWUSR, add=True, recursive=False
                    )
                else:
                    adjust_permissions(
                        app.installdir, stat.S_IWUSR, add=True, recursive=False
                    )
                    mkdir(new_log_dir, parents=True)
                    adjust_permissions(
                        app.installdir, stat.S_IWUSR, add=False, recursive=False
                    )
            ...
            if os.path.exists(archive_reprod_dir):
                ...
                _log.info(
                    "Existing reproducability directory %s backed up to %s",
                    archive_reprod_dir,
                    backup_dir,
                )
    ...
    print_msg(
        "%s: Installation %s %s (took %s)" % (summary, ended, succ, req_time),
        log=_log,
        silent=silent,
    )
    ...
    if filetools.errors_found_in_log > 0:
        print_msg(
            "WARNING: %d possible error(s) were detected in the "
    ...
    if dry_run:
        ...
        if app.ignored_errors:
            dry_run_warning(
                "One or more errors were ignored, see warnings above", silent=silent
            )
        else:
...
def reproduce_build(app, reprod_dir_root):
    ...
    for easyblock_class in inspect.getmro(type(app)):
        ...
        if easyblock_class.__name__ in [EasyBlock.__name__, "ExtensionEasyBlock"]:
            ...
            copy_file(
                easyblock_path, os.path.join(reprod_easyblock_dir, easyblock_filename)
            )
            _log.info(
                "Dumped easyblock %s required for reproduction to %s",
                easyblock_filename,
                reprod_easyblock_dir,
            )
    ...
    if hooks_path:
        ...
        _log.info(
            "Dumped hooks file %s which is (potentially) required for reproduction to %s",
            hooks_path,
            target,
        )
...
def build_easyconfigs(easyconfigs, output_dir, test_results):
    ...
    for app in apps:
        ...
        if app:
            applog = os.path.join(
                output_dir, "%s-%s.log" % (app.name, det_full_ec_version(app.cfg))
            )
            ...
            if app not in build_stopped:
                # gather build stats
                buildstats = get_build_stats(
                    app, start_time, build_option("command_line")
                )
                succes.append((app, buildstats))

    for result in test_results:
        _log.info(
            "%s crashed with an error during fase: %s, error: %s, log file: %s" % result
        )
...
def inject_checksums(ecs, checksum_type):
        ...
        for fn, checksum in checksums:
            ...
            if len(checksum_line) > MAX_LINE_LENGTH:
                checksum_lines.extend(
                    ["%s# %s" % (line_indent, fn), "%s'%s'," % (line_indent, checksum),]
                )
            else:
    ...
    for ec in ecs:
        ...
        print_msg(
            "injecting %s checksums in %s" % (checksum_type, ec["spec"]), log=_log
        )
        ...
        if found_checksums:
            if build_option("force"):
                print_warning(
                    "Found existing checksums in %s, overwriting them (due to use of --force)..."
                    % ec_fn
                )
            else:
                raise EasyBuildError(
                    "Found existing checksums, use --force to overwrite them"
                )
        ...
        print_msg(
            "injecting %s checksums for sources & patches in %s..."
            % (checksum_type, ec_fn),
            log=_log,
        )
        checksums = []
        for entry in app.src + app.patches:
            ...
            print_msg(
                "* %s: %s" % (os.path.basename(entry["path"]), checksum), log=_log
            )
        ...
        if app.cfg["checksums"]:
            _log.debug(
                "Removing existing 'checksums' easyconfig parameter definition..."
            )
        ...
        if app.src:
            ...
            _log.debug(
                "Raw lines for %s easyconfig parameters: %s", "/".join(keys), raw
            )
            ...
            ectxt = regex.sub(
                source_urls_raw + sources_raw + patches_raw + checksums_txt + "\n",
                ectxt,
                count=1,
            )
        ...
        if app.exts:
            print_msg(
                "injecting %s checksums for extensions in %s..."
                % (checksum_type, ec_fn),
                log=_log,
            )
            ...
            for ext in app.exts:
                ...
                if list(ext.keys()) == ["name"]:
                    ...
                    exts_list_lines.append(
                        "%s(%s, %s," % (INDENT_4SPACES, ext_name, ext_version)
                    )
                    ...
                    for key, val in sorted(ext_options.items()):
                        if key != "checksums" and val != exts_default_options.get(key):
                            ...
                            if isinstance(val, list) and len(val) > 1:
                                exts_list_lines.append(
                                    "%s'%s': [" % (INDENT_4SPACES * 2, key)
                                )
                                exts_list_lines.extend(
                                    make_list_lines(val, indent_level=3)
                                )
                    ...
                    if ext_checksums:
                        if len(ext_checksums) == 1:
                            exts_list_lines.append(
                                "%s'checksums': ['%s'],"
                                % (INDENT_4SPACES * 2, checksum)
                            )
                        else:
                            exts_list_lines.append(
                                "%s'checksums': [" % (INDENT_4SPACES * 2)
                            )
                            exts_list_lines.extend(
                                make_checksum_lines(ext_checksums, indent_level=3)
                            )
