...
DEFAULT_CONFIG = {
    ...
    "docurls": [
        None,
        "List of urls with documentation of the software (not necessarily on homepage)",
        MANDATORY,
    ],
    ...
    "onlytcmod": [
        False,
        (
            "Boolean/string to indicate if the toolchain should only load "
            "the environment with module (True) or also set all other "
            "variables (False) like compiler CC etc (if string: comma "
            "separated list of variables that will be ignored)."
        ),
        TOOLCHAIN,
    ],
    ...
    "bitbucket_account": [
        "%(namelower)s",
        "Bitbucket account name to be used to resolve template values in source"
        " URLs",
        BUILD,
    ],
    "buildopts": [
        "",
        "Extra options passed to make step (default already has -j X)",
        BUILD,
    ],
    "checksums": [[], "Checksums for sources and patches", BUILD],
    "configopts": [
        "",
        "Extra options passed to configure (default already has --prefix)",
        BUILD,
    ],
    "easyblock": [
        None,
        "EasyBlock to use for building; if set to None, an easyblock is selected "
        "based on the software name",
        BUILD,
    ],
    "easybuild_version": [
        None,
        "EasyBuild-version this spec-file was written for",
        BUILD,
    ],
    "fix_perl_shebang_for": [
        None,
        "List of files for which Perl shebang should be fixed "
        "to '#!/usr/bin/env perl' (glob patterns supported)",
        BUILD,
    ],
    "fix_python_shebang_for": [
        None,
        "List of files for which Python shebang should be fixed "
        "to '#!/usr/bin/env python' (glob patterns supported)",
        BUILD,
    ],
    "github_account": [
        "%(namelower)s",
        "GitHub account name to be used to resolve template values in source URLs",
        BUILD,
    ],
    "hidden": [
        False,
        "Install module file as 'hidden' by prefixing its version with '.'",
        BUILD,
    ],
    ...
    "parallel": [
        None,
        (
            "Degree of parallelism for e.g. make (default: based on the number of "
            "cores, active cpuset and restrictions in ulimit)"
        ),
        BUILD,
    ],
    ...
    "runtest": [
        None,
        (
            "Indicates if a test should be run after make; should specify argument "
            'after make (for e.g.,"test" for make test)'
        ),
        BUILD,
    ],
    "sanity_check_commands": [
        [],
        (
            "format: [(name, options)] e.g. [('gzip','-h')]. "
            "Using a non-tuple is equivalent to (name, '-h')"
        ),
        BUILD,
    ],
    "sanity_check_paths": [
    ...
    "tests": [
        [],
        (
            "List of test-scripts to run after install. A test script should return a "
            "non-zero exit status to fail"
        ),
        BUILD,
    ],
    ...
    "unwanted_env_vars": [
        [],
        "List of environment variables that shouldn't be set during build",
        BUILD,
    ],
    "versionprefix": [
        "",
        (
            "Additional prefix for software version "
            "(placed before version and toolchain name)"
        ),
        BUILD,
    ],
    "versionsuffix": [
        "",
        "Additional suffix for software version (placed after toolchain name)",
        BUILD,
    ],
    # FILEMANAGEMENT easyconfig parameters
    "buildininstalldir": [
        False,
        ("Boolean to build (True) or not build (False) in the installation directory"),
        FILEMANAGEMENT,
    ],
    "cleanupoldbuild": [
        True,
        (
            "Boolean to remove (True) or backup (False) the previous build "
    ...
    "cleanupoldinstall": [
        True,
        (
            "Boolean to remove (True) or backup (False) the previous install "
            "directory with identical name or not."
        ),
        FILEMANAGEMENT,
    ],
    "dontcreateinstalldir": [
        False,
        ("Boolean to create (False) or not create (True) the install directory"),
        FILEMANAGEMENT,
    ],
    "keeppreviousinstall": [
        False,
        (
            "Boolean to keep the previous installation with identical "
            "name. Experts only!"
        ),
        FILEMANAGEMENT,
    ],
    "keepsymlinks": [
        False,
        (
            "Boolean to determine whether symlinks are to be kept during copying "
    ...
    "start_dir": [
        None,
        (
            "Path to start the make in. If the path is absolute, use that path. "
            "If not, this is added to the guessed path."
        ),
        FILEMANAGEMENT,
    ],
    ...
    "allow_system_deps": [
        [],
        "Allow listed system dependencies (format: (<name>, <version>))",
        DEPENDENCIES,
    ],
    ...
    "hiddendependencies": [
        [],
        "List of dependencies available as hidden modules",
        DEPENDENCIES,
    ],
    "multi_deps": [
        {},
        "Dict of lists of dependency versions over which to iterate",
        DEPENDENCIES,
    ],
    "multi_deps_load_default": [
        True,
        "Load module for first version listed in multi_deps by default",
        DEPENDENCIES,
    ],
    "osdependencies": [
        [],
        "OS dependencies that should be present on the system",
        DEPENDENCIES,
    ],
    # LICENSE easyconfig parameters
    "group": [
        None,
        "Name of the user group for which the software should be available; "
        "format: string or 2-tuple with group name + custom error for users outside group",
        LICENSE,
    ],
    ...
    "exts_download_dep_fail": [
        False,
        "Fail if downloaded dependencies are detected for extensions",
        EXTENSIONS,
    ],
    "exts_classmap": [
        {},
        "Map of extension name to class for handling build and installation.",
        EXTENSIONS,
    ],
    "exts_defaultclass": [
        None,
    ...
    "exts_filter": [
        None,
        (
            "Extension filter details: template for cmd and input to cmd "
            "(templates for ext_name, ext_version and src)."
        ),
        EXTENSIONS,
    ],
    "exts_list": [
    ...
    "allow_prepend_abs_path": [
        False,
        "Allow specifying absolute paths to prepend in modextrapaths",
        MODULES,
    ],
    "include_modpath_extensions": [
        True,
        "Include $MODULEPATH extensions specified by module naming scheme.",
        MODULES,
    ],
    ...
    "modextravars": [
        {},
        "Extra environment variables to be added to module file",
        MODULES,
    ],
    "modloadmsg": [
        {},
        "Message that should be printed when generated module is loaded",
        MODULES,
    ],
    "modluafooter": [
        "",
        "Footer to include in generated module file (Lua syntax)",
        MODULES,
    ],
    "modaltsoftname": [
        None,
        "Module name to use (rather than using software name",
        MODULES,
    ],
    "modtclfooter": [
        "",
        "Footer to include in generated module file (Tcl syntax)",
        MODULES,
    ],
    "moduleclass": [
        MODULECLASS_BASE,
        "Module class to be used for this software",
        MODULES,
    ],
    "moduleforceunload": [
        False,
        "Force unload of all modules when loading the extension",
        MODULES,
    ],
    "moduleloadnoconflict": [
        False,
        "Don't check for conflicts, unload other versions instead ",
        MODULES,
    ],
    "module_depends_on": [
        False,
        "Use depends_on (Lmod 7.6.1+) for dependencies in generated module "
        "(implies recursive unloading of modules).",
        MODULES,
    ],
    "recursive_module_unload": [
        False,
        "Recursive unload of all dependencies when unloading module",
        MODULES,
    ],
    ...
    "site_contacts": [
        None,
        "String/list of strings with site contacts for the software",
        MODULES,
    ],
    "upstream_contacts": [
        None,
        "String/list of strings with upstream contact addresses "
        "(e.g., support e-mail, mailing list, bugtracker)",
        MODULES,
    ],
    "usage": [None, "Usage instructions for the software", MODULES],
    "whatis": [
        None,
        "List of brief (one line) description entries for the software",
        MODULES,
    ],
    ...
    "block": [
        None,
        "List of other 'block' sections on which this block depends "
        "(only relevant in easyconfigs with subblocks)",
        OTHER,
    ],
    "buildstats": [None, "A list of dicts with build statistics", OTHER],
    "deprecated": [
        False,
        "String specifying reason why this easyconfig file is deprecated "
        "and will be archived in the next major release of EasyBuild",
        OTHER,
    ],
}
...
def get_easyconfig_parameter_default(param):
    ...
    if param not in DEFAULT_CONFIG:
        raise EasyBuildError(
            "Unknown easyconfig parameter: %s (known: %s)",
            param,
            sorted(DEFAULT_CONFIG.keys()),
        )
    else:
        _log.debug(
            "Returning default value for easyconfig parameter %s: %s"
            % (param, DEFAULT_CONFIG[param][0])
        )
