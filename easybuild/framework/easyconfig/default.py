...
DEFAULT_CONFIG = {
    ...
    # TODO not yet in MANDATORY_PARAMS, so not enforced  (only enforced in v2)
    'doc_urls': [None, 'List of urls with documentation of the software (not necessarily on homepage)', MANDATORY],
    ...
    'install_opts': ['', 'Extra options for installation', BUILD],
    'max_parallel': [None, 'Max degree of parallelism', BUILD],
    ...
    'skip_fortran_mod_files_sanity_check': [False, "Skip the check for .mod files in a GCCcore level install", BUILD],
    'skip_steps': [[], "Skip these steps", BUILD],
    ...
    'module_depends_on': [False, 'Use depends_on (Lmod 7.6.1+) for deps in generated module '
                          '(implies recursive unloading of modules).', MODULES],
    'recursive_module_unload': [None, "Recursive unload of all deps when unloading module "
