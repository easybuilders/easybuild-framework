...
TEMPLATE_CONSTANTS = [
    ...
    ("GNU_SOURCE", "https://ftpmirror.gnu.org/gnu/%(namelower)s", "gnu.org source url"),
    (
        "GOOGLECODE_SOURCE",
...
for ext in extensions:
    ...
    TEMPLATE_CONSTANTS += [
        (
            "SOURCE_%s" % suffix,
            "%(name)s-%(version)s." + ext,
            "Source .%s bundle" % ext,
        ),
        (
            "SOURCELOWER_%s" % suffix,
            "%(namelower)s-%(version)s." + ext,
            "Source .%s bundle with lowercase name" % ext,
        ),
    ]
...
def template_constant_dict(config, ignore=None, skip_lower=None):
    ...
    for name in TEMPLATE_NAMES_LOWER:
        ...
        try:
            template_values[
                TEMPLATE_NAMES_LOWER_TEMPLATE % {"name": name}
            ] = value.lower()
        except Exception:
            _log.warning(
                "Failed to get .lower() for name %s value %s (type %s)",
                name,
                value,
                type(value),
            )
    ...
    while value != old_value:
        ...
        for tval, tname in templ_val.items():
            ...
            if tname == "pyshortver" and ("python" + tval) in value:
                value = re.sub(
                    r"(^|\W)python" + re.escape(tval) + r"(\W|$)",
                    r"\1python%(" + tname + r")s\2",
                    value,
                )
...
def template_documentation():
    ...
    for name in TEMPLATE_NAMES_LOWER:
        ...
        doc.append(
            "%s%%(%s)s: lower case of value of %s" % (indent_l1, namelower, name)
        )
