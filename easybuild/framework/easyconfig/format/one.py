...
REFORMAT_THRESHOLD_LENGTH = (
    100  # only reformat lines that would be longer than this amount of characters
)
    ...
    if dep["external_module"]:
        ...
        if dep["toolchain"] != toolchain:
            if dep[SYSTEM_TOOLCHAIN_NAME]:
                ...
                tup += (
                    dep["versionsuffix"],
                    (dep["toolchain"]["name"], dep["toolchain"]["version"]),
                )
...
class FormatOneZero(EasyConfigFormatConfigObj):
    ...
    def get_config_dict(self):
        ...
        if spec_version is not None and not spec_version == cfg["version"]:
            raise EasyBuildError(
                "Requested version %s not available, only %s",
                spec_version,
                cfg["version"],
            )
    ...
    def _reformat_line(self, param_name, param_val, outer=False, addlen=0):
        ...
        if param_name in REFORMAT_SKIPPED_PARAMS:
            ...
            if isinstance(param_val, (list, tuple, dict)) and (
                ...
                if isinstance(param_val, dict):
                    ...
                    for item_key in ordered_item_keys:
                        ...
                        formatted_item_val = self._reformat_line(
                            param_name, item_val, addlen=addlen
                        )
                    ...
                    for item in param_val:
                        ...
                        is_list_of_lists_of_tuples = isinstance(item, list) and all(
            ...
            if (
                isinstance(param_val, string_type)
                and param_name not in DEPENDENCY_PARAMETERS
            ):
    ...
    def _get_item_comments(self, key, val):
        ...
        for comment_key, comment_val in (
            self.comments["iterinline"].get(key, {}).items()
        ):
            if str(val) in comment_key:
    ...
    def _find_defined_params(
        self, ecfg, keyset, default_values, templ_const, templ_val
    ):
        """
                    ...
                    if key in DEPENDENCY_PARAMETERS:
                        if key in ecfg.iterate_options:
                            if "multi_deps" in ecfg:
                                ...
                                valstr = [
                                    [dump_dependency(d, ecfg["toolchain"]) for d in dep]
                                    for dep in val
                                ]
                        else:
                            valstr = [
                        ...
                        valstr = (
                            "{'name': '%(name)s', 'version': '%(version)s'}" % ecfg[key]
                        )
                    else:
                        valstr = quote_py_str(ecfg[key])

                    eclines.extend(
                        self._find_param_with_comments(
                            key, valstr, templ_const, templ_val
                        )
                    )
    ...
    def dump(self, ecfg, default_values, templ_const, templ_val):
        ...
        for key in default_values:
            ...
            if key not in keys_to_ignore and (mandatory or non_default_value):
                dump.extend(
                    self._find_param_with_comments(
                        key, quote_py_str(ecfg[key]), templ_const, templ_val
                    )
                )
        dump.append("")

        # print last parameters
        params, _ = self._find_defined_params(
            ecfg, [[k] for k in LAST_PARAMS], default_values, templ_const, templ_val
        )
        dump.extend(params)
    ...
    def extract_comments(self, rawtxt):
        ...
        def grab_more_comment_lines(lines, param_key):
            ...
            while lines and (
                comment_regex.match(lines[0]) or whitespace_regex.match(lines[0])
            ):
                line = lines.pop(0)
            ...
            if rawline.startswith("#"):
                ...
                if rawlines:
                    ...
                    if res:
                        ...
                        self.comments["iterabove"].setdefault(last_param_key, {})[
                            comment_key
                        ] = comment
                else:
                ...
                # then we have an indented comment, and we need to figure out for what exactly
                    ...
                    if inline_comment:
                        ...
                        self.comments["iterinline"].setdefault(last_param_key, {})[
                            comment_key
                        ] = inline_comment
                else:
...
def retrieve_blocks_in_spec(spec, only_blocks, silent=False):
    ...
    if pieces and ec_format_version == EasyVersion("1.0"):
        ...
        for block in blocks:
            ...
            (fd, block_path) = tempfile.mkstemp(
                prefix="easybuild-", suffix="%s-%s" % (spec_fn, name)
            )
            os.close(fd)
