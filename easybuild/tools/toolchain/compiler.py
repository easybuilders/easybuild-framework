...
class Compiler(Toolchain):
    ...
    def _set_compiler_flags(self):
        ...
        if self.options.get('openmp') is not None:
            ...
            if self.options['openmp'] not in openmpoptions:
                raise EasyBuildError(f"Unknown value for openmp: {self.options['openmp']} "
                                     f"(possibilities are {', '.join(str(key) for key in openmpoptions)}).")
        ...
        if self.options.get('vectorize') is not None:
            ...
            if self.options['vectorize'] not in vectoptions:
                raise EasyBuildError(f"Unknown value for 'vectorize': {self.options['vectorize']} "
                                     f"(possibilities are {', '.join(str(key) for key in vectoptions)}).")
