...
class Compiler(Toolchain):
    ...
    COMPILER_SHARED_OPTS = {
        ...
        ),  # fortran only -> no: MKL and icc give -DMKL_ILP64
    ...
    def set_options(self, options):
        ...
        self.log.devel(
            "_compiler_set_options: compiler toolchain options %s", self.options
        )
    ...
    def _set_compiler_toolchainoptions(self):
        ...
        self.options.add_options(
            self.COMPILER_SHARED_OPTS, self.COMPILER_SHARED_OPTION_MAP
        )
    ...
    def _set_compiler_vars(self):
        ...
        if is32bit:
            self.log.debug(
                "_set_compiler_vars: 32bit set: changing compiler definitions"
            )
        ...
        for infix in [""] + [mk_infix(prefix) for prefix in self.prefixes]:
                ...
                if value is None:
                    if infix is None:
                        raise EasyBuildError(
                            "_set_compiler_vars: compiler variable %s undefined", var
                        )
                    else:
                        # only warn if infix is set, not all languages may be supported (e.g., no Fortran for CUDA)
                        self.log.warning(
                            "_set_compiler_vars: %s compiler variable %s undefined",
                            infix,
                            var,
                        )
            ...
            for var_tuple in COMPILER_VARIABLES:
                ...
                val = self.options.option(
                    "_opt_%s" % pref_var, templatedict=comp_var_tmpl_dict
                )
        ...
        if self.options.get("cciscxx", None):
            self.log.debug(
                "_set_compiler_vars: cciscxx set: switching CXX %s for CC value %s"
    ...
    def _set_compiler_flags(self):
        ...
        flags = [
            ...
            for x in self.COMPILER_FLAGS
            if self.options.get(x, False)
        ]
        cflags = [
            self.options.option(x)
            for x in self.COMPILER_C_FLAGS + self.COMPILER_C_UNIQUE_FLAGS
            if self.options.get(x, False)
        ]
        fflags = [
            self.options.option(x)
            for x in self.COMPILER_F_FLAGS + self.COMPILER_F_UNIQUE_FLAGS
            if self.options.get(x, False)
        ]
        ...
        if default_opt_level not in self.COMPILER_OPT_FLAGS:
            raise EasyBuildError(
                "Unknown value for default optimisation: %s (possibilities are %s)"
        ...
        optflags = (
            [
                self.options.option(x)
                for x in self.COMPILER_OPT_FLAGS
                if self.options.get(x, False)
            ]
        ...
        if self.options.get("vectorize") is not None:
            ...
            if isinstance(optflags[0], list):
                optflags[0] = [
                    flag for flag in optflags[0] if flag not in vectoptions.values()
                ]
        ...
        precflags = [
            self.options.option(x)
            for x in self.COMPILER_PREC_FLAGS
            if self.options.get(x, False)
    ...
    def _set_optimal_architecture(self, default_optarch=None):
        ...
        if isinstance(ec_optarch, string_type):
            if OPTARCH_MAP_CHAR in ec_optarch:
                error_msg = (
                    "When setting optarch in the easyconfig (found %s), " % ec_optarch
                )
                error_msg += (
                    "the <compiler%sflags> syntax is not allowed. " % OPTARCH_MAP_CHAR
                )
                error_msg += (
                    "Use <flags> (omitting the first dash) for the specific compiler."
                )
        ...
        if optarch is not None and isinstance(optarch, dict):
            ...
            if isinstance(optarch, dict):
                optarch = None
                self.log.info(
                    "_set_optimal_architecture: no optarch found for compiler %s. Ignoring option.",
                    current_compiler,
                )
        ...
        if optarch is not None:
            ...
            if isinstance(optarch, string_type):
                ...
                raise EasyBuildError(
                    "optarch is neither an string or a dict %s. This should never happen",
                    optarch,
                )
        ...
        elif optarch is None and (self.arch, self.cpu_family) in (
            self.COMPILER_OPTIMAL_ARCHITECTURE_OPTION or []
        ):
            optarch = self.COMPILER_OPTIMAL_ARCHITECTURE_OPTION[
        ...
        if self.options.options_map.get("optarch", None) is None:
            ...
            error_msg = "Don't know how to set %s for %s/%s! " % (
                optarch_flags_str,
                self.arch,
                self.cpu_family,
            )
            ...
            error_msg += (
                "for details) and consider contributing your settings back (see "
            )
