    ...
    if is_single_value:
        ...
        for key, compiler_opt in optarch_parts:
            ...
            if arch_specs in compiler_dict:
                if arch_specs is None:
                    error = "The optarch option contains a duplicated entry"
                else:
                    error = f"The optarch option contains a duplicated entry {arch_specs}"
                error += f"for compiler {compiler}: {optarch}"
                raise EasyBuildError(error, exit_code=EasyBuildExit.OPTION_ERROR)
            else:
