...
from easybuild.tools.toolchain.constants import (
    COMPILER_VARIABLES,
    MPI_COMPILER_TEMPLATE,
    SEQ_COMPILER_TEMPLATE,
)
...
class Mpi(Toolchain):
    ...
    def _set_mpi_compiler_variables(self):
        ...
        if is32bit:
            self.log.debug(
                "_set_mpi_compiler_variables: 32bit set: changing compiler definitions"
            )
        ...
        for var_tuple in COMPILER_VARIABLES:
            ...
            if value is None:
                raise EasyBuildError(
                    "_set_mpi_compiler_variables: mpi compiler variable %s undefined",
                    var,
                )
            ...
            self.variables.nappend_el(
                var, self.options.option("_opt_%s" % var, templatedict=templatedict)
            )
            ...
            if self.options.get("usempi", None):
                ...
                self.log.debug(
                    "usempi set: defining %s as %s", var_seq, self.variables[c_var]
                )
                self.variables[var_seq] = self.variables[c_var]
                self.log.debug(
                    "usempi set: switching %s value %s for %s value %s",
                    c_var,
                    self.variables[c_var],
                    var,
                    self.variables[var],
                )
        ...
        if self.options.get("cciscxx", None):
            self.log.debug(
                "_set_mpi_compiler_variables: cciscxx set: switching MPICXX %s for MPICC value %s",
                self.variables["MPICXX"],
                self.variables["MPICC"],
            )
    ...
    def mpi_cmd_for(self, cmd, nr_ranks):
        ...
        if mpi_cmd_template:
            self.log.info(
                "Using specified template for MPI commands: %s", mpi_cmd_template
            )
        else:
        ...
        if mpi_cmd_template is None:
            ...
            if mpi_family == toolchain.INTELMPI:
                ...
                if LooseVersion(impi_ver) <= LooseVersion("4.1"):
                    ...
                    mpi_cmds[
                        toolchain.INTELMPI
                    ] = "mpirun %(mpdbf)s %(nodesfile)s -np %(nr_ranks)d %(cmd)s"
                    ...
                    if len(mpd_tmpdir) > 20:
                        self.log.warning(
                            "$I_MPI_MPD_TMPDIR should be (very) short to avoid problems: %s",
                            mpd_tmpdir,
                        )
            ...
            if mpi_family in mpi_cmds.keys():
                ...
                self.log.info(
                    "Using template MPI command '%s' for MPI family '%s'",
                    mpi_cmd_template,
                    mpi_family,
                )
            else:
                raise EasyBuildError(
                    "Don't know which template MPI command to use for MPI family '%s'",
                    mpi_family,
                )
        ...
        try:
            ...
            raise EasyBuildError(
                "Failed to complete MPI cmd template '%s' with %s: %s",
                mpi_cmd_template,
                params,
                err,
            )
