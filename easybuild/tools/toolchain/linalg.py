...
class LinAlg(Toolchain):
    ...
    def _set_blas_variables(self):
        ...
        self.BLAS_LIB = self.variables.nappend(
            "LIBBLAS", [x % self.BLAS_LIB_MAP for x in self.BLAS_LIB]
        )
        self.variables.add_begin_end_linkerflags(
            self.BLAS_LIB,
        ...
        if self.BLAS_LIB_MT is None:
            ...
            self.BLAS_LIB_MT = self.variables.nappend(
                "LIBBLAS_MT", [x % self.BLAS_LIB_MAP for x in self.BLAS_LIB_MT]
            )
            self.variables.add_begin_end_linkerflags(
                self.BLAS_LIB_MT,
                toggle_startstopgroup=self.BLAS_LIB_GROUP,
        ...
        self._add_dependency_variables(
            self.BLAS_MODULE_NAME, ld=self.BLAS_LIB_DIR, cpp=self.BLAS_INCLUDE_DIR
        )
    ...
    def _set_lapack_variables(self):
        ...
        """
        self.log.devel(
            "_set_lapack_variables: LAPACK_IS_BLAS %s LAPACK_REQUIRES %s",
            self.LAPACK_IS_BLAS,
            self.LAPACK_REQUIRES,
        )
        if self.LAPACK_IS_BLAS:
            ...
            if self.LAPACK_REQUIRES is not None:
                self.variables.join(
                    "LIBLAPACK", "LIBLAPACK_ONLY", *self.LAPACK_REQUIRES
                )
                ...
                if getattr(self, "LIB_MULTITHREAD", None) is not None:
                    self.variables.nappend(
                        "LIBLAPACK_MT", self.LIB_MULTITHREAD, position=10
                    )
            ...
            for root in self.get_software_root(self.LAPACK_MODULE_NAME):
                self.variables.append_exists(
                    "LAPACK_LIB_DIR", root, self.LAPACK_LIB_DIR
                )
                self.variables.append_exists(
                    "LAPACK_INC_DIR", root, self.LAPACK_INCLUDE_DIR
                )
        ...
        self.variables.join(
            "BLAS_LAPACK_STATIC_LIBS", "LAPACK_STATIC_LIBS", "BLAS_STATIC_LIBS"
        )
        self.variables.join(
            "BLAS_LAPACK_MT_STATIC_LIBS", "LAPACK_MT_STATIC_LIBS", "BLAS_MT_STATIC_LIBS"
        )

        ## add general dependency variables
        self._add_dependency_variables(
            self.LAPACK_MODULE_NAME, ld=self.LAPACK_LIB_DIR, cpp=self.LAPACK_INCLUDE_DIR
        )
    ...
    def _set_blacs_variables(self):
        ...
        ## BLACS
        self.BLACS_LIB = self.variables.nappend(
            "LIBBLACS", [x % lib_map for x in self.BLACS_LIB]
        )
        ...
        ## add general dependency variables
        self._add_dependency_variables(
            self.BLACS_MODULE_NAME, ld=self.BLACS_LIB_DIR, cpp=self.BLACS_INCLUDE_DIR
        )
    ...
    def _set_scalapack_variables(self):
        ...
        self.SCALAPACK_LIB = self.variables.nappend(
            "LIBSCALAPACK_ONLY", [x % lib_map for x in self.SCALAPACK_LIB]
        )
        self.variables.add_begin_end_linkerflags(
            self.SCALAPACK_LIB,
        ...
        if self.SCALAPACK_LIB_MT is None:
            ...
            self.SCALAPACK_LIB_MT = self.variables.nappend(
                "LIBSCALAPACK_MT_ONLY", [x % lib_map for x in self.SCALAPACK_LIB_MT]
            )
            self.variables.add_begin_end_linkerflags(
                self.SCALAPACK_LIB_MT,
                toggle_startstopgroup=self.SCALAPACK_LIB_GROUP,
                toggle_staticdynamic=self.SCALAPACK_LIB_STATIC,
            )
        ...
        if self.SCALAPACK_REQUIRES is not None:
            ...
            self.variables.join(
                "LIBSCALAPACK", "LIBSCALAPACK_ONLY", *scalapack_requires
            )
            scalapack_mt = ["%s_MT" % x for x in scalapack_requires]
            self.variables.join(
                "LIBSCALAPACK_MT", "LIBSCALAPACK_MT_ONLY", *scalapack_mt
            )
            if getattr(self, "LIB_MULTITHREAD", None) is not None:
                self.variables.nappend("LIBSCALAPACK_MT", self.LIB_MULTITHREAD)
        else:
            raise EasyBuildError(
                "_set_scalapack_variables: LIBSCALAPACK without SCALAPACK_REQUIRES not implemented"
            )
        ...
        for root in self.get_software_root(self.SCALAPACK_MODULE_NAME):
            self.variables.append_exists(
                "SCALAPACK_LIB_DIR", root, self.SCALAPACK_LIB_DIR
            )
            self.variables.append_exists(
                "SCALAPACK_INC_DIR", root, self.SCALAPACK_INCLUDE_DIR
            )

        self._add_dependency_variables(
            self.SCALAPACK_MODULE_NAME,
            ld=self.SCALAPACK_LIB_DIR,
            cpp=self.SCALAPACK_INCLUDE_DIR,
        )
