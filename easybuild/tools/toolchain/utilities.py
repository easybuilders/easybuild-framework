...
def search_toolchain(name):
    ...
    if not hasattr(package, check_attr_name) or not getattr(package, check_attr_name):
            ...
            for elem in [getattr(tc_mod, x) for x in dir(tc_mod)]:
                if hasattr(elem, "__module__"):
                    ...
                    if not tc_mod.__file__ == sys.modules[elem.__module__].__file__:
                        ...
                        _log.debug(
                            "Adding %s to list of imported classes used for looking for constants",
                            elem_name,
                        )
                        mod_classes.append(elem)

            # look for constants in modules of imported classes, and make them available
            for mod_class_mod in [
                sys.modules[mod_class.__module__] for mod_class in mod_classes
            ]:
                for elem in dir(mod_class_mod):
                    ...
                    if res:
                        ...
                        _log.debug(
                            "Found constant %s ('%s') in module %s, adding it to %s",
                            tc_const_name,
                            tc_const_value,
                        ...
                        if hasattr(package, tc_const_name):
                            ...
                            if not tc_const_value == cur_value:
                                raise EasyBuildError(
                                    "Constant %s.%s defined as '%s', can't set it to '%s'.",
                                    package.__name__,
                                    tc_const_name,
                                    cur_value,
                                    tc_const_value,
        ...
        _log.debug(
            "Skipping importing of toolchain modules, processing of toolchain constants is already done."
        )
...
def get_toolchain(tc, tcopts, mns=None, tcdeps=None, modtool=None):
    ...
    if key in _initial_toolchain_instances:
        ...
        _log.debug(
            "Obtained cached toolchain instance for %s: %s" % (key, tc_inst.as_dict())
        )
    else:
        ...
        if not tc_class:
            ...
            raise EasyBuildError(
                "Toolchain %s not found, available toolchains: %s",
                tc["name"],
                all_tcs_names,
            )
