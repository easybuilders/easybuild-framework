...
def get_avail_core_count():
    ...
    if os_type == LINUX:
        ...
        # BSD-type systems
        out, _ = run_cmd('sysctl -n hw.ncpu', forced=True)
        try:
            if int(out) > 0:
                core_cnt = int(out)
        except ValueError:
...
def get_cpu_vendor():
    ...
    if os_type == LINUX and os.path.exists(PROC_CPUINFO_FP):
        ...
        out, ec = run_cmd(cmd, forced=True)
        out = out.strip()
...
def check_os_dependency(dep):
    ...
    if which('rpm'):
        cmd = "rpm -q %s" % dep
        found = run_cmd(cmd, simple=True, log_all=False, log_ok=False, forced=True)
...
def get_tool_version(tool, version_option='--version'):
    ...
    """
    out, ec = run_cmd(' '.join([tool, version_option]), simple=False, log_ok=False, forced=True)
    if ec:
...
def det_parallelism(par=None, maxpar=None):
    ...
    if par is not None:
        ...
        # check ulimit -u
        out, ec = run_cmd('ulimit -u', forced=True)
        try:
