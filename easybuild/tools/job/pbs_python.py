...
try:
    ...
    _log.debug(
        "Failed to import pbs/PBSQuery from pbs_python."
...
class PbsPython(JobBackend):
        ...
        if res:
            ...
            if LooseVersion(version) < LooseVersion(self.REQ_VERSION):
                raise EasyBuildError(
                    "Found pbs_python version %s, but version %s or more recent is required",
                    version,
                    self.REQ_VERSION,
                )
        else:
            raise EasyBuildError(
                "Failed to parse pbs_python version string '%s' using pattern %s",
                pbs.version,
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.pbs_server = (
            pbs_server or build_option("job_target_resource") or pbs.pbs_default()
        )
        self.conn = None
        self._ppn = None
    ...
    def _get_ppn(self):
        ...
        if not self._ppn:
            ...
            res = {}
            for np in [
                int(x["np"][0]) for x in node_vals if x["state"][0] in interesting_nodes
            ]:
                res.setdefault(np, 0)
                res[np] += 1
...
class PbsJob(object):
    ...
    def __init__(
        ...
        if cores > max_cores:
            self.log.warning(
                "number of requested cores (%s) was greater than available (%s) "
                % (cores, max_cores)
            )
            cores = max_cores
    ...
    def __str__(self):
        ...
        return str(self.jobid) if self.jobid is not None else repr(self)
    ...
    def _submit(self):
        ...
        if self.deps:
            ...
            deps_attributes[0].value = ",".join(
                [self.job_deps_type + ":" + dep.jobid for dep in self.deps]
            )
        ...
        pbsvars = [
            "PBS_O_%s=%s" % (x, os.environ.get(x, "NOTFOUND_%s" % x)) for x in defvars
        ]
    ...
    def set_hold(self, hold_type=None):
        ...
        if hold_type not in self.holds:
            ...
            self.log.warning(
                "Hold type %s was already set for %s" % (hold_type, self.jobid)
            )
    ...
    def release_hold(self, hold_type=None):
        ...
        if hold_type in self.holds:
            ...
            self.log.debug(
                "Released hold of type %s for job %s" % (hold_type, self.jobid)
            )
            is_error, errormsg = pbs.error()
            if is_error or ec:
                raise EasyBuildError(
    ...
    def state(self):
        ...
        self.log.debug(
            "Jobid %s jid %s state %s ehosts %s (%s)"
            % (self.jobid, jid, jstate, ehosts, state)
        )
        if jstate == "Q":
            return "queued"
        else:
    ...
    def info(self, types=None):
        ...
        if len(jobs) == 0:
            ...
            self.log.debug(
                "Request for jobid %s returned one result %s" % (self.jobid, jobs)
            )
        else:
            raise EasyBuildError(
    ...
    def remove(self):
        ...
        result = pbs.pbs_deljob(
            self.pbsconn, self.jobid, ""
        )  # use empty string, not NULL
        if result:
            raise EasyBuildError(
                "Failed to delete job %s: error %s", self.jobid, result
            )
        else:
