...
try:
    ...
    _log.debug(
        "Failed to import gc3libs from GC3Pie."
...
class GC3Pie(JobBackend):
    ...
    def _check_version(self):
        ...
        try:
            ...
            print_warning(
                "Failed to check required GC3Pie version (>= %s)", self.REQ_VERSION
            )
    ...
    def make_job(self, script, name, env_vars=None, hours=None, cores=None):
        ...
        named_args.update(
            {
                ...
                "stdout": "eb-%s-gc3pie-job-%s.log"
                % (name, strftime("%Y%M%d-UTC-%H-%M-%S", gmtime())),
            }
        )
        ...
        if cores:
            ...
            self.log.warning(
                "Number of cores to request not specified, falling back to GC3Pie default"
            )
    ...
    def complete(self):
        ...
        try:
            self._engine = create_engine(
                *self.config_files, resource_errors_are_fatal=True
            )
        ...
        if target_resource:
            ...
            if res == 0:
                raise EasyBuildError(
                    "Failed to select target resource '%s' in GC3Pie", target_resource
                )
        ...
        if stats["failed"] > 0:
            error_msg = "%d jobs failed: %s" % (
                stats["failed"],
                ", ".join(self._list_failed_jobs()),
            )
    ...
    def _print_status_report(self, stats):
        ...
        print_msg(
            "GC3Pie job overview: %s (total: %s)" % (states, self.job_cnt),
            log=self.log,
