...
def module_generator(app, fake=False):
    ...
    if module_syntax not in available_mod_gens:
        raise EasyBuildError(
            "No module generator available for specified syntax '%s' (available: %s)",
            module_syntax,
            available_mod_gens,
        )
...
class ModuleGenerator(object):
    ...
    def append_paths(self, key, paths, allow_abs=False, expand_relpaths=True):
        ...
        return self.update_paths(
            ...
            prepend=False,
            allow_abs=allow_abs,
            expand_relpaths=expand_relpaths,
        )
    ...
    def create_symlinks(self, mod_symlink_paths, fake=False):
        ...
        class_mod_files = [
            self.get_module_filepath(fake=fake, mod_path_suffix=p)
        ...
        try:
            ...
            raise EasyBuildError(
                "Failed to create symlinks from %s to %s: %s",
                class_mod_files,
                mod_filepath,
                err,
            )
    ...
    def get_modules_path(self, fake=False, mod_path_suffix=None):
        ...
        if fake:
            self.log.debug(
                "Fake mode: using %s (instead of %s)" % (self.fake_mod_path, mod_path)
            )
        ...
        if module_version:
            if isinstance(module_version, dict):
                ...
                if sorted(module_version.keys()) == expected_keys:
                    ...
                    raise EasyBuildError(
                        "Incorrect module_version spec, expected keys: %s",
                        expected_keys,
                    )
            else:
                raise EasyBuildError(
                    "Incorrect module_version value type: %s", type(module_version)
                )
    ...
    def _write_modulerc_file(self, modulerc_path, modulerc_txt, wrapped_mod_name=None):
        ...
        if wrapped_mod_name is not None:
            if isinstance(self.modules_tool, Lmod) and LooseVersion(
                self.modules_tool.version
            ) < LooseVersion("7.0"):
                ...
                wrapped_mod_fp = os.path.join(
                    mod_dir, os.path.basename(wrapped_mod_name)
                )
                wrapped_mod_exists = os.path.exists(wrapped_mod_fp)
                if not wrapped_mod_exists and self.MODULE_FILE_EXTENSION:
                    wrapped_mod_exists = os.path.exists(
                        wrapped_mod_fp + self.MODULE_FILE_EXTENSION
                    )
        ...
        if os.path.exists(modulerc_path):
            ...
            if modulerc_txt.startswith(tcl_shebang) and curr_modulerc.startswith(
                tcl_shebang
            ):
    ...
    def modulerc(self, module_version=None, filepath=None, modulerc_txt=None):
        ...
        if modulerc_txt is None:
            ...
            self.log.info(
                "Generating .modulerc contents in Tcl syntax (args: module_version: %s",
                module_version,
            )
            ...
            if self._modulerc_check_module_version(module_version):
                ...
                if self.modules_tool.__class__ == EnvironmentModulesC:
                    ...
                    modname, sym_version, version = [
                        module_version[key] for key in keys
                    ]
                    ...
                    if version in modname:
                        ...
                        module_version["sym_modname"] = modname.replace(
                            version, sym_version
                        )
                    else:
                        raise EasyBuildError(
                            "Version '%s' does not appear in module name '%s'",
                            version,
                            modname,
                        )
        ...
        if filepath:
            ...
            self._write_modulerc_file(
                filepath, modulerc_txt, wrapped_mod_name=module_version["modname"]
            )
    ...
    def check_version(
        self, minimal_version_maj, minimal_version_min, minimal_version_patch="0"
    ):
        """
    ...
    def update_paths(
        self, key, paths, prepend=True, allow_abs=False, expand_relpaths=True
    ):
        """
    ...
    def _generate_extension_list(self):
        ...
        extensions = ", ".join(
            sorted(["-".join(ext[:2]) for ext in exts_list], key=str.lower)
        )
    ...
    def _generate_help_text(self):
        ...
        lines = self._generate_section(
            "Description", self.app.cfg["description"], strip=True
        )
        ...
        # Examples (optional)
        lines.extend(
            self._generate_section("Examples", self.app.cfg["examples"], strip=True)
        )

        # Additional information: homepage + (if available) doc paths/urls, upstream/site contact
        lines.extend(
            self._generate_section(
                "More information", " - Homepage: %s" % self.app.cfg["homepage"]
            )
        )
        ...
        for contacts_type in ["upstream", "site"]:
            ...
            if contacts:
                if isinstance(contacts, list):
                    ...
                    lines.append(
                        " - %s contact: %s" % (contacts_type.capitalize(), contacts)
                    )
        ...
        if multi_deps:
            ...
            lines.extend(
                self._generate_section("Compatible modules", compatible_modules_txt)
            )
...
class ModuleGeneratorTcl(ModuleGenerator):
    ...
    def check_group(self, group, error_msg=None):
        ...
        """
        self.log.warning(
            "Can't generate robust check in TCL modules for users belonging to group %s.",
            group,
        )
        return ""
    ...
    def get_description(self, conflict=True):
        ...
        txt = "\n".join(
            [
                "proc ModulesHelp { } {",
                "    puts stderr {%s"
                % re.sub("([{}\[\]])", r"\\\1", self._generate_help_text()),
                "    }",
                "}",
                "",
            ]
        ...
        if self.app.cfg["moduleloadnoconflict"]:
            cond_unload = self.conditional_statement(
                self.is_loaded("%(name)s"), "module unload %(name)s"
            )
            lines.extend(
                [
                    "",
                    self.conditional_statement(
                        self.is_loaded("%(name)s/%(version)s"),
                        cond_unload,
                        negative=True,
                    ),
                ]
        ...
        whatis_lines = [
            "module-whatis {%s}" % re.sub(r"([{}\[\]])", r"\\\1", l)
            for l in self._generate_whatis_lines()
        ]
        txt += "\n".join([""] + lines + [""]) % {
            "name": self.app.name,
    ...
    def load_module(
        ...
        if build_option("mod_depends_on") or depends_on:
            if not self.modules_tool.supports_depends_on:
                raise EasyBuildError(
                    "depends-on statements in generated module are not supported by modules tool"
                )
        ...
        if depends_on:
            ...
            load_statement = [
                self.conditional_statement(
                    load_guards, body, negative=True, cond_tmpl=cond_tmpl
                )
            ]
    ...
    def msg_on_load(self, msg):
        ...
        return "\n".join(
            [
                "",
                self.conditional_statement(
                    "module-info mode load", print_cmd, indent=False
                ),
            ]
        )
    ...
    def update_paths(
        ...
        if not self.define_env_var(key):
            self.log.info(
                "Not including statement to %s environment variable $%s, as specified",
                update_type,
                key,
            )
            return ""
        ...
        if isinstance(paths, string_type):
            self.log.debug(
                "Wrapping %s into a list before using it to %s path %s",
                paths,
                update_type,
                key,
            )
        ...
        for path in paths:
            if os.path.isabs(path) and not allow_abs:
                raise EasyBuildError(
                    "Absolute path %s passed to update_paths which only expects relative paths.",
                    path,
                )
    ...
    def set_as_default(self, module_dir_path, module_version, mod_symlink_paths=None):
        ...
        for mod_symlink_path in mod_symlink_paths:
            mod_symlink_dir = os.path.join(
                install_path("mod"), mod_symlink_path, module_dir_name
            )
            ...
            if os.path.islink(dot_version_link_path):
                ...
                raise EasyBuildError(
                    "Found an unexpected file named .version in dir %s", mod_symlink_dir
                )
    ...
    def set_environment(self, key, value, relpath=False):
        ...
        if not self.define_env_var(key):
            self.log.info(
                "Not including statement to define environment variable $%s, as specified",
                key,
            )
            return ""
    ...
    def swap_module(self, mod_name_out, mod_name_in, guarded=True):
        ...
        if guarded:
            ...
            swap_statement = [
                self.conditional_statement(
                    self.is_loaded(mod_name_out), body, else_body=alt_body
                )
            ]
        else:
            swap_statement = [body, ""]
    ...
    def use(self, paths, prefix=None, guarded=False, user_modpath=None):
        ...
        for path in paths:
            ...
            if guarded:
                cond_statement = self.conditional_statement(
                    "file isdirectory %s" % full_path, "module use %s" % full_path
                )
...
class ModuleGeneratorLua(ModuleGenerator):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.modules_tool:
            if self.modules_tool.version and LooseVersion(
                self.modules_tool.version
            ) >= LooseVersion("7.7.38"):
    ...
    def check_group(self, group, error_msg=None):
        ...
        if LooseVersion(lmod_version) >= LooseVersion(min_lmod_version):
            if error_msg is None:
                error_msg = (
                    "You are not part of '%s' group of users that have access to this software; "
                    % group
                )
                error_msg += "Please consult with user support how to become a member of this group"

            error_msg = 'LmodError("' + error_msg + '")'
            res = self.conditional_statement(
                'userInGroup("%s")' % group, error_msg, negative=True
            )
        else:
    ...
    def check_str(self, txt):
        ...
        if self.START_STR in txt or self.END_STR in txt:
            raise EasyBuildError(
                "Found unwanted '%s' or '%s' in: %s", self.START_STR, self.END_STR, txt
            )
        else:
            return txt
    ...
    def get_description(self, conflict=True):
        ...
        txt = "\n".join(
            [
                "help(%s%s"
                % (self.START_STR, self.check_str(self._generate_help_text())),
                "%s)" % self.END_STR,
                "",
            ]
        )
        ...
        if self.app.cfg["moduleloadnoconflict"]:
            self.log.info(
                "Nothing to do to ensure no conflicts can occur on load when using Lua modules files/Lmod"
            )

        elif conflict:
            # conflict on 'name' part of module name (excluding version part at the end)
            lines.extend(
                ["", 'conflict("%s")' % os.path.dirname(self.app.short_mod_name)]
            )

        whatis_lines = []
        for line in self._generate_whatis_lines():
            whatis_lines.append(
                "whatis(%s%s%s)" % (self.START_STR, self.check_str(line), self.END_STR)
            )
        ...
        if build_option("module_extensions"):
            ...
            if extensions_list:
                extensions_stmt = 'extensions("%s")' % ",".join(
                    ["%s" % x for x in extensions_list]
                )
                ...
                lines.extend(
                    [
                        "",
                        self.conditional_statement(
                            self.check_version("8", "2", "8"), extensions_stmt
                        ),
                    ]
                )
    ...
    def load_module(
        ...
        if build_option("mod_depends_on") or depends_on:
            if not self.modules_tool.supports_depends_on:
                raise EasyBuildError(
                    "depends_on statements in generated module are not supported by modules tool"
                )
            load_template = self.LOAD_TEMPLATE_DEPENDS_ON
    ...
    def modulerc(self, module_version=None, filepath=None, modulerc_txt=None):
        ...
        if modulerc_txt is None:
            ...
            if LooseVersion(lmod_ver) >= LooseVersion(min_ver):
                self.log.info(
                    "Found Lmod v%s >= v%s, so will generate .modulerc.lua in Lua syntax",
                    lmod_ver,
                    min_ver,
                )
                ...
                if self._modulerc_check_module_version(module_version):
                    module_version_statement = (
                        'module_version("%(modname)s", "%(sym_version)s")'
                    )
                    modulerc.append(module_version_statement % module_version)
                ...
                self.log.info(
                    "Lmod v%s < v%s, need to stick to Tcl syntax for .modulerc",
                    lmod_ver,
                    min_ver,
                )

        return super(ModuleGeneratorLua, self).modulerc(
            module_version=module_version, filepath=filepath, modulerc_txt=modulerc_txt
        )
    ...
    def update_paths(
        ...
        statements = [
            self.UPDATE_PATH_TEMPLATE % (update_type, key, p) for p in abspaths
        ]
        statements.append("")
        return "\n".join(statements)
    ...
    def set_as_default(self, module_dir_path, module_version, mod_symlink_paths=None):
        ...
        def create_default_symlink(path):
            ...
            if os.path.islink(default_filepath):
                ...
                raise EasyBuildError(
                    "Found an unexpected file named default in dir %s", module_dir_path
                )

            symlink(
                module_version + self.MODULE_FILE_EXTENSION,
                default_filepath,
                use_abspath_source=False,
            )
            self.log.info(
                "Module default version file written to point to %s", default_filepath
            )
        ...
        for mod_symlink_path in mod_symlink_paths:
            ...
            mod_symlink_dir = os.path.join(
                install_path("mod"), mod_symlink_path, mod_dir_name
            )
    ...
    def use(self, paths, prefix=None, guarded=False, user_modpath=None):
        ...
        for path in paths:
            ...
            prepend_modulepath = self.UPDATE_PATH_TEMPLATE % (
                "prepend",
                "MODULEPATH",
                full_path,
            )
            if guarded:
                cond_statement = self.conditional_statement(
                    "isDir(%s)" % full_path, prepend_modulepath
                )
                use_statements.append(cond_statement)
            else:
