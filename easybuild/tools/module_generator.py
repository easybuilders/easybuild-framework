...
class ModuleGeneratorTcl(ModuleGenerator):
    ...
    def msg_on_load(self, msg):
        ...
        print_cmd = "puts stderr %s" % quote_str(msg, syntax=self.SYNTAX)
    ...
    def set_alias(self, key, value):
        ...
        return 'set-alias\t%s\t\t%s\n' % (key, quote_str(value, syntax=self.SYNTAX))
    ...
    def set_environment(self, key, value, relpath=False):
        ...
        if relpath:
            if value:
                val = quote_str(os.path.join('$root', value), syntax=self.SYNTAX)
            else:
                val = '"$root"'
        else:
            val = quote_str(value, syntax=self.SYNTAX)
        return 'setenv\t%s\t\t%s\n' % (key, val)
    ...
    def use(self, paths, prefix=None, guarded=False, user_modpath=None):
        ...
        use_statements = []
        for path in paths:
            quoted_path = quote_str(path, syntax=self.SYNTAX)
            if user_modpath:
