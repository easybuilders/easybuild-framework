...
from easybuild.tools.environment import (
    ORIG_OS_ENVIRON,
    restore_env,
    setvar,
    unset_env_vars,
...
class ModulesTool(object):
    ...
    def __init__(self, mod_paths=None, testing=False):
        ...
        if env_cmd_path:
            ...
            if cmd_path is None:
                ...
                self.log.debug(
                    "Set %s command via environment variable %s: %s",
                    self.NAME,
            ...
            # check whether paths obtained via $PATH and $LMOD_CMD are different
                ...
                self.log.debug(
                    "Different paths found for %s command '%s' via which/$PATH and $%s: %s vs %s",
                    self.NAME,
                    self.COMMAND,
                    self.COMMAND_ENVIRONMENT,
                    cmd_path,
    ...
    def set_and_check_version(self):
        ...
        if self.cmd in MODULE_VERSION_CACHE:
            ...
            self.log.debug(
                "Found cached version for %s command %s: %s",
                self.NAME,
                self.COMMAND,
                self.version,
            )
        ...
        try:
            ...
            if res:
                ...
                raise EasyBuildError(
                    "Failed to determine %s version from option '%s' output: %s",
                    self.NAME,
                    self.VERSION_OPTION,
                    txt,
                )
        ...
        if self.REQ_VERSION is None and self.MAX_VERSION is None:
            ...
            self.log.debug(
                "Checking whether %s version %s meets requirements",
                self.NAME,
                self.version,
            )
            ...
            if self.REQ_VERSION is not None:
                self.log.debug(
                    "Required minimum %s version defined: %s",
                    self.NAME,
                    self.REQ_VERSION,
                )
                if StrictVersion(self.version) < StrictVersion(self.REQ_VERSION):
                    raise EasyBuildError(
                        "EasyBuild requires %s >= v%s, found v%s",
                        self.NAME,
                        self.REQ_VERSION,
                        self.version,
                    )
                    ...
                    self.log.debug(
                        "%s version %s matches requirement >= %s",
                        self.NAME,
                        self.version,
                        self.REQ_VERSION,
                    )
            ...
            if self.DEPR_VERSION is not None:
                self.log.debug(
                    "Deprecated %s version limit defined: %s",
                    self.NAME,
                    self.DEPR_VERSION,
                )
                if StrictVersion(self.version) < StrictVersion(self.DEPR_VERSION):
                    depr_msg = "Support for %s version < %s is deprecated, " % (
                        self.NAME,
                        self.DEPR_VERSION,
                    )
                    ...
                    silence_deprecation_warnings = (
                        build_option("silence_deprecation_warnings") or []
                    )

                    if (
                        self.version.startswith("6")
                        and "Lmod6" in silence_deprecation_warnings
                    ):
            ...
            if self.MAX_VERSION is not None:
                self.log.debug(
                    "Maximum allowed %s version defined: %s",
                    self.NAME,
                    self.MAX_VERSION,
                )
                if StrictVersion(self.version) > StrictVersion(self.MAX_VERSION):
                    ...
                    self.log.debug(
                        "Version %s matches requirement <= %s",
                        self.version,
                        self.MAX_VERSION,
                    )
        else:
            self.log.debug(
                "Skipping modules tool version '%s' requirements check", self.version
            )
    ...
    def check_cmd_avail(self):
        ...
        if cmd_path is not None:
            self.cmd = cmd_path
            self.log.info(
                "Full path for %s command is %s, so using it", self.NAME, self.cmd
            )
        else:
            mod_tools = avail_modules_tools().keys()
            error_msg = (
                "%s modules tool can not be used, '%s' command is not available"
                % (self.NAME, self.cmd)
            )
            error_msg += (
                "; use --modules-tool to specify a different modules tool to use (%s)"
                % ", ".join(mod_tools)
            )
    ...
    def check_module_function(self, allow_mismatch=False, regex=None):
        ...
        if ec == 0:
            if mod_cmd_re.search(out):
                self.log.debug(
                    "Found pattern '%s' in defined 'module' function."
                    % mod_cmd_re.pattern
                )
            else:
            ...
            self.log.warning(
                "No 'module' function defined, can't check if it matches %s."
                % mod_details
            )
    ...
    def mk_module_cache_key(self, partial_key):
        ...
        return (
            "MODULEPATH=%s" % os.environ.get("MODULEPATH", ""),
            self.COMMAND,
            partial_key,
        )
    ...
    def set_mod_paths(self, mod_paths=None):
        ...
        self.log.debug(
            "$MODULEPATH after set_mod_paths: %s" % os.environ.get("MODULEPATH", "")
        )
    ...
    def use(self, path, priority=None):
        ...
        if priority:
            self.log.info(
                "Ignoring specified priority '%s' when running 'module use %s' (Lmod-specific)",
                priority,
                path,
            )
    ...
    def prepend_module_path(self, path, set_mod_paths=True, priority=None):
        ...
        if priority:
            self.log.info(
                "Ignoring specified priority '%s' when prepending %s to $MODULEPATH (Lmod-specific)",
                priority,
                path,
            )
    ...
    def check_module_path(self):
        ...
        if self.mod_paths is None:
            ...
            self.set_mod_paths()
            self.log.debug(
                "self.mod_paths set based on $MODULEPATH: %s" % self.mod_paths
            )

            # determine module path for EasyBuild install path to be included in $MODULEPATH
            eb_modpath = os.path.join(
                install_path(typ="modules"), build_option("suffix_modules_path")
            )
        ...
        self.log.debug(
            "Current module paths: %s; target module paths: %s",
            curr_mod_paths,
            self.mod_paths,
        )
        if curr_mod_paths == self.mod_paths:
            self.log.debug(
                "Current value of $MODULEPATH already matches list of module path %s",
                self.mod_paths,
            )
        else:
            ...
            while curr_mod_paths[-idx:] == self.mod_paths[-idx:]:
                idx += 1
            self.log.debug(
                "Not prepending %d last entries of %s", idx - 1, self.mod_paths
            )
            ...
            self.log.info(
                "$MODULEPATH set via list of module paths (w/ 'module use'): %s"
    ...
    def available(self, mod_name=None, extra_args=None):
        ...
        if not mod_name and key in MODULE_AVAIL_CACHE:
            ...
            self.log.debug(
                "Found cached result for 'module avail' with key '%s': %s", key, ans
            )
        else:
            ...
            self.log.debug(
                "'module available %s' gave %d answers: %s" % (mod_name, len(ans), ans)
            )

            if not mod_name:
                MODULE_AVAIL_CACHE[key] = ans
                self.log.debug(
                    "Cached result for 'module avail' with key '%s': %s", key, ans
                )

        return ans

    def module_wrapper_exists(
        self, mod_name, modulerc_fn=".modulerc", mod_wrapper_regex_template=None
    ):
        """
        ...
        if mod_wrapper_regex_template is None:
            mod_wrapper_regex_template = (
                "^[ ]*module-version (?P<wrapped_mod>[^ ]*) %s$"
            )
        ...
        for mod_path in curr_module_paths():
            ...
            if os.path.exists(modulerc_cand):
                ...
                if res:
                    ...
                    self.log.debug(
                        "Confirmed that %s is a module wrapper for %s",
                        mod_name,
                        wrapped_mod,
                    )
                    break
        ...
        if wrapped_mod is not None and not wrapped_mod.startswith(mod_dir):
            ...
            self.log.debug(
                "Full module name prefix mismatch between module wrapper '%s' and wrapped module '%s'",
                mod_name,
                wrapped_mod,
            )
            ...
            wrapped_mod_subdir = ""
            while (
                not os.path.join(wrapped_mod_subdir, wrapped_mod).startswith(mod_dir)
                and mod_name_parts
            ):
                wrapped_mod_subdir = os.path.join(
                    wrapped_mod_subdir, mod_name_parts.pop(0)
                )

            full_wrapped_mod_name = os.path.join(wrapped_mod_subdir, wrapped_mod)
            if full_wrapped_mod_name.startswith(mod_dir):
                self.log.debug(
                    "Full module name for wrapped module %s: %s",
                    wrapped_mod,
                    full_wrapped_mod_name,
                )
                wrapped_mod = full_wrapped_mod_name
            else:
                raise EasyBuildError(
                    "Failed to determine full module name for module wrapped by %s: %s | %s",
                    mod_name,
                    wrapped_mod_subdir,
                    wrapped_mod,
                )
    ...
    def exist(
        self,
        mod_names,
        mod_exists_regex_template=r"^\s*\S*/%s.*:\s*$",
        skip_avail=False,
        maybe_partial=True,
    ):
        """
        ...
        mod_names = [
            (mod_name, not os.path.basename(mod_name).startswith("."))
        ...
        for (mod_name, visible) in mod_names:
            if visible:
                ...
                self.log.debug(
                    "checking whether hidden module %s exists via 'show'..." % mod_name
                )
                mod_exists = mod_exists_via_show(mod_name)
            ...
            if not mod_exists:
                self.log.debug(
                    "Module %s not found via module avail/show, checking whether it is a wrapper",
                    mod_name,
                )
                ...
                if wrapped_mod is not None:
                    ...
                    mod_exists = wrapped_mod in avail_mod_names or mod_exists_via_show(
                        wrapped_mod
                    )
                    self.log.debug(
                        "Result for existence check of wrapped module %s: %s",
                        wrapped_mod,
                        mod_exists,
                    )

            self.log.debug(
                "Result for existence check of %s module: %s", mod_name, mod_exists
            )
    ...
    def load(
        self, modules, mod_paths=None, purge=False, init_env=None, allow_reload=True
    ):
        """
        ...
        for mod_path in mod_paths:
            full_mod_path = os.path.join(
                install_path("mod"), build_option("suffix_modules_path"), mod_path
            )
    ...
    def purge(self):
        ...
        """
        self.log.debug(
            "List of loaded modules before purge: %s" % os.getenv("_LMFILES_")
        )
    ...
    def show(self, mod_name):
        ...
        if key in MODULE_SHOW_CACHE:
            ...
            self.log.debug(
                "Found cached result for 'module show %s' with key '%s': %s",
                mod_name,
                key,
                ans,
            )
            ...
            ans = self.run_module(
                "show", mod_name, check_output=False, return_output=True
            )
            MODULE_SHOW_CACHE[key] = ans
            self.log.debug(
                "Cached result for 'module show %s' with key '%s': %s",
                mod_name,
                key,
                ans,
            )
    ...
    def get_value_from_modulefile(self, mod_name, regex):
        ...
        if self.exist([mod_name], skip_avail=True)[0]:
            ...
            if res:
                ...
                raise EasyBuildError(
                    "Failed to determine value from 'show' (pattern: '%s') in %s",
                    regex.pattern,
                    modinfo,
                )
        else:
            raise EasyBuildError(
                "Can't get value from a non-existing module %s", mod_name
            )
    ...
    def compose_cmd_list(self, args, opts=None):
        ...
        if self.COMMAND_SHELL is not None:
            if not isinstance(self.COMMAND_SHELL, (list, tuple)):
                raise EasyBuildError(
                    "COMMAND_SHELL needs to be list or tuple, now %s (value %s)",
                    type(self.COMMAND_SHELL),
                    self.COMMAND_SHELL,
                )
    ...
    def run_module(self, *args, **kwargs):
        ...
        for key in LD_ENV_VAR_KEYS:
            ...
            self.log.debug(
                "Changing %s from '%s' to '%s' in environment for module command",
                key,
                os.environ.get(key, ""),
                environ[key],
            )
        ...
        self.log.debug(
            "Output of module command '%s': stdout: %s; stderr: %s"
        ...
        if kwargs.get("check_exit_code", True) and exit_code != 0:
            raise EasyBuildError(
                "Module command 'module %s' failed with exit code %s; stderr: %s; stdout: %s",
                " ".join(cmd_list[2:]),
                exit_code,
                stderr,
                stdout,
        ...
        if kwargs.get("return_output", False):
            ...
            prev_ld_values = dict(
                [
                    (key, os.environ.get(key, "").split(os.pathsep)[::-1])
                    for key in LD_ENV_VAR_KEYS
                ]
            )
            ...
            try:
                ...
                raise EasyBuildError(
                    "Changing environment as dictated by module failed: %s (%s)",
                    err,
                    out,
                )
            ...
            for key in LD_ENV_VAR_KEYS:
                ...
                new_ld_val = [
                    x for x in nub(prev_ld_values[key] + curr_ld_val[::-1]) if x
                ][::-1]

                self.log.debug(
                    "Correcting paths in $%s from %s to %s"
                    % (key, curr_ld_val, new_ld_val)
                )
    ...
    def check_loaded_modules(self):
        ...
        """
        allowed_keys = [
            get_software_root_env_var_name(x)
            for x in build_option("allow_loaded_modules") or []
            if x
        ]
        ...
        if eb_module_keys:
            ...
            if eb_module_keys:
                ...
                msg = (
                    "Found defined $%s* environment variables without matching loaded module: %s"
                    % tup
                )
                msg_control = (
                    "\n(control action via --check-ebroot-env-vars={%s})"
                    % ",".join(EBROOT_ENV_VAR_ACTIONS)
                )
                ...
                if action == ERROR:
                    ...
                    print_warning(
                        msg + "; unsetting them", silent=build_option("silent")
                    )
            ...
            if loaded_eb_modules:
                ...
                verbose_msg = "\n".join(
                    ...
                    + [
                        ...
                        "To specify action to take when loaded modules are detected, use %s."
                        % opt,
                        "",
                ...
                if action == ERROR:
                    ...
                    msg = (
                        "Unloading non-allowed loaded (EasyBuild-generated) modules: %s"
                    )
                    print_warning(
                        msg % ", ".join(loaded_eb_modules),
                        silent=build_option("silent"),
    ...
    def interpret_raw_path_lua(self, txt):
        ...
        if txt.startswith('"') and txt.endswith('"'):
            ...
            res = re.sub(
                r'os.getenv\("(?P<key>[^"]*)"\)',
                lambda res: '"%s"' % os.getenv(res.group("key"), ""),
                txt,
            )
    ...
    def interpret_raw_path_tcl(self, txt):
        ...
        res = re.sub(
            r"\$env\((?P<key>[^)]*)\)", lambda res: os.getenv(res.group("key"), ""), res
        )
    ...
    def path_to_top_of_module_tree(
        self, top_paths, mod_name, full_mod_subdir, deps, modpath_exts=None
    ):
        """
        ...
        * 3rd iteration: try to find module that extends $MODULEPATH with '<prefix>/Core'
        ...
        self.log.debug(
            "Checking for dependency that extends $MODULEPATH with %s" % full_mod_subdir
        )

        if modpath_exts is None:
            # only retain dependencies that have a non-empty lists of $MODULEPATH extensions
            modpath_exts = dict(
                [(k, v) for k, v in self.modpath_extensions_for(deps).items() if v]
            )
            self.log.debug(
        ...
        for dep in modpath_exts:
            ...
            if path_matches(full_mod_subdir, full_modpath_exts):
                ...
                dep_full_mod_subdir = self.modulefile_path(dep, strip_ext=True)[
                    : -len(dep) - 1
                ]
...
class EnvironmentModulesTcl(EnvironmentModulesC):
    ...
    def run_module(self, *args, **kwargs):
        ...
        def tweak_stdout(txt):
            """Tweak stdout before it's exec'ed as Python code."""
            modulescript_regex = (
                r"^exec\s+[\"'](?P<modulescript>/tmp/modulescript_[0-9_]+)[\"']$"
            )
...
class EnvironmentModules(EnvironmentModulesTcl):
    ...
    COMMAND = os.path.join(
        os.getenv("MODULESHOME", "MODULESHOME_NOT_DEFINED"), "libexec", "modulecmd.tcl"
    )
    REQ_VERSION = "4.0.0"
    MAX_VERSION = None
    ...
    def check_module_output(self, cmd, stdout, stderr):
        ...
        if "_mlstatus = False" in stdout:
            raise EasyBuildError(
                "Failed module command detected: %s (stdout: %s, stderr: %s)",
                cmd,
                stdout,
                stderr,
            )
...
class Lmod(ModulesTool):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.supports_depends_on = StrictVersion(self.version) >= StrictVersion(
            self.REQ_VERSION_DEPENDS_ON
        )
    ...
    def check_module_output(self, cmd, stdout, stderr):
        ...
        if stdout:
            ...
            raise EasyBuildError(
                "Found empty stdout, seems like '%s' failed: %s", cmd, stderr
            )
    ...
    def available(self, mod_name=None):
        ...
        correct_real_mods = [
            mod for mod in real_mods if mod_name is None or mod.startswith(mod_name)
        ]
    ...
    def update(self):
        ...
        if build_option("update_modules_tool_cache"):
            ...
            if stderr:
                raise EasyBuildError(
                    "An error occurred when running '%s': %s", " ".join(cmd), stderr
                )
            ...
            if self.testing:
                ...
                try:
                    ...
                    raise EasyBuildError(
                        "Failed to update Lmod spider cache %s: %s", cache_fp, err
                    )
    ...
    def module_wrapper_exists(self, mod_name):
        ...
        if StrictVersion(self.version) >= StrictVersion("7.8"):
            mod_wrapper_regex_template = (
                r'^module_version\("(?P<wrapped_mod>.*)", "%s"\)$'
            )
            res = super(Lmod, self).module_wrapper_exists(
                mod_name,
                modulerc_fn=".modulerc.lua",
    ...
    def exist(self, mod_names, skip_avail=False, maybe_partial=True):
        ...
        return super(Lmod, self).exist(
            mod_names,
            mod_exists_regex_template=r"^\s*\S*/%s.*(\.lua)?:\s*$",
            skip_avail=skip_avail,
            maybe_partial=maybe_partial,
        )
    ...
    if root:
        for lib_subdir in lib_subdirs:
            ...
            if os.path.exists(lib_dir_path):
                ...
                if any(
                    os.path.samefile(lib_dir_path, os.path.join(root, x)) for x in res
                ):
                    _log.debug(
                        "%s is the same as one of the other paths, so skipping it",
                        lib_dir_path,
                    )

                elif fs is None or any(
                    os.path.exists(os.path.join(lib_dir_path, f)) for f in fs
                ):
                    _log.debug(
                        "Retaining library subdir '%s' (found at %s)",
                        lib_subdir,
                        lib_dir_path,
                    )
...
def curr_module_paths():
    ...
    return [
        p
        for p in os.environ.get("MODULEPATH", "").split(":")
...
def invalidate_module_caches_for(path):
    ...
    if not os.path.exists(path):
        raise EasyBuildError(
            "Non-existing path specified to invalidate module caches: %s", path
        )
    ...
    for cache, subcmd in [(MODULE_AVAIL_CACHE, "avail"), (MODULE_SHOW_CACHE, "show")]:
        for key in list(cache.keys()):
            ...
            for path_in_key in paths_in_key:
                if path == path_in_key or (
                    os.path.exists(path_in_key) and os.path.samefile(path, path_in_key)
                ):
                    _log.debug(
                        "Entry '%s' in 'module %s' cache is evicted, marked as invalid via path '%s': %s",
                        key,
                        subcmd,
                        path,
                        cache[key],
...
class Modules(EnvironmentModulesC):
    ...
    def __init__(self, *args, **kwargs):
        _log.nosupport(
            "modules.Modules class is now an abstract interface, use modules.modules_tool instead",
            "2.0",
        )
