...
SINGULARITY_BOOTSTRAP_AGENTS = sorted(
    SINGULARITY_BOOTSTRAP_AGENTS_DISTRO + SINGULARITY_BOOTSTRAP_AGENTS_IMAGE
)
...
class SingularityContainer(ContainerGenerator):
    ...
    def singularity_version():
        ...
        if ec:
            raise EasyBuildError(
                "Error running '%s': %s for tool {1} with output: {2}"
                % (version_cmd, out)
            )
    ...
    def resolve_template_data_config(self):
        ...
        if bootstrap:
            if bootstrap not in SINGULARITY_BOOTSTRAP_AGENTS:
                raise EasyBuildError(
                    "Unknown value specified for 'bootstrap' keyword: %s (known: %s)",
                    bootstrap,
                    ", ".join(SINGULARITY_BOOTSTRAP_AGENTS),
                )
        else:
            raise EasyBuildError(
                "Keyword 'bootstrap' is required in container base config"
            )

        # make sure 'from' is specified when required
        if (
            bootstrap in SINGULARITY_BOOTSTRAP_AGENTS_IMAGE
            and template_data.get("from") is None
        ):
            raise EasyBuildError(
                "Keyword 'from' is required in container base config when using bootstrap agent '%s'",
                bootstrap,
            )

        # use default value for mirror URI if none was specified
        if (
            bootstrap in SINGULARITY_MIRRORURL_DEFAULTS
            and template_data.get("mirrorurl") is None
        ):
        ...
        if (
            mirrorurl
            and "%{OSVERSION}" in mirrorurl
            and template_data.get("osversion") is None
        ):
            raise EasyBuildError(
                "Keyword 'osversion' is required in container base config when '%%{OSVERSION}' "
                "is used in mirror URI: %s",
                mirrorurl,
            )

        # use default value for list of included OS packages if nothing else was specified
        if (
            bootstrap in SINGULARITY_INCLUDE_DEFAULTS
            and template_data.get("include") is None
        ):
    ...
    def resolve_template_data(self):
        ...
        for key in ["From", "OSVersion", "MirrorURL", "Include"]:
            if key.lower() in template_data:
                bootstrap_config_lines.append(
                    "%s: %s" % (key, template_data[key.lower()])
                )
        ...
        for osdep in osdeps:
            ...
            # tuple entry indicates multiple options
                ...
                install_os_deps.append(
                    "yum --skip-broken --quiet --assumeyes install %s" % " ".join(osdep)
                )
            else:
                raise EasyBuildError(
                    "Unknown format of OS dependency specification encountered: %s",
                    osdep,
                )
        ...
        template_data["easyconfigs"] = " ".join(
            os.path.basename(e["spec"]) for e in self.easyconfigs
        )
    ...
    def build_image(self, recipe_path):
        ...
        cmd = " ".join(
            ["sudo", cmd_env, singularity, "build", cmd_opts, img_path, recipe_path]
        )
