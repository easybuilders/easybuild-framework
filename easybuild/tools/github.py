...
class Githubfs(object):
    ...
    def __init__(
        ...
        self.gh = RestClient(
            GITHUB_API_URL, username=username, password=password, token=token
        )
    ...
    def read(self, path, api=True):
        ...
        if not api:
            ...
            url = "/".join(
                [GITHUB_RAW, self.githubuser, self.reponame, self.branchname, path]
            )
...
def fetch_latest_commit_sha(
    repo, account, branch="master", github_user=None, token=None
):
    """
    ...
    :return: latest SHA1
    """
    status, data = github_api_get_request(
        lambda x: x.repos[account][repo].branches,
    ...
    if res is None:
        error_msg = "No branch with name %s found in repo %s/%s" % (
            branch,
            account,
            repo,
        )
        if len(data) >= GITHUB_MAX_PER_PAGE:
            error_msg += (
                "; only %d branches were checked (too many branches in %s/%s?)"
                % (len(data), account, repo)
            )
...
def download_repo(
    ...
    latest_commit_sha = fetch_latest_commit_sha(
        repo, account, branch, github_user=github_user
    )
    ...
    if os.path.exists(latest_sha_path):
        ...
        if latest_commit_sha == sha:
            _log.debug(
                "Not redownloading %s/%s as it already exists: %s"
    ...
    _log.debug(
        "downloading repo %s/%s as archive from %s to %s"
    ...
    extracted_path = os.path.join(
        extract_file(target_path, path, forced=True), extracted_dir_name
    )
    ...
    _log.debug(
        "Repo %s at branch %s extracted into %s" % (repo, branch, extracted_path)
    )
...
def fetch_easyconfigs_from_pr(pr, path=None, github_user=None):
    ...
    _log.debug(
        "Fetching easyconfigs from %s/%s PR #%s into %s",
        github_account,
        github_repo,
        pr,
        path,
    ...
    repo_target_branch = download_repo(
        repo=github_repo,
    ...
    patched_files = det_patched_files(
        txt=diff_txt, omit_ab_prefix=True, github=True, filter_deleted=True
    )
    ...
    if pr_merged:
        ...
        try:
            _log.debug(
                "Trying to apply PR patch %s to %s...",
                diff_filepath,
                repo_target_branch,
            )
            apply_patch(diff_filepath, repo_target_branch, use_git_am=True)
            _log.info(
                "Using %s which included PR patch to test PR #%s",
                repo_target_branch,
                pr,
            )
            final_path = repo_target_branch
            ...
            _log.warning(
                "Ignoring problem that occured when applying PR patch: %s", err
            )
    ...
    if final_path is None:
        ...
        for patched_file in patched_files:
            ...
            full_url = URL_SEPARATOR.join(
                [GITHUB_RAW, github_account, github_repo, sha, patched_file]
            )
    ...
    if final_path != path:
        ...
        for eb_dir in os.listdir(dirpath):
            symlink(
                os.path.join(dirpath, eb_dir),
                os.path.join(path, os.path.basename(eb_dir)),
            )
...
def create_gist(txt, fn, descr=None, github_user=None, github_token=None):
    ...
    body = {"description": descr, "public": True, "files": {fn: {"content": txt,}}}
...
def delete_gist(gist_id, github_user=None, github_token=None):
    ...
    if status != HTTP_STATUS_NO_CONTENT:
        raise EasyBuildError(
            "Failed to delete gist with ID %s: status %s, data: %s", status, data
        )


def post_comment_in_issue(
    issue, txt, account=GITHUB_EB_MAIN, repo=GITHUB_EASYCONFIGS_REPO, github_user=None
):
    ...
    if not dry_run:
        ...
        if not status == HTTP_STATUS_CREATED:
            raise EasyBuildError(
                "Failed to create comment in PR %s#%d; status %s, data: %s",
                repo,
                issue,
                status,
                data,
...
def setup_repo_from(git_repo, github_url, target_account, branch_name, silent=False):
    ...
    print_msg(
        "fetching branch '%s' from %s..." % (branch_name, github_url), silent=silent
    )
    try:
        res = origin.fetch()
    except GitCommandError as err:
    ...
    if res:
        if res[0].flags & res[0].ERROR:
            raise EasyBuildError(
                "Fetching branch '%s' from remote %s failed: %s",
                branch_name,
                origin,
                res[0].note,
            )
            ...
            _log.debug(
                "Fetched branch '%s' from remote %s (note: %s)",
                branch_name,
                origin,
                res[0].note,
            )
        ...
        raise EasyBuildError(
            "Fetching branch '%s' from remote %s failed: empty result",
            branch_name,
            origin,
        )
    ...
    try:
        ...
        _log.debug(
            "Trying to work around checkout error ('%s') by using different branch name '%s'",
            err,
            alt_branch,
        )
        try:
            ...
            raise EasyBuildError(
                "Failed to check out branch '%s' from repo at %s: %s",
                alt_branch,
                github_url,
                err,
            )
...
def setup_repo(
    git_repo, target_account, target_repo, branch_name, silent=False, git_only=False
):
    """
    ...
    for tmpl_github_url in tmpl_github_urls:
        ...
        try:
            res = setup_repo_from(
                git_repo, github_url, target_account, branch_name, silent=silent
            )
            break

        except EasyBuildError as err:
            errors.append(
                "Checking out branch '%s' from %s failed: %s"
                % (branch_name, github_url, err)
            )
...
def _easyconfigs_pr_common(
    paths, ecs, start_branch=None, pr_branch=None, start_account=None, commit_msg=None
):
    """
    ...
    * push PR branch to GitHub (to account specified by --github-user)
    ...
    if commit_msg:
        ...
        _log.debug(
            "Using specified commit message for all %d new/modified easyconfigs at once: %s",
            cnt,
            commit_msg,
        )
        ...
        commit_msg = "adding easyconfigs: %s" % ", ".join(
            os.path.basename(p) for p in file_info["paths_in_repo"]
        )
        if paths["patch_files"]:
            commit_msg += " and patches: %s" % ", ".join(
                os.path.basename(p) for p in paths["patch_files"]
            )
    else:
        raise EasyBuildError(
    ...
    for fn in paths["files_to_delete"]:
        ...
        if os.path.exists(fullpath):
            ...
            hits = glob.glob(
                os.path.join(repo_path, "easybuild", "easyconfigs", "*", "*", fn)
            )
            if len(hits) == 1:
                deleted_paths.append(hits[0])
            else:
                raise EasyBuildError(
                    "Path doesn't exist or file to delete isn't found in target branch: %s",
                    fn,
                )
    ...
    if ecs is not None:
        ...
        dep_paths = [
            ec["spec"] for ec in ecs if os.path.realpath(ec["spec"]) not in abs_paths
        ]
    ...
    if pr_branch is None:
        if ec_paths:
            label = file_info["ecs"][0].name + re.sub(
                "[.-]", "", file_info["ecs"][0].version
            )
        else:
    ...
    # stage
    _log.debug(
        "Staging all %d new/modified easyconfigs", len(file_info["paths_in_repo"])
    )
    ...
    if paths["patch_files"]:
        _log.debug(
            "Staging all %d new/modified patch files", len(patch_info["paths_in_repo"])
        )
    ...
    if not diff_stat:
        raise EasyBuildError(
            "No changed files found when comparing to current develop branch. "
...
def create_remote(git_repo, account, repo, https=False):
    ...
    try:
        ...
        raise EasyBuildError(
            "Failed to create remote %s for %s: %s", remote_name, github_url, err
        )
...
def push_branch_to_github(git_repo, target_account, target_repo, branch):
    ...
    push_branch_msg = "pushing branch '%s' to remote '%s' (%s)" % (
        branch,
        remote.name,
        github_url,
    )
    if dry_run:
        ...
        if res:
            if res[0].ERROR & res[0].flags:
                ...
                _log.debug(
                    "Pushed branch %s to remote %s (%s): %s",
                    branch,
                    remote,
                    github_url,
                    res[0].summary,
    ...
    for patch_path in patch_paths:
        ...
        if soft_name:
            ...
            if soft_name:
                ...
                # still nothing found
                raise EasyBuildError(
                    "Failed to determine software name to which patch file %s relates",
                    patch_path,
                )
...
def check_pr_eligible_to_merge(pr_data):
    ...
    target = "%s/%s" % (
        pr_data["base"]["repo"]["owner"]["login"],
        pr_data["base"]["repo"]["name"],
    )
    print_msg(
        "Checking eligibility of %s PR #%s for merging..."
        ...
        for comment in pr_data["issue_comments"][::-1]:
            ...
            if test_report_regex.search(comment):
                if "SUCCESS" in comment:
                    ...
                    print_warning(
                        "Failed to determine outcome of test report for comment:\n%s"
                        % comment
                    )
...
def reasons_for_closing(pr_data):
    ...
    if pr_data["status_last_commit"]:
        print_msg(
            "Status of last commit is %s\n" % pr_data["status_last_commit"].upper(),
            prefix=False,
        )
    ...
    if pr_data["issue_comments"]:
        ...
        print_msg(
            "Last comment on %s, by %s, was:\n\n%s"
            % (timestamp, username, last_comment["body"]),
            prefix=False,
        )
    ...
    if pr_data["reviews"]:
        ...
        print_msg(
            "Last reviewed on %s by %s, state %s\n\n%s"
            % (timestamp, username, state, body),
            prefix=False,
        )
    ...
    print_msg(
        "No activity since %s" % pr_data["updated_at"].replace("T", " at ")[:-1],
        prefix=False,
    )
    ...
    pr_files = [
        path
        for path in fetch_easyconfigs_from_pr(pr_data["number"])
        if path.endswith(".eb")
    ]
    ...
    for pr_file in pr_files:
        ...
        for robot_path in robot_paths:
                    ...
                    if fn.endswith(".eb"):
                        ...
                        if ec.get("easyblock") == "Toolchain":
                            ...
                            if pr_tc == archived_tc:
                                print_msg(
                                    " - uses archived toolchain %s" % pr_tc,
                                    prefix=False,
                                )
            ...
            for (dirpath, _, filenames) in os.walk(
                os.path.join(robot_path, pr_ec["name"].lower()[0], pr_ec["name"])
            ):
                for fn in filenames:
                    if fn.endswith(".eb"):
                        ec = EasyConfigParser(
            ...
            if newer_versions:
                print_msg(
                    " - found newer versions %s" % ", ".join(sorted(newer_versions)),
                    prefix=False,
                )
...
def close_pr(pr, motivation_msg=None):
    ...
    pr_data, _ = fetch_pr_data(
        pr, pr_target_account, pr_target_repo, github_user, full=True
    )

    if pr_data["state"] == GITHUB_STATE_CLOSED:
        raise EasyBuildError(
            "PR #%d from %s/%s is already closed.",
            pr,
            pr_target_account,
            pr_target_repo,
        )
    ...
    if not motivation_msg:
        ...
        if not possible_reasons:
            raise EasyBuildError(
                "No reason specified and none found from PR data, "
                "please use --close-pr-reasons or --close-pr-msg"
            )
        else:
            motivation_msg = ", ".join(
                [VALID_CLOSE_PR_REASONS[reason] for reason in possible_reasons]
            )
            print_msg(
    ...
    post_comment_in_issue(
        pr, msg, account=pr_target_account, repo=pr_target_repo, github_user=github_user
    )

    if dry_run:
        print_msg(
            "[DRY RUN] Closed %s/%s PR #%s" % (pr_target_account, pr_target_repo, pr),
            prefix=False,
        )
        if reopen:
            print_msg(
                "[DRY RUN] Reopened %s/%s PR #%s"
                % (pr_target_account, pr_target_repo, pr),
                prefix=False,
            )
    else:
        github_token = fetch_github_token(github_user)
        if github_token is None:
            raise EasyBuildError(
                "GitHub token for user '%s' must be available to use --close-pr",
                github_user,
            )
        g = RestClient(GITHUB_API_URL, username=github_user, token=github_token)
        ...
        if not status == HTTP_STATUS_OK:
            raise EasyBuildError(
                "Failed to close PR #%s; status %s, data: %s", pr, status, data
            )
        if reopen:
            body = {"state": "open"}
            status, data = pull_url.post(body=body)
...
def list_prs(params, per_page=GITHUB_MAX_PER_PAGE, github_user=None):
    ...
    print_msg(
        "Listing PRs with parameters: %s"
        % ", ".join(k + "=" + str(parameters[k]) for k in sorted(parameters))
    )
    ...
    pr_data, _ = fetch_pr_data(
        None, pr_target_account, pr_target_repo, github_user, **parameters
    )
...
def merge_pr(pr):
    ...
    if check_pr_eligible_to_merge(pr_data) or force:
        ...
        if dry_run:
            print_msg(
                "[DRY RUN] Merged %s/%s pull request #%s"
                % (pr_target_account, pr_target_repo, pr),
                prefix=False,
            )
        else:
        ...
        print_warning(
            "Review indicates this PR should not be merged (use -f/--force to do so anyway)"
        )
...
def new_branch_github(paths, ecs, commit_msg=None):
    ...
    res = _easyconfigs_pr_common(
        paths, ecs, pr_branch=branch_name, commit_msg=commit_msg
    )
...
def new_pr_from_branch(branch_name, title=None, descr=None, pr_metadata=None):
    ...
    if github_token is None:
        raise EasyBuildError(
            "GitHub token for user '%s' must be available to open a pull request",
            github_user,
        )
    ...
    if pr_metadata:
        ...
        print_msg(
            "syncing '%s' with current '%s/develop' branch..."
            % (branch_name, pr_target_account),
            log=_log,
        )
        ...
        print_msg(
            "checking out target branch '%s/%s'..."
            % (pr_target_account, pr_target_branch),
            log=_log,
        )
        ...
        print_msg(
            "determining metadata for pull request based on changed files...", log=_log
        )
        ...
        if changed_files:
            ...
            msg = [
                "found %d changed file(s) in '%s' relative to '%s':"
                % (len(changed_files), from_branch, to_branch)
            ]
            if ec_paths:
            ...
            raise EasyBuildError(
                "No changes in '%s' branch compared to current 'develop' branch!",
                branch_name,
            )
        ...
        print_msg(
            "checking out PR branch '%s/%s'..." % (github_account, branch_name),
            log=_log,
        )
        ...
        git_repo.git.checkout(
            "remotes/%s/%s" % (remote.name, branch_name), track=True, force=True
        )
    ...
    toolchain_label = ",".join(
        [tc for (cnt, tc) in toolchains_counted if cnt == toolchains_counted[-1][0]]
    )
    ...
    if title is None:
        ...
        if file_info["ecs"] and all(file_info["new"]) and not deleted_paths:
            ...
            raise EasyBuildError(
                "Don't know how to make a PR title for this PR. "
    ...
    if not dry_run:
        ...
        if not status == HTTP_STATUS_CREATED:
            raise EasyBuildError(
                "Failed to open PR for branch %s; status %s, data: %s",
                branch_name,
                status,
                data,
            )
        ...
        if labels:
            ...
            try:
                ...
                if status == HTTP_STATUS_OK:
                    print_msg(
                        "Added labels %s to PR#%s" % (", ".join(labels), pr),
                        log=_log,
                        prefix=False,
                    )
...
def det_account_branch_for_pr(pr_id, github_user=None):
    ...
    print_msg(
        "Determined branch name corresponding to %s PR #%s: %s"
        % (github_target, pr_id, branch),
        log=_log,
    )
...
def update_branch(branch_name, paths, ecs, github_account=None, commit_msg=None):
    ...
    _, _, _, _, diff_stat = _easyconfigs_pr_common(
        paths,
        ecs,
        start_branch=branch_name,
        pr_branch=branch_name,
        start_account=github_account,
...
def update_pr(pr_id, paths, ecs, commit_msg=None):
    ...
    update_branch(
        branch_name, paths, ecs, github_account=github_account, commit_msg=commit_msg
    )

    full_repo = "%s/%s" % (
        build_option("pr_target_account"),
        build_option("pr_target_repo"),
    )
...
def check_github():
    ...
    * check whether location to local working directories for Git repositories is available (not strictly needed)
    ...
    try:
        ...
        print_msg("FAIL")
        raise EasyBuildError(
            "checking status of GitHub integration must be done online"
        )
    ...
    if github_user is None:
        ...
        status["--new-pr"] = status["--update-pr"] = status[
            "--upload-test-report"
        ] = False
    else:
    ...
    msg = "* push access to %s/%s repo @ GitHub..." % (
        github_account,
    ...
    try:
        ...
        _log.warning(
            "Exception when testing push access to %s/%s: %s",
            github_account,
            GITHUB_EASYCONFIGS_REPO,
            err,
        )
    ...
    if res:
        if res[0].flags & res[0].ERROR:
            _log.warning(
                "Error occurred when pushing test branch to GitHub: %s", res[0].summary
            )
        ...
        if "git" in sys.modules:
            ...
            if LooseVersion(ver) < LooseVersion(req_ver):
                check_res = (
                    "FAIL (GitPython version %s is too old, should be version %s or newer)"
                    % (ver, req_ver)
                )
            else:
    ...
    if git_repo:
        try:
            if (
                git_repo
                and hasattr(git_repo, "remotes")
                and hasattr(git_repo.remotes, "origin")
            ):
                git_repo.remotes.origin.push(branch_name, delete=True)
        except GitCommandError as err:
            sys.stderr.write(
                "WARNNIG: failed to delete test branch from GitHub: %s\n" % err
            )
    ...
    try:
        ...
        _log.info(
            "Gist with ID %s successfully created, now deleting it again...", gist_id
        )
    ...
    # check whether location to local working directories for Git repositories is available (not strictly needed)
    print_msg(
        "* location to Git working dirs... ", log=_log, prefix=False, newline=False
    )
    git_working_dirs_path = build_option("git_working_dirs_path")
    if git_working_dirs_path:
        ...
        if token is None:
            python_cmd = "; ".join(
                [
                    ...
                    'keyring.set_password("%s", "%s", getpass.getpass())'
                    % (KEYRING_GITHUB_TOKEN, user),
                ]
            )
            msg = "\n".join(
                [
...
def install_github_token(github_user, silent=False):
    ...
    if current_token:
        ...
        if build_option("force"):
            msg = "WARNING: overwriting installed token '%s' for user '%s'..." % (
                current_token,
                github_user,
            )
    ...
    if valid_token:
        print_msg(
            "Token seems to be valid, installing it.", prefix=False, silent=silent
        )
    else:
        raise EasyBuildError(
    ...
    print_msg(
        "Token '%s..%s' installed!" % (token[:3], token[-3:]),
        prefix=False,
        silent=silent,
    )
...
def validate_github_token(token, github_user):
    ...
    * see if it can be used for authenticated access
    ...
    if sanity_check:
        ...
        _log.warning(
            "Sanity check on token failed; token doesn't match pattern '%s'",
            sha_regex.pattern,
        )
    ...
    try:
        ...
        _log.warning(
            "An exception occurred when trying to use token for authenticated GitHub access: %s",
            err,
        )
    ...
    if token_test:
        _log.info(
            "GitHub token can be used for authenticated GitHub access, validation passed"
        )
...
def find_easybuild_easyconfig(github_user=None):
    ...
    """
    dev_repo = download_repo(
        GITHUB_EASYCONFIGS_REPO,
        branch="develop",
        account=GITHUB_EB_MAIN,
        github_user=github_user,
    )
    eb_parent_path = os.path.join(
        dev_repo, "easybuild", "easyconfigs", "e", "EasyBuild"
    )
...
def fetch_pr_data(
    pr, pr_target_account, pr_target_repo, github_user, full=False, **parameters
):
    ...
    if full:
        ...
        def status_url(gh):
            ...
            return (
                gh.repos[pr_target_account][pr_target_repo]
                .commits[pr_data["head"]["sha"]]
                .status
            )

        status, status_data = github_api_get_request(
            status_url, github_user, **parameters
        )
        if status != HTTP_STATUS_OK:
            raise EasyBuildError(
                "Failed to get status of last commit for PR #%d from %s/%s (status: %d %s)",
                pr,
                pr_target_account,
                pr_target_repo,
                status,
        ...
        status, comments_data = github_api_get_request(
            comments_url, github_user, **parameters
        )
        if status != HTTP_STATUS_OK:
            raise EasyBuildError(
        ...
        status, reviews_data = github_api_get_request(
            reviews_url, github_user, **parameters
        )
        if status != HTTP_STATUS_OK:
            raise EasyBuildError(
...
def sync_with_develop(git_repo, branch_name, github_account, github_repo):
    ...
    msg = "pulling latest version of '%s' branch from %s/%s..." % (
    ...
    _log.debug(
        "Output of 'git pull %s %s': %s", remote.name, GITHUB_DEVELOP_BRANCH, pull_out
    )
    ...
    _log.debug(
        "Top of 'git log' after 'git merge %s':\n%s",
        GITHUB_DEVELOP_BRANCH,
        post_merge_log,
    )
...
def sync_pr_with_develop(pr_id):
    ...
    if github_user is None:
        raise EasyBuildError(
            "GitHub user must be specified to use --sync-pr-with-develop"
        )
...
def sync_branch_with_develop(branch_name):
    ...
    if github_user is None:
        raise EasyBuildError(
            "GitHub user must be specified to use --sync-branch-with-develop"
        )
