...
def avail_cfgfile_constants(go_cfg_constants, output_format=FORMAT_TXT):
    ...
    """
    return generate_doc(
        "avail_cfgfile_constants_%s" % output_format, [go_cfg_constants]
    )
...
def avail_cfgfile_constants_txt(go_cfg_constants):
    ...
    for section in go_cfg_constants:
        ...
        for cst_name, (cst_value, cst_help) in sorted(
            go_cfg_constants[section].items()
        ):
...
def avail_easyconfig_licenses_txt():
    ...
    for lic_name, lic in sorted(EASYCONFIG_LICENSES_DICT.items()):
        ...
        doc.append(
            "%s%s: %s%s" % (INDENT_4SPACES, lic_inst.name, lic_inst.description, strver)
        )
...
def avail_easyconfig_params_txt(title, grouped_params):
    ...
    for grpname in grouped_params:
        ...
        for name, (descr, dflt) in sorted(grouped_params[grpname].items()):
            doc.append(
                "{0:<{nw}}   {1:} [default: {2:}]".format(
                    name, descr, str(quote_str(dflt)), nw=nw
                )
            )
        doc.append("")
...
def avail_easyconfig_params(easyblock, output_format=FORMAT_TXT):
    ...
    return generate_doc(
        "avail_easyconfig_params_%s" % output_format, [title, grouped_params]
    )
...
def avail_easyconfig_templates_txt():
    ...
    for name, pref in TEMPLATE_SOFTWARE_VERSIONS:
        doc.append(
            "%s%%(%sshortver)s: short version for %s (<major>.<minor>)"
            % (INDENT_4SPACES, pref, name)
        )
    ...
    for name in TEMPLATE_NAMES_LOWER:
        ...
        doc.append(
            "%s%%(%s)s: lower case of value of %s"
            % (INDENT_4SPACES, template_name, name)
        )
...
def avail_easyconfig_templates_rst():
    ...
    table_values = [
        [
            "``%%(%s)s``" % (TEMPLATE_NAMES_LOWER_TEMPLATE % {"name": name})
            for name in TEMPLATE_NAMES_LOWER
        ],
...
def avail_classes_tree(
    classes, class_names, locations, detailed, format_strings, depth=0
):
    ...
    for class_name in class_names:
        ...
        if detailed:
            ...
            txt.append(
                format_strings["zero_indent"]
                + format_strings["indent"] * depth
                + format_strings["sep"]
                + "%s (%s %s)" % (class_name, mod, loc)
            )
        else:
            txt.append(
                format_strings["zero_indent"]
        ...
        classes.update(
            {
                cls.__name__: {
                    ...
                    "children": sorted(
                        [c.__name__ for c in children], key=lambda x: x.lower()
                    ),
                }
            }
        )
...
def list_software(output_format=FORMAT_TXT, detailed=False, only_installed=False):
    ...
    for idx, ec_path in enumerate(ec_paths):
        ...
        print_msg(
            "Processed %d/%d easyconfigs..." % (idx + 1, cnt),
            newline=False,
            silent=silent,
        )
    ...
    if only_installed:
        ...
        print_msg(
            "Retained %d installed software packages" % len(software), silent=silent
        )
    ...
    for key in sorted_keys:
        ...
        if detailed:
            ...
            for _, vsuff, ver in sorted_pairs:
                ...
                tcs = [
                    x["toolchain"]
                    for x in software[key]
                    if x["version"] == ver and x["versionsuffix"] == vsuff
                ]
                table_values[-1].append(
                    ", ".join("``%s``" % tc for tc in sorted(nub(tcs)))
                )
...
def avail_toolchain_opts(name, output_format=FORMAT_TXT):
    ...
    if not tc_class:
        raise EasyBuildError(
            "Couldn't find toolchain: '%s'. To see available toolchains, use --list-toolchains"
            % name
        )
...
def avail_toolchain_opts_txt(name, tc_dict):
    ...
    for opt_name in sorted(tc_dict.keys()):
        doc.append(
            "%s%s: %s (default: %s)"
            % (INDENT_4SPACES, opt_name, tc_dict[opt_name][1], tc_dict[opt_name][0])
        )

    return "\n".join(doc)


def gen_easyblocks_overview_rst(
    package_name, path_to_examples, common_params={}, doc_functions=[]
):
    """
    ...
    for mod in modules:
        for name, obj in inspect.getmembers(mod, inspect.isclass):
            ...
            if (
                eb_class.__module__.startswith(package_name)
                and eb_class not in all_blocks
            ):
    ...
    for eb_class in sorted(all_blocks, key=lambda c: c.__name__):
        doc.extend(
            gen_easyblock_doc_section_rst(
                eb_class, path_to_examples, common_params, doc_functions, all_blocks
            )
        )
...
def gen_easyblock_doc_section_rst(
    eb_class, path_to_examples, common_params, doc_functions, all_blocks
):
    """
