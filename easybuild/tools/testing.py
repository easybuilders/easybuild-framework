...
def regtest(easyconfig_paths, modtool, build_specs=None):
    ...
    if aggregate_regtest is not None:
        output_file = os.path.join(
            aggregate_regtest, "%s-aggregate.xml" % os.path.basename(aggregate_regtest)
        )
        aggregate_xml_in_dirs(aggregate_regtest, output_file)
        _log.info(
            "aggregated xml files inside %s, output written to: %s"
    ...
    for ecfile in ecfiles:
        try:
            ...
            test_results.append(
                (ecfile, "parsing_easyconfigs", "easyconfig file error: %s" % err, _log)
            )
...
def create_test_report(
    msg, ecs_with_res, init_session_state, pr_nr=None, gist_log=False
):
    ...
    if pr_nr is not None:
        test_report.extend(
            [
                "Test report for https://github.com/easybuilders/easybuild-easyconfigs/pull/%s"
                % pr_nr,
                "",
            ]
        )
        ...
        if ec_res.get("success", False):
            ...
            if gist_log and "log_file" in ec_res:
                ...
                gist_url = create_gist(
                    partial_log_txt, fn, descr=descr, github_user=user
                )
                test_log = "(partial log available at %s)" % gist_url

        build_overview.append(
            " * **%s** _%s_ %s" % (test_result, os.path.basename(ec["spec"]), test_log)
        )
    test_report.extend(
        ["#### Overview of tested easyconfigs (in order)"] + build_overview + [""]
    )
    ...
    test_report.extend(
        ["#### Time info", " * start: %s" % start_time, " * end: %s" % end_time, ""]
    )
...
def post_easyconfigs_pr_test_report(
    pr_nr, test_report, msg, init_session_state, success
):
    ...
    fn = "easybuild_test_report_easyconfigs_pr%s_%s.md" % (
        pr_nr,
        strftime("%Y%M%d-UTC-%H-%M-%S", gmtime()),
    )
    ...
    system_info = init_session_state["system_info"]
    short_system_info = (
        "%(hostname)s - %(os_type)s %(os_name)s %(os_version)s, %(cpu_model)s, Python %(pyver)s"
        % {
    ...
    msg = (
        "Test report uploaded to %s and mentioned in a comment in easyconfigs PR#%s"
        % (gist_url, pr_nr)
    )
    return msg
...
def overall_test_report(ecs_with_res, orig_cnt, success, msg, init_session_state):
    ...
    if upload:
        ...
        test_report = create_test_report(
            msg, ecs_with_res, init_session_state, pr_nr=pr_nr, gist_log=True
        )
        if pr_nr:
            # upload test report to gist and issue a comment in the PR to notify
            txt = post_easyconfigs_pr_test_report(
                pr_nr, test_report, msg, init_session_state, success
            )
        else:
