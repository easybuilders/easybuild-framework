...
from easybuild.base import (
    fancylogger,
)  # build_log should always stay there, to ensure EasyBuildLog
...
from easybuild.tools.config import (
    ...
    DEFAULT_PREFIX,
    DEFAULT_REPOSITORY,
    EBROOT_ENV_VAR_ACTIONS,
    ERROR,
)
from easybuild.tools.config import (
    FORCE_DOWNLOAD_CHOICES,
    GENERAL_CLASS,
    IGNORE,
...
from easybuild.tools.github import (
    GITHUB_PR_DIRECTION_DESC,
    GITHUB_PR_ORDER_CREATED,
    GITHUB_PR_STATE_OPEN,
)
from easybuild.tools.github import (
    GITHUB_PR_STATES,
    GITHUB_PR_ORDERS,
    GITHUB_PR_DIRECTIONS,
)
...
from easybuild.tools.include import (
    include_easyblocks,
...
from easybuild.tools.toolchain.compiler import (
    DEFAULT_OPT_LEVEL,
    OPTARCH_MAP_CHAR,
    OPTARCH_SEP,
    Compiler,
...
from easybuild.tools.systemtools import (
...
DEFAULT_SYS_CFGFILES = [
    f
    for d in XDG_CONFIG_DIRS
    for f in sorted(glob.glob(os.path.join(d, "easybuild.d", "*.cfg")))
]
...
def cleanup_and_exit(tmpdir):
    ...
    try:
        ...
        raise EasyBuildError(
            "Failed to clean up temporary directory %s: %s", tmpdir, err
        )
...
def pretty_print_opts(opts_dict):
    ...
    for opt in sorted(opts_dict):
        opt_val, loc = opts_dict[opt]
        lines.append(
            "{0:<{nwopt}} ({1:}) = {2:}".format(opt, loc, opt_val, nwopt=nwopt)
        )
...
def use_color(colorize, stream=sys.stdout):
    ...
    if colorize == fancylogger.Colorize.AUTO:
        ...
        assert (
            colorize == fancylogger.Colorize.NEVER
...
class EasyBuildOptions(GeneralOption):
    ...
    CONFIGFILES_RAISE_MISSING = (
        True  # don't allow non-existing config files to be specified
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.default_robot_paths = (
            get_paths_for(subdir=EASYCONFIGS_PKG_SUBDIR, robot_path=None) or []
        )
    ...
    def basic_options(self):
        ...
        opts = OrderedDict(
            {
                "dry-run": (
                    "Print build overview incl. dependencies (full paths)",
                    None,
                    "store_true",
                    False,
                ),
                ...
                "extended-dry-run": (
                    "Print build environment and (expected) build procedure that will be performed",
                    None,
                    "store_true",
                    False,
                    "x",
                ...
                "extended-dry-run-ignore-errors": (
                    "Ignore errors that occur during dry run",
                    None,
                    "store_true",
                    True,
                ),
                "force": (
                    ...
                    "and skipping check for OS dependencies",
                    None,
                    "store_true",
                    False,
                    "f",
                ...
                "missing-modules": (
                    "Print list of missing modules for dependencies of specified easyconfigs",
                    None,
                    "store_true",
                    False,
                    "M",
                ...
                "rebuild": (
                    "Rebuild software, even if module already exists (don't skip OS dependencies checks)",
                    None,
                    "store_true",
                    False,
                ),
                ...
                "search-paths": (
                    "Additional locations to consider in --search (next to --robot and --robot-paths paths)",
                    "pathlist",
                    "store_or_None",
                    [],
                    {"metavar": "PATH[%sPATH]" % os.pathsep},
    ...
    def software_options(self):
        ...
        opts["map-toolchains"] = (
            "Enable mapping of (sub)toolchains when --try-toolchain(-version) is used",
            None,
            "store_true",
            True,
        )
        ...
        opts = OrderedDict(
            {
                "add-dummy-to-minimal-toolchains": (
                    "Include dummy toolchain in minimal toolchain searches "
                ...
                "add-system-to-minimal-toolchains": (
                    "Include system toolchain in minimal toolchain searches",
                    None,
                    "store_true",
                    False,
                ),
                "allow-loaded-modules": (
                    "List of software names for which to allow loaded modules in initial environment",
                    "strlist",
                    "store",
                    DEFAULT_ALLOW_LOADED_MODULES,
                ),
                ...
                "allow-use-as-root-and-accept-consequences": (
                    "Allow using of EasyBuild as root (NOT RECOMMENDED!)",
                    None,
                    "store_true",
                    False,
                ),
                "backup-modules": (
                    ...
                    "store_true",
                    None,
                ),  # default None to allow auto-enabling if not disabled
                "check-ebroot-env-vars": (
                    "Action to take when defined $EBROOT* environment variables are found "
                    "for which there is no matching loaded module; "
                    "supported values: %s" % ", ".join(EBROOT_ENV_VAR_ACTIONS),
                    None,
                    "store",
                    WARN,
                ),
                "cleanup-builddir": (
                    "Cleanup build dir after successful installation.",
                    None,
                    "store_true",
                    True,
                ),
                "cleanup-tmpdir": (
                    "Cleanup tmp dir after successful run.",
                    None,
                    "store_true",
                    True,
                ),
                "color": (
                    ...
                    fancylogger.Colorize,
                    {"metavar": "WHEN"},
                ),
                "consider-archived-easyconfigs": (
                    "Also consider archived easyconfigs",
                    None,
                    "store_true",
                    False,
                ),
                "containerize": (
                    "Generate container recipe/image",
                    None,
                    "store_true",
                    False,
                    "C",
                ...
                "copy-ec": (
                    "Copy specified easyconfig(s) to specified location",
                    None,
                    "store_true",
                    False,
                ),
                ...
                "debug-lmod": (
                    "Run Lmod modules tool commands in debug module",
                    None,
                    "store_true",
                    False,
                ),
                "default-opt-level": (
                    "Specify default optimisation level",
                    "choice",
                    "store",
                    DEFAULT_OPT_LEVEL,
                ...
                "detect-loaded-modules": (
                    "Detect loaded EasyBuild-generated modules, act accordingly; "
                    "supported values: %s" % ", ".join(LOADED_MODULES_ACTIONS),
                    None,
                    "store",
                    WARN,
                ),
                "devel": (
                    "Enable including of development log messages",
                    None,
                    "store_true",
                    False,
                ),
                ...
                "dump-autopep8": (
                    "Reformat easyconfigs using autopep8 when dumping them",
                    None,
                    "store_true",
                    False,
                ),
                ...
                "enforce-checksums": (
                    "Enforce availability of checksums for all sources/patches, so they can be verified",
                    None,
                    "store_true",
                    False,
                ),
                ...
                "fetch": (
                    "Allow downloading sources ignoring OS and modules tool dependencies, "
                    "implies --stop=fetch, --ignore-osdeps and ignore modules tool",
                    None,
                    "store_true",
                    False,
                ),
                ...
                "filter-env-vars": (
                    "List of names of environment variables that should *not* be defined/updated by "
                    "module files generated by EasyBuild",
                    "strlist",
                    "extend",
                    None,
                ),
                "fixed-installdir-naming-scheme": (
                    "Use fixed naming scheme for installation directories",
                    None,
                    "store_true",
                    True,
                ),
                "force-download": (
                    "Force re-downloading of sources and/or patches, "
                ...
                "group-writable-installdir": (
                    "Enable group write permissions on installation directory after installation",
                    None,
                    "store_true",
                    False,
                ),
                "hidden": (
                    "Install 'hidden' module file(s) by prefixing their version with '.'",
                    None,
                    "store_true",
                    False,
                ),
                ...
                "hide-toolchains": (
                    ...
                    "(e.g. --hide-toolchains=GCCcore)",
                    "strlist",
                    "extend",
                    None,
                ),
                "ignore-checksums": (
                    "Ignore failing checksum verification",
                    None,
                    "store_true",
                    False,
                ),
                "ignore-osdeps": (
                    "Ignore any listed OS dependencies",
                    None,
                    "store_true",
                    False,
                ),
                "install-latest-eb-release": (
                    "Install latest known version of easybuild",
                    None,
                    "store_true",
                    False,
                ),
                "lib64-fallback-sanity-check": (
                    "Fallback in sanity check to lib64/ equivalent for missing libraries",
                    None,
                    "store_true",
                    True,
                ),
                "max-fail-ratio-adjust-permissions": (
                    "Maximum ratio for failures to allow when adjusting permissions",
                    "float",
                    "store",
                    DEFAULT_MAX_FAIL_RATIO_PERMS,
                ),
                "minimal-toolchains": (
                    "Use minimal toolchain when resolving dependencies",
                    None,
                    "store_true",
                    False,
                ),
                "module-only": (
                    ...
                    % ", ".join(MODULE_ONLY_STEPS),
                    None,
                    "store_true",
                    False,
                ),
                "modules-tool-version-check": (
                    "Check version of modules tool being used",
                    None,
                    "store_true",
                    True,
                ),
                ...
                "pre-create-installdir": (
                    "Create installation directory before submitting build jobs",
                    None,
                    "store_true",
                    True,
                ),
                ...
                "remove-ghost-install-dirs": (
                    "Remove ghost installation directories when --force or --rebuild is used, "
                ...
                "rpath": (
                    "Enable use of RPATH for linking with libraries",
                    None,
                    "store_true",
                    False,
                ),
                "rpath-filter": (
                    "List of regex patterns to use for filtering out RPATH paths",
                    "strlist",
                    "store",
                    None,
                ),
                "set-default-module": (
                    "Set the generated module as default",
                    None,
                    "store_true",
                    False,
                ),
                "set-gid-bit": (
                    "Set group ID bit on newly created directories",
                    None,
                    "store_true",
                    False,
                ),
                "silence-deprecation-warnings": (
                    "Silence specified deprecation warnings",
                    "strlist",
                    "extend",
                    None,
                ),
                "sticky-bit": (
                    "Set sticky bit on newly created directories",
                    None,
                    "store_true",
                    False,
                ),
                ...
                "trace": (
                    "Provide more information in output to stdout on progress",
                    None,
                    "store_true",
                    False,
                    "T",
                ...
                "use-ccache": (
                    "Enable use of ccache to speed up compilation, with specified cache dir",
                    str,
                    "store",
                    False,
                    {"metavar": "PATH"},
        ...
        opts = OrderedDict(
            {
                ...
                "containerpath": (
                    "Location where container recipe & image will be stored",
                    None,
                    "store",
                    mk_full_default_path("containerpath"),
                ),
                "external-modules-metadata": (
                    "List of (glob patterns for) paths to files specifying metadata "
                    "for external modules (INI format)",
                    "strlist",
                    "store",
                    None,
                ),
                "hooks": (
                    "Location of Python module with hook implementations",
                    "str",
                    "store",
                    None,
                ),
                ...
                "include-easyblocks": (
                    "Location(s) of extra or customized easyblocks",
                    "strlist",
                    "store",
                    [],
                ),
                "include-module-naming-schemes": (
                    "Location(s) of extra or customized module naming schemes",
                    "strlist",
                    "store",
                    [],
                ),
                ...
                "job-backend": (
                    "Backend to use for submitting jobs",
                    "choice",
                    "store",
                    DEFAULT_JOB_BACKEND,
                    sorted(avail_job_backends().keys()),
                ),
                ...
                "module-extensions": (
                    "Include 'extensions' statement in generated module file (Lua syntax only)",
                    None,
                    "store_true",
                    False,
                ),
                ...
                "packagepath": (
                    "The destination path for the packages built by package-tool",
                    None,
                    "store",
                    mk_full_default_path("packagepath"),
                ),
                "package-naming-scheme": (
                    "Packaging naming scheme choice",
                    "choice",
                    "store",
                    DEFAULT_PNS,
                    sorted(avail_package_naming_schemes().keys()),
                ),
                "prefix": (
                    (
                ...
                "suffix-modules-path": (
                    "Suffix for module files install path",
                    None,
                    "store",
                    GENERAL_CLASS,
                ),
        ...
        opts = OrderedDict(
            {
                ...
                "show-config": (
                    "Show current EasyBuild configuration (only non-default + selected settings)",
                    None,
                    "store_true",
                    False,
                ),
                ...
                "show-ec": (
                    "Show contents of specified easyconfig(s)",
                    None,
                    "store_true",
                    False,
                ),
                "show-full-config": (
                    "Show current EasyBuild configuration (all settings)",
                    None,
                    "store_true",
                    False,
                ),
                "show-system-info": (
                    "Show system information relevant to EasyBuild",
                    None,
                    "store_true",
                    False,
                ),
        ...
        opts = OrderedDict(
            {
                ...
                "check-contrib": (
                    "Runs checks to see whether the given easyconfigs are ready to be contributed back",
                    None,
                    "store_true",
                    False,
                ),
                "check-style": (
                    "Run a style check on the given easyconfigs",
                    None,
                    "store_true",
                    False,
                ),
                "cleanup-easyconfigs": (
                    "Clean up easyconfig files for pull request",
                    None,
                    "store_true",
                    True,
                ),
                "dump-test-report": (
                    "Dump test report to specified path",
                    None,
                    "store_or_None",
                    "test_report.md",
                ),
                "from-pr": (
                    "Obtain easyconfigs from specified PR",
                    int,
                    "store",
                    None,
                    {"metavar": "PR#"},
                ...
                "git-working-dirs-path": (
                    "Path to Git working directories for EasyBuild repositories",
                    str,
                    "store",
                    None,
                ),
                ...
                "install-github-token": (
                    "Install GitHub token (requires --github-user)",
                    None,
                    "store_true",
                    False,
                ),
                ...
                "close-pr-msg": (
                    "Custom close message for pull request closed with --close-pr; ",
                    str,
                    "store",
                    None,
                ),
                "close-pr-reasons": (
                    "Close reason for pull request closed with --close-pr; "
                    "supported values: %s" % ", ".join(VALID_CLOSE_PR_REASONS),
                    str,
                    "store",
                    None,
                ),
                "list-prs": (
                    ...
                    ",".join(
                        [
                            DEFAULT_LIST_PR_STATE,
                            DEFAULT_LIST_PR_ORDER,
                ...
                "new-branch-github": (
                    "Create new branch in GitHub in preparation for a PR",
                    None,
                    "store_true",
                    False,
                ),
                ...
                "new-pr-from-branch": (
                    "Open a new pull request from branch in GitHub",
                    str,
                    "store",
                    None,
                ),
                "pr-branch-name": (
                    "Branch name to use for new PRs; '<timestamp>_new_pr_<name><version>' if unspecified",
                    str,
                    "store",
                    None,
                ),
                "pr-commit-msg": (
                    "Commit message for new/updated pull request created with --new-pr",
                    str,
                    "store",
                    None,
                ),
                "pr-descr": (
                    "Description for new pull request created with --new-pr",
                    str,
                    "store",
                    None,
                ),
                "pr-target-account": (
                    "Target account for new PRs",
                    str,
                    "store",
                    GITHUB_EB_MAIN,
                ),
                "pr-target-branch": (
                    "Target branch for new PRs",
                    str,
                    "store",
                    "develop",
                ),
                "pr-target-repo": (
                    "Target repository for new/updating PRs",
                    str,
                    "store",
                    GITHUB_EASYCONFIGS_REPO,
                ),
                "pr-title": (
                    "Title for new pull request created with --new-pr",
                    str,
                    "store",
                    None,
                ),
                ...
                "sync-branch-with-develop": (
                    "Sync branch with current 'develop' branch",
                    str,
                    "store",
                    None,
                ),
                ...
                "update-branch-github": (
                    "Update specified branch in GitHub",
                    str,
                    "store",
                    None,
                ),
        ...
        opts = OrderedDict(
            {
                ...
                "regtest": ("Enable regression test mode", None, "store_true", False),
                "regtest-output-dir": (
        ...
        opts = OrderedDict(
            {
                ...
                "package-tool": (
                    "Packaging tool to use",
                    None,
                    "store",
                    DEFAULT_PKG_TOOL,
                ),
                "package-tool-options": (
                    "Extra options for packaging tool",
                    None,
                    "store",
                    "",
                ),
                "package-type": (
                    "Type of package to generate",
                    None,
                    "store",
                    DEFAULT_PKG_TYPE,
                ),
                "package-release": (
                    "Package release iteration number",
                    None,
                    "store",
                    DEFAULT_PKG_RELEASE,
                ),
    ...
    def container_options(self):
        ...
        descr = (
            "Container options",
            "Options related to generating container recipes & images",
        )

        opts = OrderedDict(
            {
                "build-image": (
                    "Build container image (requires sudo privileges!)",
                    None,
                    "store_true",
                    False,
                ),
                ...
                "template-recipe": (
                    "Template recipe for container image",
                    str,
                    "store",
                    None,
                ),
                ...
                "type": (
                    "Type of container recipe/image to create",
                    "choice",
                    "store",
                    DEFAULT_CONT_TYPE,
                    CONT_TYPES,
    ...
    def easyconfig_options(self):
        descr = (
            ...
            "Options that affect all specified easyconfig files.",
        )

        opts = OrderedDict(
            {
                "fix-deprecated-easyconfigs": (
                    "Fix use of deprecated functionality in specified easyconfig files.",
                    None,
                    "store_true",
                    False,
                ),
                "inject-checksums": (
                    "Inject checksums of specified type for sources/patches into easyconfig file(s)",
                    "choice",
                    "store_or_None",
                    CHECKSUM_TYPE_SHA256,
                    CHECKSUM_TYPES,
                ...
                "local-var-naming-check": (
                    "Mode to use when checking whether local variables follow the recommended "
                    "naming scheme ('log': only log warnings (no printed messages); 'warn': print "
                    "warnings; 'error': fail with an error)",
                    "choice",
                    "store",
                    LOCAL_VAR_NAMING_CHECK_WARN,
                    LOCAL_VAR_NAMING_CHECKS,
                ),
            }
        )
    ...
    def job_options(self):
        ...
        descr = (
            ...
            "Options for job backend (only relevant when --job is used)",
        )
        ...
        opts = OrderedDict(
            {
                ...
                "deps-type": (
                    ...
                    "store",
                    None,
                    [JOB_DEPS_TYPE_ABORT_ON_ERROR, JOB_DEPS_TYPE_ALWAYS_RUN],
                ),
                "max-jobs": (
                ...
                "max-walltime": (
                    "Maximum walltime for jobs (in hours)",
                    "int",
                    "store",
                    24,
                ),
                "output-dir": (
                    "Output directory for jobs (default: current directory)",
                    None,
                    "store",
                    os.getcwd(),
                ),
                "polling-interval": (
                    "Interval between polls for status of jobs (in seconds)",
                    float,
                    "store",
                    30.0,
                ),
    ...
    def validate(self):
        ...
        if (
            self.options.module_naming_scheme
            and self.options.module_naming_scheme not in avail_mnss
        ):
            msg = "Selected module naming scheme '%s' is unknown: %s" % (
                self.options.module_naming_scheme,
                avail_mnss,
            )
            error_msgs.append(msg)
        ...
        if self.options.cuda_compute_capabilities:
            ...
            faulty_cuda_ccs = [
                x
                for x in self.options.cuda_compute_capabilities
                if not cuda_cc_regex.match(x)
            ]
            if faulty_cuda_ccs:
                error_msg = "Incorrect values in --cuda-compute-capabilities (expected pattern: '%s'): %s"
                error_msgs.append(
                    error_msg % (cuda_cc_regex.pattern, ", ".join(faulty_cuda_ccs))
                )
    ...
    def postprocess(self):
        ...
        if any(
            [
                ...
                self.options.avail_toolchain_opts,
                self.options.avail_hooks,
                self.options.show_system_info,
            ]
        ):
        ...
        if (
            self.options.module_only or self.options.skip
        ) and self.options.backup_modules is None:
            self.log.debug(
                "Auto-enabling --backup-modules because of --module-only or --skip"
            )
    ...
    def _postprocess_optarch(self):
        ...
        if (n_parts > 1 and any(c != 1 for c in map_char_cnts)) or (
            n_parts == 1 and map_char_cnts[0] > 1
        ):
            raise EasyBuildError(
            ...
            if OPTARCH_MAP_CHAR in optarch_parts[0]:
                optarch_dict = {}
                for compiler, compiler_opt in [
                    p.split(OPTARCH_MAP_CHAR) for p in optarch_parts
                ]:
                ...
                self.log.info(
                    "Transforming optarch into a dict: %s", self.options.optarch
                )
    ...
    def _postprocess_close_pr_reasons(self):
        ...
        if self.options.close_pr_msg:
            raise EasyBuildError(
                "Please either specify predefined reasons with --close-pr-reasons or "
        ...
        if any([reason not in VALID_CLOSE_PR_REASONS.keys() for reason in reasons]):
            raise EasyBuildError(
                "Argument to --close-pr_reasons must be a comma separated list of valid reasons "
                + "among %s" % VALID_CLOSE_PR_REASONS.keys()
            )
        self.options.close_pr_msg = ", ".join(
            [VALID_CLOSE_PR_REASONS[reason] for reason in reasons]
        )
    ...
    def _postprocess_list_prs(self):
        ...
        if list_pr_state not in GITHUB_PR_STATES:
            raise EasyBuildError(
                "1st item in --list-prs ('%s') must be one of %s",
                list_pr_state,
                GITHUB_PR_STATES,
            )
        if list_pr_order not in GITHUB_PR_ORDERS:
            raise EasyBuildError(
                "2nd item in --list-prs ('%s') must be one of %s",
                list_pr_order,
                GITHUB_PR_ORDERS,
            )
        if list_pr_direc not in GITHUB_PR_DIRECTIONS:
            raise EasyBuildError(
                "3rd item in --list-prs ('%s') must be one of %s",
                list_pr_direc,
                GITHUB_PR_DIRECTIONS,
            )
    ...
    def _postprocess_include(self):
        ...
        if self.options.include_module_naming_schemes:
            include_module_naming_schemes(
                self.tmpdir, self.options.include_module_naming_schemes
            )
    ...
    def _postprocess_checks(self):
        ...
        if (
            self.options.module_syntax == ModuleGeneratorLua.SYNTAX
            and self.options.modules_tool != Lmod.__name__
        ):
            error_msg = "Generating Lua module files requires Lmod as modules tool; "
            mod_syntaxes = ", ".join(sorted(avail_module_generators().keys()))
            error_msg += (
                "use --module-syntax to specify a different module syntax to use (%s)"
                % mod_syntaxes
            )
        ...
        if self.options.detect_loaded_modules not in LOADED_MODULES_ACTIONS:
            ...
            raise EasyBuildError(
                error_msg
                % (
                    self.options.detect_loaded_modules,
                    ", ".join(LOADED_MODULES_ACTIONS),
                )
            )
    ...
    def _postprocess_config(self):
            ...
            for dest in path_opts:
                if not self.options._action_taken.get(dest, False):
                    if dest == "repository":
                        ...
                        repositorypath = [
                            mk_full_default_path(dest, prefix=self.options.prefix)
                        ]
                        setattr(self.options, dest, repositorypath)
                        self.go_cfg_constants[self.DEFAULTSECT][
                            "DEFAULT_REPOSITORYPATH"
                        ] = repositorypath
                    else:
                        setattr(
        ...
        if self.options.robot is not None:
            ...
            if len(self.options.robot) == 1:
                ...
                if not os.path.isdir(robot_arg):
                    if robot_arg.endswith(EB_FORMAT_EXTENSION) or robot_arg.endswith(
                        YEB_FORMAT_EXTENSION
                    ):
                        info_msg = (
                            "Sole --robot argument %s is not an existing directory, "
                        )
                        ...
                        raise EasyBuildError(
                            "Argument passed to --robot is not an existing directory: %s",
                            robot_arg,
                        )
            ...
            self.options.robot_paths = [
                os.path.abspath(path)
                for path in self.options.robot + self.options.robot_paths
            ]
        ...
        if self.options.search_paths is not None:
            self.options.search_paths = [
                os.path.abspath(path) for path in self.options.search_paths
            ]
    ...
    def _postprocess_list_avail(self):
        ...
        if self.options.avail_cfgfile_constants:
            msg += avail_cfgfile_constants(
                self.go_cfg_constants, self.options.output_format
            )

        # dump possible easyconfig params
        if self.options.avail_easyconfig_params:
            msg += avail_easyconfig_params(
                self.options.easyblock, self.options.output_format
            )
        ...
        if self.options.list_easyblocks:
            msg += list_easyblocks(
                self.options.list_easyblocks, self.options.output_format
            )
        ...
        if self.options.avail_toolchain_opts:
            msg += avail_toolchain_opts(
                self.options.avail_toolchain_opts, self.options.output_format
            )
        ...
        if self.options.avail_module_naming_schemes:
            msg += self.avail_list(
                "module naming schemes", avail_module_naming_schemes()
            )
    ...
    def show_default_configfiles(self):
        ...
        system_cfg_glob_paths = os.path.join(
            "{" + ", ".join(XDG_CONFIG_DIRS) + "}", "easybuild.d", "*.cfg"
        )
        ...
        lines = [
            ...
            "* user-level: %s"
            % os.path.join(
                "${XDG_CONFIG_HOME:-$HOME/.config}", "easybuild", "config.cfg"
            ),
            "  -> %s => %s"
            % (
                DEFAULT_USER_CFGFILE,
                ("not found", "found")[os.path.exists(DEFAULT_USER_CFGFILE)],
            ),
            "* system-level: %s"
            % os.path.join("${XDG_CONFIG_DIRS:-/etc}", "easybuild.d", "*.cfg"),
            "  -> %s => %s"
            % (
                system_cfg_glob_paths,
                ", ".join(DEFAULT_SYS_CFGFILES) or "(no matches)",
            ),
            "",
            "Default list of existing configuration files (%d): %s"
            % (found_cfgfile_cnt, found_cfgfile_list),
        ]
        return "\n".join(lines)
    ...
    def show_config(self):
        ...
        def reparse_cfg(args=None, withcfg=True):
            ...
            if args is None:
                args = []
            cfg = EasyBuildOptions(
                go_args=args,
                go_useconfigfiles=withcfg,
                envvar_prefix=CONFIG_ENV_VAR_PREFIX,
                with_include=False,
                single_cfg_level=True,
            )
        ...
        unset_env_vars(
            [v for v in os.environ if v.startswith(CONFIG_ENV_VAR_PREFIX)],
            verbose=False,
        )
        ...
        for prefix in sorted(default_opts_dict):
            for opt in sorted(default_opts_dict[prefix]):
                ...
                if (
                    self.options.show_full_config
                    or opt in include_opts
                    or not is_default
                ):
...
def parse_options(args=None, with_include=True):
    ...
    if os.environ.get("DEBUG_EASYBUILD_OPTIONS", "0").lower() in (
        "1",
        "true",
        "yes",
        "y",
...
def set_up_configuration(args=None, logfile=None, testing=False, silent=False):
    ...
    tweaked_ecs_paths, pr_path = alt_easyconfig_paths(
        tmpdir, tweaked_ecs=tweaked_ecs, from_pr=options.from_pr
    )
    auto_robot = (
        try_to_generate or options.check_conflicts or options.dep_graph or search_query
    )
    robot_path = det_robot_path(
        options.robot_paths, tweaked_ecs_paths, pr_path, auto_robot=auto_robot
    )
    ...
    new_update_opt = (
        options.new_pr
        or options.new_pr_from_branch
        or options.update_branch_github
        or options.update_pr
    )
    ...
    build_options = {
        ...
        "external_modules_metadata": parse_external_modules_metadata(
            options.external_modules_metadata
        ),
        "pr_path": pr_path,
...
def process_software_build_specs(options):
    ...
    for opt in ["software", "toolchain"]:
        ...
        if val or tryval:
            if val and tryval:
                logger.warning(
                    "Ignoring --try-%(opt)s, only using --%(opt)s specification"
                    % {"opt": opt}
                )
            elif tryval:
                try_to_generate = True
            ...
            build_specs.update(
                {"%sname" % key_prefix: val[0], "%sversion" % key_prefix: val[1],}
            )
...
def parse_external_modules_metadata(cfgs):
    ...
    if not cfgs:
        ...
        for topdir in topdirs:
            cfgs.extend(
                glob.glob(os.path.join(topdir, "etc", "*external_modules_metadata.cfg"))
            )
...
def set_tmpdir(tmpdir=None, raise_error=False):
    ...
    try:
        ...
        raise EasyBuildError(
            "Failed to create temporary directory (tmpdir: %s): %s", tmpdir, err
        )
    ...
    if re.search(special_chars_regex, current_tmpdir):
        ...
        _log.info(
            "Detected special characters in path to temporary directory, replacing them to avoid trouble: %s"
        )
        try:
            os.makedirs(current_tmpdir)
        except OSError as err:
            raise EasyBuildError(
                "Failed to create path to temporary directory %s: %s",
                current_tmpdir,
                err,
            )
    ...
    try:
        ...
        if not run_cmd(
            ...
            msg = (
                "The temporary directory (%s) does not allow to execute files. "
                % tempfile.gettempdir()
            )
            msg += (
                "This can cause problems in the build process, consider using --tmpdir."
            )
            if raise_error:
            ...
            _log.debug(
                "Temporary directory %s allows to execute files, good!"
                % tempfile.gettempdir()
            )
        ...
        raise EasyBuildError(
            "Failed to test whether temporary directory allows to execute files: %s",
            err,
        )
