...
class Pandaseq(Application):
    """
    Support for building pandaseq  (paired end read assembler for Illumina sequences)
    """

    def configure(self):
        """
        First create the configure script before running it
        """
        srcdir = self.getcfg('startfrom')
	cmd = os.path.join(srcdir, "autogen.sh")
