...
class YebTest(EnhancedTestCase):
    ...
    def test_parse_yeb(self):
        ...
        for filename in test_files:
            ...
            ec_file = glob.glob(
                os.path.join(test_easyconfigs, "test_ecs", "*", "*", "%s.eb" % filename)
            )[0]
    ...
    def test_is_yeb_format(self):
        ...
        testdir = os.path.dirname(os.path.abspath(__file__))
        test_yeb = os.path.join(
            testdir, "easyconfigs", "yeb", "bzip2-1.0.6-GCC-4.9.2.yeb"
        )
        ...
        test_eb = os.path.join(
            testdir, "easyconfigs", "test_ecs", "g", "gzip", "gzip-1.4.eb"
        )
    ...
    def test_bad_toolchain_format(self):
        ...
        expected = (
            r"Can not convert list .* to toolchain dict. Expected 2 or 3 elements"
        )
    ...
    def test_external_module_toolchain(self):
        ...
        ecpath = os.path.join(
            os.path.dirname(__file__), "easyconfigs", "yeb", "CrayCCE-5.1.29.yeb"
        )
        metadata = {
            "name": ["foo", "bar"],
            "version": ["1.2.3", "3.2.1"],
