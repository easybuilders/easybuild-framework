...
class ToolchainTest(EnhancedTestCase):
    ...
    def test_mpi_exec_nranks(self):
        """Test mpi_exec_nranks function."""
        self.modtool.prepend_module_path(self.test_prefix)

        tc = self.get_toolchain('gompi', version='2018a')
        tc.prepare()
        self.assertEqual(tc.mpi_exec_nranks(nr_ranks=2), "mpirun -n 2")
        self.modtool.purge()
        ...
        self.assertTrue(mpi_exec_nranks_re.match(tc.mpi_exec_nranks(nr_ranks=4)))

        # test specifying custom template for MPI commands
        init_config(build_options={'mpi_exec_template': "mpiexec -np %(nr_ranks)s --", 'silent': True})
        self.assertEqual(tc.mpi_exec_nranks(nr_ranks="7"), "mpiexec -np 7 --")
