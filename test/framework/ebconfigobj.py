...
class TestEBConfigObj(EnhancedTestCase):
    ...
    def test_ebconfigobj_default(self):
        ...
        data = [
            ...
            (
                ...
                {"toolchain": {"name": self.tc_first, "version": "1"}},
            ),
        ]
    ...
    def test_squash_invalid(self):
        ...
        txt_wrong_versions = [
            "[SUPPORTED]",
            "versions = %s"
            % ", ".join(all_wrong_versions),  # there's a conflict in the versions list
            "toolchains = %s,%s" % (tc_tmpl % tc_first, tc_tmpl % tc_last),
        ]
    ...
    def test_toolchain_squash_nested(self):
        ...
        tests = [
            ...
            (
                ...
                "2.1",
                {"y": "d", "x": "3", "z": "3"},
            ),  # values from most precise versop
            (tc_first, "1.0", {"y": "z1"}),  # toolchain section, not default
            (
                tc_first,
                "1.1",
                {"y": "b", "x": "1"},
            ),  # the version section precedes the toolchain section
    ...
    def test_ebconfigobj(self):
        ...
        tcfirst = ",".join(
            ["%s == 0.0.0" % self.tc_namesmax[0], "%s > 0.0.0" % self.tc_namesmax[0]]
        )
        configobj_txt = [
            "[SUPPORTED]",
        ...
        res = cov.get_specs_for(
            version="2.3.4", tcname=self.tc_first, tcversion="1.0.0"
        )
