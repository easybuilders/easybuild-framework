...
class EasyBuildConfigTest(EnhancedTestCase):
    ...
    def test_default_config(self):
        ...
        self.assertEqual(
            config_options["sourcepath"], os.path.join(eb_homedir, "sources")
        )
        ...
        self.assertEqual(
            config_options["repositorypath"], [os.path.join(eb_homedir, "ebfiles_repo")]
        )
        self.assertEqual(config_options["logfile_format"][0], "easybuild")
        self.assertEqual(
            config_options["logfile_format"][1],
    ...
    def test_error_env_var_typo(self):
        ...
        error = (
            r"Found 2 environment variable\(s\) that are prefixed with %s "
            % CONFIG_ENV_VAR_PREFIX
        )
        error += "but do not match valid option\(s\): "
        error += ",".join(
            ["EASYBUILD_FOO", "EASYBUILD_THERESNOSUCHCONFIGURATIONOPTION"]
        )
    ...
    def test_install_path(self):
        ...
        # defaults
        self.assertEqual(
            install_path(), os.path.join(self.test_installpath, "software")
        )
        self.assertEqual(
            install_path("software"), os.path.join(self.test_installpath, "software")
        )
        self.assertEqual(
            install_path(typ="mod"), os.path.join(self.test_installpath, "modules")
        )
        self.assertEqual(
            install_path("modules"), os.path.join(self.test_installpath, "modules")
        )

        self.assertErrorRegex(
            EasyBuildError, "Unknown type specified", install_path, typ="foo"
        )
    ...
    def test_generaloption_config_file(self):
        ...
        cfgtxt = "\n".join(["[config]", "installpath = %s" % testpath2,])
        ...
        self.assertEqual(
            install_path("mod"), os.path.join(testpath2, "modules")
        )  # via config file
        ...
        self.assertEqual(
            get_repositorypath(), [os.path.join(topdir, "ebfiles_repo"), "somesubdir"]
        )  # via config file
        ...
        self.assertEqual(
            options.robot_paths[3], os.path.join("/tmp", os.getenv("USER"))
        )
        # first path in DEFAULT_ROBOT_PATHS
        self.assertEqual(
            options.robot_paths[4], os.path.join(tmpdir, "easybuild", "easyconfigs")
        )
        ...
        )  # via environment variable $EASYBUILD_SOURCEPATHS
        self.assertEqual(
            install_path(), os.path.join(testpath3, "software")
        )  # via command line
        self.assertEqual(install_path("mod"), installpath_modules),  # via config file
        self.assertEqual(build_path(), testpath1)  # via config file
    ...
    def test_configuration_variables(self):
        ...
        cv3 = ConfigurationVariables(
            {"foo": "bar"}
        )  # note: argument is ignored, an instance is already available
    ...
    def test_build_options(self):
        ...
        BuildOptions.__class__._instances.clear()
        bo = BuildOptions({"debug": False, "force": True})
        ...
        self.assertErrorRegex(
            Exception,
            ".*(item assignment|no attribute).*",
            lambda x: bo.update(x),
            {"debug": True},
        )
        self.assertErrorRegex(
            AttributeError,
            ".*no attribute.*",
            lambda x: bo.__setitem__(*x),
            ("debug", True),
        )
        ...
        self.assertErrorRegex(
            KeyError, msg, BuildOptions, {"thisisclearlynotavalidbuildoption": "FAIL"}
        )
        ...
        self.assertErrorRegex(
            ...
            msg,
            init_build_options,
            {"thisisclearlynotavalidbuildoption": "FAIL"},
        )
        bo = init_build_options(
            {"robot_path": "/some/robot/path", "stop": "configure",}
        )
    ...
    def test_XDG_CONFIG_env_vars(self):
        ...
        cfg_files = [
            ...
            os.path.join(
                homedir, "easybuild", "config.cfg"
            ),  # $XDG_CONFIG_HOME goes last
        ]
        reload(eboptions)
        eb_go = eboptions.parse_options(args=[])
        self.assertEqual(eb_go.options.configfiles, cfg_files)
    ...
    def test_flex_robot_paths(self):
        ...
        self.assertEqual(
            eb_go.options.robot_paths, ["/foo/bar", tmp_ecs_dir, "/baz", "/trala"]
        )
        eb_go = eboptions.parse_options(args=["--robot-paths=/foo/bar:/trala::/baz"])
        self.assertEqual(
            eb_go.options.robot_paths, ["/foo/bar", "/trala", tmp_ecs_dir, "/baz"]
        )
        ...
        config_file = os.path.join(self.tmpdir, "testconfig.cfg")
        cfgtxt = "\n".join(["[config]", "robot-paths=/cfgfirst::/cfglast",])
        write_file(config_file, cfgtxt)
        eb_go = eboptions.parse_options(args=["--configfiles=%s" % config_file])
        self.assertEqual(
            eb_go.options.robot_paths, ["/cfgfirst", tmp_ecs_dir, "/cfglast"]
        )
        ...
        eb_go = eboptions.parse_options(
            args=["--robot-paths=/veryfirst:", "--configfiles=%s" % config_file]
        )
        ...
        mkdir(first)
        eb_go = eboptions.parse_options(
            args=["--robot-paths=/foo/bar::/baz", "--robot=%s" % first]
        )
        self.assertEqual(
            eb_go.options.robot_paths, [first, "/foo/bar", tmp_ecs_dir, "/baz"]
        )
    ...
    def test_get_log_filename(self):
        ...
        regex = re.compile(
            os.path.join(tmpdir, r"easybuild-foo-1\.2\.3-[0-9]{8}\.[0-9]{6}\.log$")
        )
        self.assertTrue(
            regex.match(res), "Pattern '%s' matches '%s'" % (regex.pattern, res)
        )

        res = get_log_filename("foo", "1.2.3", date="19700101")
        regex = re.compile(
            os.path.join(tmpdir, r"easybuild-foo-1\.2\.3-19700101\.[0-9]{6}\.log$")
        )
        self.assertTrue(
            regex.match(res), "Pattern '%s' matches '%s'" % (regex.pattern, res)
        )
        ...
        regex = re.compile(
            os.path.join(tmpdir, r"easybuild-foo-1\.2\.3-[0-9]{8}\.094651\.log$")
        )
        self.assertTrue(
            regex.match(res), "Pattern '%s' matches '%s'" % (regex.pattern, res)
        )
        ...
        regex = re.compile(
            os.path.join(tmpdir, r"easybuild-foo-1\.2\.3-19700101\.094651\.log$")
        )
        self.assertTrue(
            regex.match(res), "Pattern '%s' matches '%s'" % (regex.pattern, res)
        )
        ...
        res = get_log_filename("foo", "1.2.3", date="19700101", timestamp="094651")
        regex = re.compile(
            os.path.join(tmpdir, r"easybuild-foo-1\.2\.3-19700101\.094651\.log\.1$")
        )
        self.assertTrue(
            regex.match(res), "Pattern '%s' matches '%s'" % (regex.pattern, res)
        )
        ...
        prev_log_filenames = []
        for i in range(10):
            res = get_log_filename(
                "foo", "1.2.3", date="19700101", timestamp="094651", add_salt=True
            )
            regex = re.compile(
                os.path.join(
                    tmpdir, r"easybuild-foo-1\.2\.3-19700101\.094651\.[a-zA-Z]{5}\.log$"
                )
            )
            self.assertTrue(
                regex.match(res), "Pattern '%s' matches '%s'" % (regex.pattern, res)
    ...
    def test_log_file_format(self):
        ...
        self.assertEqual(
            log_file_format(), "easybuild-%(name)s-%(version)s-%(date)s.%(time)s.log"
        )
        ...
        res = log_file_format(
            return_directory=True, ec=ec, date="20190322", timestamp="094356"
        )
        self.assertEqual(res, "easybuild")

        # partial templating is done when only some values are provided...
        self.assertEqual(
            log_file_format(ec=ec), "easybuild-foo-1.2.3-%(date)s.%(time)s.log"
        )
        ...
        init_config(
            args=[
                "--logfile-format=eb-%(name)s-%(date)s,log-%(version)s-%(date)s-%(time)s.out"
            ]
        )
    ...
    def test_log_path(self):
        ...
        init_config(
            args=[
                "--logfile-format=easybuild-%(name)s-%(version)s-%(date)s-%(time)s,log.txt"
            ]
        )
