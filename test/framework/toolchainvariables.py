...
class ToolchainVariablesTest(EnhancedTestCase):
    ...
    def test_toolchainvariables(self):
        ...
        # sanitize is on self
        self.assertEqual(
            tcv["LIBBLAS"].__repr__(), "[['a', 'b', 'c'], ['d', 'e', 'f']]"
        )
        ...
        # packed_linker
        tcv.try_function_on_element(
            "set_packed_linker_options"
        )  # don't use it like this (this is internal)
        ...
        tcv["LIBLAPACK"].sanitize()
        self.assertEqual(
            tcv["LIBLAPACK"].__repr__(), "[['a', 'b', 'c', 'g', 'h'], ['d', 'e', 'f']]"
        )
        self.assertEqual(
            str(tcv["LIBLAPACK"]),
            "-Wl,-Bstatic,-Xstart,-la,-lb,-lc,-lg,-lh,-Xstop,-Bdynamic -ld -le -lf",
        )
...
def suite():
    ...
    return TestLoaderFiltered().loadTestsFromTestCase(
        ToolchainVariablesTest, sys.argv[1:]
    )
