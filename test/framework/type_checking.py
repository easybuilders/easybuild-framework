...
class TypeCheckingTest(EnhancedTestCase):
    ...
    def test_check_type_of_param_value_name_version(self):
        ...
        for key in ["name", "version"]:
            ...
            for not_a_string in [100, 1.5, ("bar",), ["baz"], None]:
                self.assertEqual(
                    check_type_of_param_value(key, not_a_string), (False, None)
                )
        ...
        self.assertEqual(
            check_type_of_param_value("version", 1.5, auto_convert=True), (True, "1.5")
        )
    ...
    def test_check_type_of_param_value_toolchain(self):
        ...
        self.assertEqual(
            check_type_of_param_value("toolchain", toolchain), (True, toolchain)
        )
        ...
        # missing 'version' key
        self.assertEqual(
            check_type_of_param_value("toolchain", {"name": "intel"}), (False, None)
        )
        # non-string value for 'version'
        toolchain = {"name": "foss", "version": 100}
        self.assertEqual(
            check_type_of_param_value("toolchain", toolchain), (False, None)
        )
        ...
        for tcspec in ["intel, 2015a", ["intel", "2015a"], toolchain]:
            self.assertEqual(
                check_type_of_param_value("toolchain", tcspec, auto_convert=True),
                (True, toolchain),
            )
    ...
    def test_check_type_of_param_value_deps(self):
            ...
            [
                {
                    ...
                    "version": "1.2.3",
                    "versionsuffix": "",
                    "toolchain": {"name": "GCC", "version": "4.7"},
                }
            ],
            [
                {
        ...
        for inp in inputs:
            self.assertEqual(
                check_type_of_param_value("dependencies", inp), (True, inp)
            )
        ...
        for inp in inputs:
            self.assertEqual(
                check_type_of_param_value("dependencies", inp), (False, None)
            )

        # dependencies (auto-convert)
        self.assertEqual(
            check_type_of_param_value(
                "dependencies", [{"foo": "1.2.3"}], auto_convert=True
            ),
        ...
        inp = [("foo", "1.2.3")]
        self.assertEqual(
            check_type_of_param_value("dependencies", inp, auto_convert=True),
            (True, [("foo", "1.2.3")]),
        )
        inp = [("foo", "1.2.3"), {"bar": "3.4.5"}, ("baz", "9.8.7")]
        out = (
            True,
            [("foo", "1.2.3"), {"name": "bar", "version": "3.4.5"}, ("baz", "9.8.7")],
        )
        self.assertEqual(
            check_type_of_param_value("dependencies", inp, auto_convert=True), out
        )
        ...
        for inp in inputs:
            self.assertEqual(
                check_type_of_param_value("osdependencies", inp), (True, inp)
            )

        inp = ["zlib", ["openssl-devel", "libssl-dev", "libopenssl-devel"]]
        self.assertEqual(
            check_type_of_param_value("osdependencies", inp), (False, None)
        )

        # osdependencies (auto-convert)
        out = ["zlib", ("openssl-devel", "libssl-dev", "libopenssl-devel")]
        self.assertEqual(
            check_type_of_param_value("osdependencies", inp, auto_convert=True),
            (True, out),
        )
    ...
    def test_check_type_of_param_value_sanity_check_paths(self):
        ...
        for inp in inputs:
            self.assertEqual(
                check_type_of_param_value("sanity_check_paths", inp), (True, inp)
            )
        ...
        for inp in inputs:
            self.assertEqual(
                check_type_of_param_value("sanity_check_paths", inp), (False, None)
            )

        # sanity_check_paths (auto-convert)
        inp = {
            "files": ["bin/foo", ["bin/bar", "bin/baz"]],
            "dirs": [["lib", "lib64", "lib32"]],
        }
        out = {
            "files": ["bin/foo", ("bin/bar", "bin/baz")],
            "dirs": [("lib", "lib64", "lib32")],
        ...
        self.assertEqual(
            check_type_of_param_value("sanity_check_paths", inp, auto_convert=True),
            (True, out),
        )
    ...
    def test_check_type_of_param_value_checksums(self):
        ...
        sha256_checksum1 = (
            "fa618be8435447a017fd1bf2c7ae922d0428056cfc7449f7a8641edf76b48265"
        )
        sha256_checksum2 = (
            "b5f9cb06105c1d2d30719db5ffb3ea67da60919fb68deaefa583deccd8813551"
        )
        sha256_checksum3 = (
    ...
    def test_convert_value_type(self):
        ...
        self.assertErrorRegex(
            EasyBuildError, "Converting type of .* failed", convert_value_type, "", int
        )
        # 1.6 can't be parsed as an int (yields "invalid literal for int() with base 10" error)
        self.assertErrorRegex(
            EasyBuildError,
            "Converting type of .* failed",
            convert_value_type,
            "1.6",
            int,
        )
        ...
        self.assertEqual(
            convert_value_type(("foo", "bar"), LIST_OF_STRINGS), ["foo", "bar"]
        )
    ...
    def test_to_toolchain_dict(self):
        ...
        # normal cases
        self.assertEqual(
            to_toolchain_dict(("intel", "2015a")), {"name": "intel", "version": "2015a"}
        )
        self.assertEqual(
            to_toolchain_dict("intel, 2015a"), {"name": "intel", "version": "2015a"}
        )
        self.assertEqual(
            to_toolchain_dict(["gcc", "4.7"]), {"name": "gcc", "version": "4.7"}
        )
        ...
        self.assertErrorRegex(
            EasyBuildError, errstr, to_toolchain_dict, "intel, 2015, True, a"
        )
        self.assertErrorRegex(EasyBuildError, errstr, to_toolchain_dict, "intel")
        self.assertErrorRegex(
            EasyBuildError, errstr, to_toolchain_dict, ["gcc", "4", "False", "7"]
        )
    ...
    def test_to_dependency(self):
        ...
        self.assertEqual(
            to_dependency({"lib": "1.2.8"}), {"name": "lib", "version": "1.2.8"}
        )
        self.assertEqual(
            to_dependency({"lib": "1.2.8", "toolchain": "GCC, 4.8.2"}), lib_dict
        )
        self.assertEqual(
            to_dependency({"lib": "1.2.8", "toolchain": ["GCC", "4.8.2"]}), lib_dict
        )
        ...
        self.assertEqual(
            to_dependency({"name": "fftw/3.3.4.2", "external_module": True}), expected
        )
        ...
        self.assertEqual(
            to_dependency({"foo": "1.3", "versionsuffix": "-bar"}), foo_dict
        )

        foo_dict.update({"toolchain": {"name": "GCC", "version": "4.8.2"}})
        self.assertEqual(
            to_dependency(
                {"foo": "1.3", "versionsuffix": "-bar", "toolchain": "GCC, 4.8.2"}
            ),
            foo_dict,
        )
        ...
        self.assertEqual(
            to_dependency(
                {"name": "foo", "version": "1.2.3", "toolchain": ["GCC", "4.8.2"]}
            ),
            foo_dict,
        )
        self.assertEqual(to_dependency(foo_dict), foo_dict)
        ...
        self.assertErrorRegex(
            EasyBuildError,
            "Found unexpected \(key, value\) pair: .*",
            to_dependency,
            foo_dict,
        )
        ...
        self.assertErrorRegex(
            EasyBuildError,
            "Can not parse dependency without name and version: .*",
            to_dependency,
            {"toolchain": "lib, 1.2.8", "versionsuffix": "suff"},
        )
        # too many values
        dep_spec = {
            "lib": "1.2.8",
        ...
        self.assertErrorRegex(
            EasyBuildError,
            r"Found unexpected \(key, value\) pair: .*",
            to_dependency,
            dep_spec,
        )
    ...
    def test_is_value_of_type(self):
        ...
        self.assertTrue(
            is_value_of_type({"name": "intel", "version": "2015a"}, TOOLCHAIN_DICT)
        )
        # version value should be string, not int
        self.assertFalse(
            is_value_of_type({"name": "intel", "version": 100}, TOOLCHAIN_DICT)
        )
        # missing version key
        self.assertFalse(
            is_value_of_type({"name": "intel", "foo": "bar"}, TOOLCHAIN_DICT)
        )
        # extra key, shouldn't be there
        self.assertFalse(
            is_value_of_type(
                {"name": "intel", "version": "2015a", "foo": "bar"}, TOOLCHAIN_DICT
            )
        )

        # dependency type check
        self.assertTrue(
            is_value_of_type({"name": "intel", "version": "2015a"}, DEPENDENCY_DICT)
        )
        self.assertTrue(
            is_value_of_type(
                {
        ...
        dependencies.append(
            {"name": "foo", "version": "1.2.3", "toolchain": "intel, 2015a"}
        )
        ...
        # not a list
        self.assertFalse(
            is_value_of_type({"name": "intel", "version": "2015a"}, DEPENDENCIES)
        )

        # no extra keys allowed, only name/version/versionsuffix/toolchain
        self.assertFalse(
            is_value_of_type(
                {"name": "intel", "version": "2015a", "foo": "bar"}, DEPENDENCIES
            )
        )
        ...
        self.assertTrue(
            is_value_of_type([("foo", "fob"), ("bar", "bat")], STRING_OR_TUPLE_LIST)
        )
        ...
        self.assertTrue(
            is_value_of_type(
                {"files": ["one", "two"], "dirs": ["dirA", "dirB"]},
                SANITY_CHECK_PATHS_DICT,
            )
        )
        self.assertTrue(
            is_value_of_type(
                {"files": ["f1", ("f2a", "f2b")], "dirs": []}, SANITY_CHECK_PATHS_DICT
            )
        )
        self.assertTrue(
            is_value_of_type({"files": [], "dirs": []}, SANITY_CHECK_PATHS_DICT)
        )
        ...
        self.assertFalse(
            is_value_of_type({"files": (1, 2), "dirs": []}, SANITY_CHECK_PATHS_DICT)
        )
        ...
        self.assertFalse(
            is_value_of_type({"files": ["foo", 2], "dirs": []}, SANITY_CHECK_PATHS_DICT)
        )
        # extra key is not allowed
        self.assertFalse(
            is_value_of_type(
                {"files": [], "dirs": [], "foo": []}, SANITY_CHECK_PATHS_DICT
            )
        )
        # no keys at all
    ...
    def test_check_known_keys(self):
        ...
        self.assertTrue(
            check_known_keys({"name": "intel", "version": "2015a"}, known_keys)
        )
        self.assertTrue(
            check_known_keys(
                {"name": "intel", "version": "2015a", "versionsuffix": "-test"},
                known_keys,
            )
        )
        self.assertFalse(
            check_known_keys(
                {"name": "intel", "version": "2015a", "foo": "bar"}, known_keys
            )
        )
    ...
    def test_check_required_keys(self):
        ...
        self.assertFalse(
            check_required_keys({"one": 1, "two": 2}, ("one", "two", "three"))
        )
        self.assertTrue(check_required_keys({"one": 1, "two": 2}, ["one"]))

        req_keys = ["name", "version"]
        self.assertTrue(
            check_required_keys({"name": "intel", "version": "2015a"}, req_keys)
        )
        self.assertFalse(check_required_keys({"name": "intel"}, req_keys))
        self.assertTrue(
            check_required_keys(
                {"name": "foo", "version": "1.2.3", "versionsuffix": "-test"}, req_keys
            )
        )
        self.assertFalse(
            check_required_keys({"name": "foo", "versionsuffix": "-test"}, req_keys)
        )
    ...
    def test_check_element_types(self):
        ...
        self.assertTrue(
            check_element_types({"one": 1, 2: "two"}, {"one": [int], 2: [str]})
        )
        self.assertFalse(
            check_element_types({"one": 1, 2: "two"}, {"one": [str], 2: [str]})
        )
        ...
        self.assertErrorRegex(
            EasyBuildError,
            "Don't know how to check element types .*",
            check_element_types,
            1,
            [],
        )
    ...
    def test_to_list_of_strings(self):
        ...
        self.assertEqual(
            to_list_of_strings(["foo", "bar", "baz"]), ["foo", "bar", "baz"]
        )
        ...
        error_pattern = (
            r"Don't know how to convert provided value to a list of strings: "
        )
        self.assertErrorRegex(
            EasyBuildError, error_pattern + "123", to_list_of_strings, 123
        )
        self.assertErrorRegex(
            EasyBuildError, error_pattern + "True", to_list_of_strings, True
        )
        self.assertErrorRegex(
            EasyBuildError, error_pattern, to_list_of_strings, [("foo", "bar")]
        )
    ...
    def test_to_list_of_strings_and_tuples(self):
        ...
        self.assertEqual(
            to_list_of_strings_and_tuples([("foo", "bar")]), [("foo", "bar")]
        )
        self.assertEqual(
            to_list_of_strings_and_tuples([("foo", "bar"), "baz"]),
            [("foo", "bar"), "baz"],
        )
        ...
        self.assertEqual(to_list_of_strings_and_tuples(("foo",)), ["foo"])
        self.assertEqual(
            to_list_of_strings_and_tuples([["bar", "baz"]]), [("bar", "baz")]
        )
        self.assertEqual(
            to_list_of_strings_and_tuples((["bar", "baz"],)), [("bar", "baz")]
        )
        ...
        self.assertErrorRegex(
            EasyBuildError, error_msg, to_list_of_strings_and_tuples, ["foo", 1]
        )
        self.assertErrorRegex(
            EasyBuildError, error_msg, to_list_of_strings_and_tuples, (1,)
        )
    ...
    def test_to_sanity_check_paths_dict(self):
        ...
        inputs = [
            (
                {"files": ["foo"], "dirs": [["bar", "baz"]]},
                {"files": ["foo"], "dirs": [("bar", "baz")]},
            ),
            (
                {"files": ["foo", ["bar", "baz"]], "dirs": []},
                {"files": ["foo", ("bar", "baz")], "dirs": []},
            ),
            (
        ...
        self.assertErrorRegex(
            EasyBuildError,
            "Expected value to be a dict",
            to_sanity_check_paths_dict,
            [],
        )
    ...
    def test_ensure_iterable_license_specs(self):
        ...
        self.assertErrorRegex(
            EasyBuildError, error_msg, ensure_iterable_license_specs, 42
        )
        self.assertErrorRegex(
            EasyBuildError, error_msg, ensure_iterable_license_specs, {"1": "foo"}
        )
        self.assertErrorRegex(
            EasyBuildError, error_msg, ensure_iterable_license_specs, [None]
        )
        self.assertErrorRegex(
            EasyBuildError, error_msg, ensure_iterable_license_specs, [42]
        )
        self.assertErrorRegex(
            EasyBuildError, error_msg, ensure_iterable_license_specs, [42, "foo"]
        )
        self.assertErrorRegex(
            EasyBuildError, error_msg, ensure_iterable_license_specs, [["foo"]]
        )
        self.assertErrorRegex(
            EasyBuildError, error_msg, ensure_iterable_license_specs, [(42, "foo")]
        )
