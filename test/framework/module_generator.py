...
class ModuleGeneratorTest(EnhancedTestCase):
    ...
    def test_set_default_module(self):
        ...
        if self.MODULE_GENERATOR_CLASS == ModuleGeneratorLua and not isinstance(
            self.modtool, Lmod
        ):
            return
        ...
        version_one = "1.0"
        version_one_path = os.path.join(
            modules_base_path, version_one + self.modgen.MODULE_FILE_EXTENSION
        )
        write_file(version_one_path, txt)

        version_two = "2.0"
        version_two_path = os.path.join(
            modules_base_path, version_two + self.modgen.MODULE_FILE_EXTENSION
        )
        ...
        if self.MODULE_GENERATOR_CLASS == ModuleGeneratorTcl:
            test_cases = [
                ...
                (
                    ...
                    "is-loaded Python/2.7.15-GCCcore-8.2.0",
                ),
                ...
                (
                    ["foo/1.2.3", "bar/4.5.6"],
                    ["is-loaded foo/1.2.3", "is-loaded bar/4.5.6"],
                ),
                (
                    ["foo", "bar", "baz"],
                    ["is-loaded foo", "is-loaded bar", "is-loaded baz"],
                ),
            ]
        else:
            test_cases = [
                ...
                (
                    "Python/2.7.15-GCCcore-8.2.0",
                    'isloaded("Python/2.7.15-GCCcore-8.2.0")',
                ),
                ...
                (
                    ["foo/1.2.3", "bar/4.5.6"],
                    ['isloaded("foo/1.2.3")', 'isloaded("bar/4.5.6")'],
                ),
                (
                    ["foo", "bar", "baz"],
                    ['isloaded("foo")', 'isloaded("bar")', 'isloaded("baz")'],
                ),
            ]
    ...
    def test_load(self):
        ...
        if self.MODULE_GENERATOR_CLASS == ModuleGeneratorTcl:
            ...
            self.assertEqual(
                expected, self.modgen.load_module("mod_name", recursive_unload=True)
            )
            ...
            if self.modtool.supports_depends_on:
                expected = "\n".join(["", "depends-on mod_name", "",])
                self.assertEqual(
                    expected, self.modgen.load_module("mod_name", depends_on=True)
                )
                ...
                self.assertErrorRegex(
                    EasyBuildError,
                    expected,
                    self.modgen.load_module,
                    "mod_name",
                    depends_on=True,
                )
                init_config(build_options={"mod_depends_on": "True"})
                self.assertErrorRegex(
                    EasyBuildError, expected, self.modgen.load_module, "mod_name"
                )
        else:
            # default: guarded module load (which implies no recursive unloading)
            expected = "\n".join(
    ...
    def test_load_multi_deps(self):
        ...
        if self.modtool.supports_depends_on:
            ...
            res = self.modgen.load_module(
                "Python/3.7.4", multi_dep_mods=multi_dep_mods, depends_on=True
            )
        ...
        if self.modtool.supports_depends_on:
            ...
            res = self.modgen.load_module(
                "foo/1.2.3", multi_dep_mods=multi_dep_mods, depends_on=True
            )
        ...
        if self.modtool.supports_depends_on:
            res = self.modgen.load_module(
                "one/1.0", multi_dep_mods=["one/1.0"], depends_on=True
            )
    ...
    def test_modulerc(self):
        ...
        self.assertErrorRegex(
            EasyBuildError,
            "Incorrect module_version value type",
            self.modgen.modulerc,
            "foo",
        )
        ...
        if isinstance(self.modtool, Lmod) and LooseVersion(
        ...
        expected = "\n".join(["#%Module", "module-version test/1.2.3.4.5 1.2.3",])
        ...
        self.assertEqual(
            self.modtool.exist(["test/1.2.3.4.5", "test/1.2.3.4", "test/1.2.3"]),
            [True, False, True],
        )
        ...
        expected = "\n".join(["#%Module", "module-version test/4.5.6 4",])
    ...
    def test_unload(self):
        ...
        if self.MODULE_GENERATOR_CLASS == ModuleGeneratorTcl:
            expected = "\n".join(["", "module unload mod_name",])
        else:
            expected = "\n".join(["", 'unload("mod_name")',])
    ...
    def test_append_paths(self):
        ...
        if self.MODULE_GENERATOR_CLASS == ModuleGeneratorTcl:
            ...
            res = self.modgen.append_paths(
                "key", ["1234@example.com"], expand_relpaths=False
            )
            ...
            self.assertEqual(
                expected, self.modgen.append_paths("key", ["/abs/path"], allow_abs=True)
            )
        ...
        self.assertErrorRegex(
            EasyBuildError,
            "Absolute path %s/foo passed to update_paths "
            "which only expects relative paths." % self.modgen.app.installdir,
            self.modgen.append_paths,
            "key2",
            ["bar", "%s/foo" % self.modgen.app.installdir],
        )
    ...
    def test_module_extensions(self):
        ...
        for pattern in patterns:
            ...
            self.assertTrue(
                regex.search(desc), "Pattern '%s' found in: %s" % (regex.pattern, desc)
            )
    ...
    def test_det_user_modpath(self):
        ...
        if self.MODULE_GENERATOR_CLASS == ModuleGeneratorTcl:
            self.assertEqual(
                self.modgen.det_user_modpath("my/own/modules"), '"my/own/modules" "all"'
            )
        else:
            self.assertEqual(
                self.modgen.det_user_modpath("my/own/modules"),
                '"my/own/modules", "all"',
            )
        ...
        if self.MODULE_GENERATOR_CLASS == ModuleGeneratorTcl:
            self.assertEqual(
                self.modgen.det_user_modpath(user_modpath),
    ...
    def test_use(self):
        ...
        if self.MODULE_GENERATOR_CLASS == ModuleGeneratorTcl:
            ...
            self.assertEqual(
                self.modgen.use(["/some/path"], prefix=quote_str("/foo"), guarded=True),
                expected,
            )
        else:
            # Test regular 'module use' statements
    ...
    def test_getenv_cmd(self):
        ...
        if self.MODULE_GENERATOR_CLASS == ModuleGeneratorTcl:
            ...
            self.assertEqual(
                'os.getenv("HOSTNAME")', self.modgen.getenv_cmd("HOSTNAME")
            )
    ...
    def test_alias(self):
        ...
        if self.MODULE_GENERATOR_CLASS == ModuleGeneratorTcl:
            ...
            self.assertEqual(
                'set-alias\tkey\t\t"va\\"lue"\n', self.modgen.set_alias("key", 'va"lue')
            )
            self.assertEqual(
                'set-alias\tkey\t\t"va\'lue"\n', self.modgen.set_alias("key", "va'lue")
            )
            self.assertEqual(
                'set-alias\tkey\t\t"""va"l\'ue"""\n',
    ...
    def test_conditional_statement(self):
        ...
        if self.MODULE_GENERATOR_CLASS == ModuleGeneratorTcl:
            ...
            expected = "\n".join(
                ["if { [ is-loaded foo ] } {", "    module load bar", "}", "",]
            )
            ...
            if_else_cond = self.modgen.conditional_statement(
                cond, load, else_body='puts "foo"'
            )
            expected = "\n".join(
                [
                    "if { [ is-loaded foo ] } {",
            ...
            if_else_cond = self.modgen.conditional_statement(
                cond, load, else_body='puts "foo"', indent=False
            )
            expected = "\n".join(
                [
                    "if { [ is-loaded foo ] } {",
            ...
            simple_cond = self.modgen.conditional_statement(cond, load)
            expected = "\n".join(
                ['if isloaded("foo") then', '    load("bar")', "end", "",]
            )
            self.assertEqual(simple_cond, expected)

            neg_cond = self.modgen.conditional_statement(cond, load, negative=True)
            expected = "\n".join(
                ['if not ( isloaded("foo") ) then', '    load("bar")', "end", "",]
            )
            self.assertEqual(neg_cond, expected)

            if_else_cond = self.modgen.conditional_statement(
                cond, load, else_body='load("bleh")'
            )
            expected = "\n".join(
                [
                    'if isloaded("foo") then',
            ...
            if_else_cond = self.modgen.conditional_statement(
                cond, load, else_body='load("bleh")', indent=False
            )
            expected = "\n".join(
                [
                    'if isloaded("foo") then',
    ...
    def test_module_naming_scheme(self):
        ...
        ec_files = [
            ...
            for (subdir, _, files) in os.walk(ecs_dir)
            for fil in files
        ]
        ...
        non_parsed = {
            ...
            "version": "1.2.3",
            "versionsuffix": "-bar",
            "toolchain": {"name": "t00ls", "version": "6.6.6",},
        }
        self.assertEqual(
            "foo/1.2.3-t00ls-6.6.6-bar", ActiveMNS().det_full_module_name(non_parsed)
        )

        # make sure test module naming schemes are available
        mns_mods = [
            "broken_module_naming_scheme",
        ...
        ] = "gzip/.65dc39f92bf634667c478c50e43f0cda96b093a9"
        for dep_ec, dep_spec in [
            ...
            self.assertEqual(
                ActiveMNS().det_full_module_name(dep_spec), ec2mod_map[dep_ec]
            )

        ec = EasyConfig(
            os.path.join(ecs_dir, "g", "gzip", "gzip-1.5-foss-2018a.eb"), hidden=True
        )
        self.assertEqual(ec.full_mod_name, ec2mod_map["gzip-1.5-foss-2018a.eb"])
        self.assertEqual(
            ec.toolchain.det_short_module_name(),
            "foss/e69469ac250145c9e814e5dde93f5fde6d80375d",
        )
    ...
    def test_hierarchical_mns(self):
        ...
        def test_ec(
            ...
            self.assertEqual(
                ActiveMNS().det_full_module_name(ec),
            ...
            self.assertEqual(
                ActiveMNS().det_user_modpath_extensions(ec), user_modpath_exts
            )
        ...
        test_ecs = {
            ...
            "foss-2018a.eb": ("foss/2018a", "Core", [], [], ["Core"]),
            "icc-2016.1.150-GCC-4.9.3-2.25.eb": (
                "icc/%s" % iccver,
                "Core",
            ...
            "imkl-2019.4.243-iimpi-2019.08.eb": (
                "imkl/2019.4.243",
                "MPI/intel/2019.4.243/impi/2019.4.243",
                [],
                [],
                ["Core"],
            ),
            "CUDA-9.1.85-GCC-6.4.0-2.28.eb": (
                "CUDA/9.1.85",
                "Compiler/GCC/6.4.0-2.28",
            ...
            "CUDA-5.5.22.eb": ("CUDA/5.5.22", "Core", [], [], ["Core"]),
            "CUDA-5.5.22-iccifort-2016.1.150-GCC-4.9.3-2.25.eb": (
                "CUDA/5.5.22",
                "Compiler/intel/%s" % iccver,
            ...
            "CUDA-10.1.243-iccifort-2019.4.243.eb": (
                "CUDA/10.1.243",
                "Compiler/intel/2019.4.243",
            ...
            "impi-5.1.2.150-iccifortcuda-2016.1.150.eb": (
                "impi/5.1.2.150",
                "Compiler/intel-CUDA/%s-5.5.22" % iccver,
        ...
        self.assertErrorRegex(
            EasyBuildError,
            "No compiler available.*MPI lib",
            ActiveMNS().det_modpath_extensions,
            ec,
        )
        ...
        test_ecs = {
            ...
            impi_ec: (
                "impi/5.1.2.150",
                "Compiler/intel/%s/mpi" % iccver,
                ["MPI/intel/%s/impi/5.1.2.150/%s" % (iccver, c) for c in moduleclasses],
                ["MPI/intel/%s/impi/5.1.2.150" % iccver],
            ),
            imkl_ec: (
                "imkl/11.3.1.150",
                "MPI/intel/%s/impi/5.1.2.150/numlib" % iccver,
                [],
                [],
            ),
        }
        for ecfile, mns_vals in test_ecs.items():
            test_ec(
                ecfile, *mns_vals, init_modpaths=["Core/%s" % c for c in moduleclasses]
            )
        ...
        test_ecs = {
            ...
            "OpenMPI-2.1.2-GCC-6.4.0-2.28.eb": (
                "mpi/OpenMPI/2.1.2-GCC-6.4.0-2.28",
                "",
                [],
                [],
                [],
    ...
    def test_dependencies_for(self):
        ...
        self.assertEqual(
            dependencies_for("foss/2018a", self.modtool, depth=0), expected[:-2]
        )
    ...
    def test_det_installdir(self):
        ...
        test_modfile = os.path.join(
            self.test_prefix, "test" + self.modgen.MODULE_FILE_EXTENSION
        )
...
def suite():
    ...
    suite = TestSuite()
    suite.addTests(
        TestLoaderFiltered().loadTestsFromTestCase(TclModuleGeneratorTest, sys.argv[1:])
    )
    suite.addTests(
        TestLoaderFiltered().loadTestsFromTestCase(LuaModuleGeneratorTest, sys.argv[1:])
    )
    return suite
