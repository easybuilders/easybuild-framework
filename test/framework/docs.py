...
class DocsTest(EnhancedTestCase):
    def test_gen_easyblocks(self):
        ...
        eb_overview = gen_easyblocks_overview_rst(
            gen_easyblocks_pkg, "easyconfigs", common_params, doc_functions
        )
        ebdoc = "\n".join(eb_overview)
        ...
        self.assertTrue(
            check_configuremake in ebdoc,
            "Found '%s' in: %s" % (check_configuremake, ebdoc),
        )
        names = []
        ...
        self.assertTrue(
            re.search(regex, ebdoc), "Pattern %s found in %s" % (regex.pattern, ebdoc)
        )
    ...
    def test_license_docs(self):
        ...
        self.assertTrue(
            regex.search(lic_docs), "%s found in: %s" % (regex.pattern, lic_docs)
        )
    ...
    def test_list_software(self):
        ...
        self.assertTrue(
            expected.match(txt), "Pattern '%s' found in: %s" % (expected.pattern, txt)
        )
