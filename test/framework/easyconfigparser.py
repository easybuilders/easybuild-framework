...
class EasyConfigParserTest(EnhancedTestCase):
    ...
    def test_raw(self):
        ...
        ec_file2 = os.path.join(
            TESTDIRBASE, "v1.0", "g", "gzip", "gzip-1.5-foss-2018a.eb"
        )
        ...
        self.assertErrorRegex(
            EasyBuildError, "Neither filename nor rawcontent provided", EasyConfigParser
        )
    ...
    def test_easyconfig_constants(self):
        ...
        for constant_name in constants:
            self.assertTrue(
                isinstance(constant_name, string_type),
                "Constant name %s is a string" % constant_name,
            )
            val = constants[constant_name]
            self.assertTrue(
                isinstance(val, (string_type, dict)),
                "Constant value %s is a string or dict" % val,
            )
    ...
    def test_check_value_types(self):
        ...
        test_ec = os.path.join(
            TESTDIRBASE, "test_ecs", "g", "gzip", "gzip-1.4-broken.eb"
        )
        error_msg_pattern = (
...
def suite():
    ...
    return TestLoaderFiltered().loadTestsFromTestCase(
        EasyConfigParserTest, sys.argv[1:]
    )
