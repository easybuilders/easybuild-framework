...
class ModulesTest(EnhancedTestCase):
    ...
    def test_long_module_path(self):
        ...
        gcc_mod_path = os.path.join(
            os.path.dirname(__file__), "modules", "GCC", "4.6.3"
        )
    ...
    def test_avail(self):
        ...
        if not isinstance(self.modtool, EnvironmentModulesTcl) or isinstance(
            self.modtool, EnvironmentModules
        ):
        ...
        if isinstance(self.modtool, Lmod) and StrictVersion(
            self.modtool.version
        ) >= StrictVersion("5.7.5"):
    ...
    def test_exist(self):
        ...
        self.assertEqual(
            self.modtool.exist(["OpenMPI/2.1.2-GCC-6.4.0-2.28"], skip_avail=True),
            [True],
        )
        ...
        self.assertEqual(
            self.modtool.exist(["OpenMPI/2.1.2"], skip_avail=True), [False]
        )
        ...
        self.assertEqual(
            self.modtool.exist(["OpenMPI"], maybe_partial=False, skip_avail=True),
            [False],
        )

        # exist works on hidden modules in Lua syntax (only with Lmod)
        test_modules_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "modules")
        )
        if isinstance(self.modtool, Lmod):
            # make sure only the .lua module file is there, otherwise this test doesn't work as intended
            self.assertTrue(
                os.path.exists(os.path.join(test_modules_path, "bzip2", ".1.0.6.lua"))
            )
            self.assertFalse(
                os.path.exists(os.path.join(test_modules_path, "bzip2", ".1.0.6"))
            )
        ...
        self.assertEqual(
            self.modtool.exist(mod_names), [True, False, True, False, True, True]
        )
        self.assertEqual(
            self.modtool.exist(mod_names, skip_avail=True),
        ...
        if self.modtool.__class__ == EnvironmentModulesC:
            ...
            modulerc_tcl_txt = "\n".join(
                ["#%Module", "module-version Java/1.8.0_181 1.8",]
            )
        ...
        self.assertEqual(
            self.modtool.exist(["Java/1.8", "Java/1.8.0_181"]), [True, True]
        )
        self.assertEqual(
            self.modtool.module_wrapper_exists("Java/1.8"), "Java/1.8.0_181"
        )
        ...
        self.assertEqual(
            self.modtool.module_wrapper_exists("Core/Java/1.8"), "Core/Java/1.8.0_181"
        )
        ...
        if isinstance(self.modtool, Lmod) and StrictVersion(
            ...
            write_file(
                os.path.join(java_mod_dir, ".modulerc.lua"),
            ...
            self.assertEqual(self.modtool.exist(["Core/Java/1.8"]), [True])
            self.assertEqual(
                self.modtool.module_wrapper_exists("Core/Java/1.8"),
                "Core/Java/1.8.0_181",
            )
    ...
    def test_load(self):
        ...
        ms = [
            ...
            if not (
                m.startswith("Core")
                or m.startswith("Compiler/")
                or m.startswith("MPI/")
                or m.startswith("CategorizedHMNS")
            )
        ]
        ...
        if not isinstance(
            self.modtool, EnvironmentModules
        ) or modtool_ver >= StrictVersion("4.2"):
        ...
        if isinstance(self.modtool, Lmod):
            # order of loaded modules only changes with Lmod
            self.assertTrue(
                self.modtool.loaded_modules()[-1] == "OpenMPI/2.1.2-GCC-6.4.0-2.28"
            )
    ...
    def test_check_module_path(self):
        ...
        self.assertEqual(
            os.environ["MODULEPATH"], mod_install_dir + ":" + test1 + ":" + test2
        )
        ...
        self.assertEqual(
            os.environ["MODULEPATH"],
            ":".join([mod_install_dir, test1, test3, "/does/not/exist", test2]),
        )
    ...
    def test_check_module_path_hmns(self):
        ...
        self.assertEqual(
            modtool.mod_paths, [os.path.dirname(core_mod_dir), core_mod_dir]
        )
        self.assertEqual(
            os.environ["MODULEPATH"],
            "%s:%s:%s" % (top_mod_dir, core_mod_dir, doesnotexist),
        )
    ...
    def test_get_software_root_version_libdir(self):
        ...
        self.assertErrorRegex(
            EasyBuildError,
            "Multiple library subdirectories found.*",
            get_software_libdir,
            name,
        )
    ...
    def test_wrong_modulepath(self):
        ...
        test_modules_path = os.path.realpath(
        ...
        os.environ["MODULEPATH"] = (
            "/some/non-existing/path:/this/doesnt/exists/anywhere:%s"
            % test_modules_path
        )
        init_config()
        ...
        self.assertTrue(
            os.path.samefile(modtool.mod_paths[0], modules_test_installpath)
        )
    ...
    def test_modulefile_path(self):
        ...
        if isinstance(self.modtool, Lmod):
            ...
            self.assertTrue(
                os.path.samefile(
                    res, os.path.join(test_dir, "modules", "bzip2", ".1.0.6.lua")
                )
            )
        ...
        mod.MODULE_SHOW_CACHE[mod_show_cache_key] = "\n".join(
            [
                "import os",
                "os.environ['MODULEPATH_modshare'] = '%s'"
                % ":".join(m + ":1" for m in modulepath),
        ...
        self.assertTrue(
            os.path.samefile(
                res, os.path.join(test_dir, "modules", "GCC", "6.4.0-2.28")
            )
        )
    ...
    def test_path_to_top_of_module_tree(self):
        ...
        path = self.modtool.path_to_top_of_module_tree(
            [], "toy/.0.0-deps", "", ["gompi/2018a"]
        )
        self.assertEqual(path, [])
    ...
    def test_path_to_top_of_module_tree_hierarchical_mns(self):
        ...
        path = self.modtool.path_to_top_of_module_tree(
            init_modpaths, "foss/2018a", core, deps
        )
        self.assertEqual(path, [])
        path = self.modtool.path_to_top_of_module_tree(
            init_modpaths, "GCC/6.4.0-2.28", core, []
        )
        self.assertEqual(path, [])
        ...
        path = self.modtool.path_to_top_of_module_tree(
            init_modpaths, "OpenMPI/2.1.2", full_mod_subdir, deps
        )
        self.assertEqual(path, ["GCC/6.4.0-2.28"])

        self.modtool.load(["gompi/2018a"])
        full_mod_subdir = os.path.join(
            mod_prefix, "MPI", "GCC", "6.4.0-2.28", "OpenMPI", "2.1.2"
        )
        deps = ["GCC/6.4.0-2.28", "OpenMPI/2.1.2"]
        path = self.modtool.path_to_top_of_module_tree(
            init_modpaths, "FFTW/3.3.7", full_mod_subdir, deps
        )
    ...
    def test_path_to_top_of_module_tree_lua(self):
        ...
        if isinstance(self.modtool, Lmod):
            ...
            error_msg = "Incorrect $MODULEPATH value after unuse: %s (orig: %s)" % (
                curr_modulepath,
                orig_modulepath,
            )
            ...
            impi_mod_dir = os.path.join(
                top_moddir, "MPI", "intel", "2016", "impi", "2016"
            )
            ...
            res = self.modtool.path_to_top_of_module_tree(
                core_dir, "imkl/2016", impi_mod_dir, imkl_deps
            )
            self.assertEqual(res, ["impi/2016", "intel/2016"])

        else:
            print(
                "Skipping test_path_to_top_of_module_tree_lua, requires Lmod as modules tool"
            )
    ...
    def test_interpret_raw_path_lua(self):
        ...
        self.assertEqual(
            self.modtool.interpret_raw_path_lua('"just/a/path"'), "just/a/path"
        )

        os.environ["TEST_VAR"] = "test123"
        self.assertEqual(
            self.modtool.interpret_raw_path_lua('os.getenv("TEST_VAR")'), "test123"
        )
        self.assertEqual(
            self.modtool.interpret_raw_path_lua(
                'os.getenv("NO_SUCH_ENVIRONMENT_VARIABLE")'
            ),
            "",
        )
        ...
        self.assertEqual(
            self.modtool.interpret_raw_path_lua(lua_str), "foo/test123/bar/test123"
        )
    ...
    def test_interpret_raw_path_tcl(self):
        ...
        self.assertEqual(
            self.modtool.interpret_raw_path_tcl('"just/a/path"'), "just/a/path"
        )

        os.environ["TEST_VAR"] = "test123"
        self.assertEqual(
            self.modtool.interpret_raw_path_tcl("$env(TEST_VAR)"), "test123"
        )
        self.assertEqual(
            self.modtool.interpret_raw_path_tcl("$env(NO_SUCH_ENVIRONMENT_VARIABLE)"),
            "",
        )

        self.assertEqual(
            self.modtool.interpret_raw_path_tcl("$env(TEST_VAR)/bar"), "test123/bar"
        )

        tcl_str = "foo/$env(TEST_VAR)/bar/$env(TEST_VAR)"
        self.assertEqual(
            self.modtool.interpret_raw_path_tcl(tcl_str), "foo/test123/bar/test123"
        )

        tcl_str = '[ file join $env(TEST_VAR) "foo/bar" ]'
        self.assertEqual(
            self.modtool.interpret_raw_path_tcl(tcl_str), "test123/foo/bar"
        )
    ...
    def test_modpath_extensions_for(self):
        ...
        res = self.modtool.modpath_extensions_for(
            ["GCC/6.4.0-2.28", "OpenMPI/2.1.2", "FFTW/3.3.7"]
        )
        ...
        expected = {
            "icc/2016.1.150-GCC-4.9.3-2.25": [
                os.path.join(intel_mod_dir, "2016.1.150-GCC-4.9.3-2.25")
            ],
        ...
        res = self.modtool.modpath_extensions_for(
            ["icc/2016.1.150-GCC-4.9.3-2.25", "ifort/2016.1.150-GCC-4.9.3-2.25"]
        )
        self.assertEqual(res, expected)
        ...
        self.assertErrorRegex(
            EasyBuildError,
            error_pattern,
            self.modtool.modpath_extensions_for,
            ["nosuchmodule/1.2"],
        )
        ...
        test_modtxt = "\n".join(
            [
                ...
                "    module use %s/Compiler/intel/2016.1.150-GCC-4.9.3-2.25"
                % mod_dir,  # indented without guard
                # quoted path
                'module use "%s/Compiler/GCC/6.4.0-2.28"' % mod_dir,
                # using prepend-path & quoted
                ' prepend-path MODULEPATH [ file join %s "MPI/GCC/6.4.0-2.28/OpenMPI/2.1.2" ]'
                % mod_dir,
        ...
        expected = {
            test_mod: [
                ...
                os.path.join(mod_dir, "MPI", "GCC", "6.4.0-2.28", "OpenMPI", "2.1.2"),
                os.path.join(
                    os.environ["HOME"], "modules", "Compiler", "GCC", "6.4.0-2.28"
                ),
                os.environ["HOME"],
        ...
        if isinstance(self.modtool, Lmod):
            ...
            test_modtxt = "\n".join(
                [
                    ...
                    '   prepend_path("MODULEPATH", "%s/Compiler/intel/2016.1.150-GCC-4.9.3-2.25")'
                    % mod_dir,
                    'prepend_path("MODULEPATH","%s/Compiler/GCC/6.4.0-2.28")' % mod_dir,
                    'prepend_path("MODULEPATH", "%s/MPI/GCC/6.4.0-2.28/OpenMPI/2.1.2")'
                    % mod_dir,
                    # conditional 'use' on subdirectory in $HOME, e.g. when --subdir-user-modules is used
                    ...
                    'prepend_path("MODULEPATH", pathJoin("%s", os.getenv("USER"), "test"))'
                    % self.test_prefix,
                ]
            )
            write_file(test_modfile, test_modtxt)
    ...
    def test_path_to_top_of_module_tree_categorized_hmns(self):
        ...
        mod_prefix = os.path.join(self.test_installpath, "modules", "all")
        init_modpaths = [
            os.path.join(mod_prefix, "Core", "compiler"),
        ...
        path = self.modtool.path_to_top_of_module_tree(
            init_modpaths, "foss/2018a", tc, deps
        )
        self.assertEqual(path, [])
        comp = os.path.join(core, "compiler")
        path = self.modtool.path_to_top_of_module_tree(
            init_modpaths, "GCC/6.4.0-2.28", comp, []
        )
        self.assertEqual(path, [])
        ...
        self.modtool.load(["GCC/6.4.0-2.28"])
        full_mod_subdir = os.path.join(
            mod_prefix, "Compiler", "GCC", "6.4.0-2.28", "mpi"
        )
        deps = ["GCC/6.4.0-2.28", "hwloc/1.11.8"]
        path = self.modtool.path_to_top_of_module_tree(
            init_modpaths, "OpenMPI/2.1.2", full_mod_subdir, deps
        ...
        self.modtool.load(["gompi/2018a"])
        full_mod_subdir = os.path.join(
            mod_prefix, "MPI", "GCC", "6.4.0-2.28", "OpenMPI", "2.1.2", "numlib"
        )
        deps = ["GCC/6.4.0-2.28", "OpenMPI/2.1.2"]
        path = self.modtool.path_to_top_of_module_tree(
            init_modpaths, "FFTW/3.3.7", full_mod_subdir, deps
    ...
    def test_modules_tool_stateless(self):
        ...
        copy_dir(
            os.path.join(test_modules_path, "Core"),
            os.path.join(self.test_prefix, "Core"),
        )
        copy_dir(
            os.path.join(test_modules_path, "Compiler"),
            os.path.join(self.test_prefix, "Compiler"),
        )

        modtxt = read_file(os.path.join(self.test_prefix, "Core", "GCC", "6.4.0-2.28"))
        modpath_extension = os.path.join(
            self.test_prefix, "Compiler", "GCC", "6.4.0-2.28"
        )
        modtxt = re.sub(
            "module use .*", "module use %s" % modpath_extension, modtxt, re.M
        )
        write_file(os.path.join(self.test_prefix, "Core", "GCC", "6.4.0-2.28"), modtxt)

        modtxt = read_file(
            os.path.join(
                self.test_prefix, "Compiler", "GCC", "6.4.0-2.28", "OpenMPI", "2.1.2"
            )
        )
        modpath_extension = os.path.join(
            self.test_prefix, "MPI", "GCC", "6.4.0-2.28", "OpenMPI", "2.1.2"
        )
        ...
        self.assertErrorRegex(
            EasyBuildError, load_err_msg, self.modtool.load, ["GCC/4.6.3"]
        )
        ...
        self.assertErrorRegex(
            EasyBuildError, load_err_msg, self.modtool.load, ["OpenMPI/2.1.2"]
        )
    ...
    def test_mk_module_cache_key(self):
        ...
        self.assertEqual(
            res,
            (
                "MODULEPATH=%s:/tmp/test" % self.test_prefix,
                self.modtool.COMMAND,
                "thisisapartialkey",
            ),
        )
        ...
        res = self.modtool.mk_module_cache_key("thisisapartialkey")
        self.assertEqual(
            res, ("MODULEPATH=", self.modtool.COMMAND, "thisisapartialkey")
        )
    ...
    def test_module_caches(self):
        ...
        modulepaths = [
            p for p in os.environ.get("MODULEPATH", "").split(os.pathsep) if p
        ]
        ...
        self.assertTrue(
            any(
                [
                    os.path.exists(mp) and os.path.samefile(test_mods_path, mp)
                    for mp in modulepaths
                ]
            )
        )
        paths_in_key = [
            p for p in avail_cache_key[0].split("=")[1].split(os.pathsep) if p
        ]
        self.assertTrue(
            any(
                [
                    os.path.exists(p) and os.path.samefile(test_mods_path, p)
                    for p in paths_in_key
                ]
            )
        )
    ...
    def test_module_use(self):
        ...
        if isinstance(self.modtool, Lmod):
            ...
            self.assertTrue(
                os.environ["MODULEPATH"].startswith("%s:%s:" % (test_dir2, test_dir3))
            )
    ...
    def test_load_in_hierarchy(self):
        ...
        self.modtool.load(["hwloc/1.11.8"])
        self.assertEqual(
            os.environ["EBROOTHWLOC"],
            "/tmp/software/Compiler/GCC/6.4.0-2.28/hwloc/1.11.8",
        )
    ...
    def test_exit_code_check(self):
        ...
        if isinstance(self.modtool, Lmod):
            ...
            error_pattern = (
                "Unable to locate a modulefile for 'nosuchmoduleavailableanywhere'"
            )
        self.assertErrorRegex(
            EasyBuildError,
            error_pattern,
            self.modtool.load,
            ["nosuchmoduleavailableanywhere"],
        )
    ...
    def test_check_loaded_modules(self):
        ...
        for pattern in patterns:
            self.assertTrue(
                re.search(pattern, stderr, re.M),
                "Pattern '%s' found in: %s" % (pattern, stderr),
            )

        # reconfigure EasyBuild to ignore loaded modules for GCC & hwloc & error out when loaded modules are detected
        options = init_config(
            args=["--allow-loaded-modules=GCC,hwloc", "--detect-loaded-modules=error"]
        )
        build_options = {
        ...
        self.assertErrorRegex(
            EasyBuildError, error_pattern, self.modtool.check_loaded_modules
        )
        ...
        ] = "/path/to/software/without/a/matching/module"
        stderr = check_loaded_modules()
        ...
        self.assertErrorRegex(
            ...
            error_msg,
            init_config,
            args=["--detect-loaded-modules=sdvbfdgh"],
        )
    ...
    def test_NoModulesTool(self):
        ...
        self.assertEqual(
            nmt.exist(["foo", "bar"], r"^\s*\S*/%s.*:\s*$", skip_avail=False),
            [False, False],
        )
