...
class FileToolsTest(EnhancedTestCase):
    ...
    def test_decode_class_name(self):
        ...
        for (class_name, encoded_class_name) in self.class_names:
            ...
            self.assertEqual(
                ft.decode_class_name(ft.encode_class_name(class_name)), class_name
            )
    ...
    def test_which(self):
        ...
        path = ft.which(
            "i_really_do_not_expect_a_command_with_a_name_like_this_to_be_available"
        )
    ...
    def test_checksums(self):
        ...
        self.assertFalse(
            ft.verify_checksum(
                fp, "7167b64b1ca062b9674ffef46f9325db7167b64b1ca062b9674ffef46f9325db"
            )
        )
        ...
        for checksum in [
            "tooshort",
            "inbetween32and64charactersisnotgoodeither",
            known_checksums["sha256"] + "foo",
        ]:
            self.assertErrorRegex(
                EasyBuildError, error_pattern, ft.verify_checksum, fp, checksum
            )

        # make sure faulty checksums are reported
        broken_checksums = dict(
            [(typ, val[:-3] + "foo") for (typ, val) in known_checksums.items()]
        )
        ...
        alt_checksums = (
            "7167b64b1ca062b9674ffef46f9325db7167b64b1ca062b9674ffef46f9325db",
            known_checksums["sha256"],
        )
        self.assertTrue(ft.verify_checksum(fp, alt_checksums))

        alt_checksums = (
            "fecf50db81148786647312bbd3b5c740",
            "2c829facaba19c0fcd81f9ce96bef712",
            "840078aeb4b5d69506e7c8edae1e1b89",
            known_checksums["md5"],
        )
        self.assertTrue(ft.verify_checksum(fp, alt_checksums))
        ...
        self.assertErrorRegex(
            EasyBuildError, "Missing checksum for", ft.verify_checksum, fp, None
        )
        self.assertTrue(ft.verify_checksum(fp, known_checksums["md5"]))
        self.assertTrue(ft.verify_checksum(fp, known_checksums["sha256"]))
    ...
    def test_common_path_prefix(self):
        ...
        self.assertEqual(
            ft.det_common_path_prefix(["/foo/bar/foo", "/foo/bar/baz", "/foo/bar/bar"]),
            "/foo/bar",
        )
        self.assertEqual(
            ft.det_common_path_prefix(["/foo/bar/", "/foo/bar/baz", "/foo/bar"]),
            "/foo/bar",
        )
    ...
    def test_download_file(self):
        ...
        self.assertEqual(
            ft.download_file(fn, "file://%s/nosuchfile" % test_dir, target_location),
            None,
        )
        ...
        proxy_handler = std_urllib.ProxyHandler(
        ...
        self.assertEqual(
            ft.download_file(fn, source_url, target_location),
            None,
        ...
        self.assertEqual(
            ft.compute_checksum(target_location),
            ft.compute_checksum(os.path.join(toy_source_dir, fn)),
        )
        ...
        try:
            ...
            self.assertEqual(
                res, target_location, "download with specified timeout works"
            )
    ...
    def test_download_file_requests_fallback(self):
        ...
        if ft.HAVE_REQUESTS:
            ...
            self.assertTrue(
                "https://easybuilders.github.io/easybuild" in ft.read_file(res)
            )

        # without requests being available, error is raised
        ft.HAVE_REQUESTS = False
        self.assertErrorRegex(
            EasyBuildError, "SSL issues with urllib2", ft.download_file, fn, url, target
        )
    ...
    def test_mkdir(self):
        ...
        self.assertFalse(
            os.stat(foodir).st_mode & (stat.S_ISGID | stat.S_ISVTX),
            "no gid/sticky bit %s" % foodir,
        )
        self.assertFalse(
            os.stat(barfoodir).st_mode & (stat.S_ISGID | stat.S_ISVTX),
            "no gid/sticky bit %s" % barfoodir,
        )
        # setting group ID bit works
        giddir = os.path.join(foodir, "gid")
        check_mkdir(giddir, set_gid=True)
        self.assertTrue(
            os.stat(giddir).st_mode & stat.S_ISGID, "gid bit set %s" % giddir
        )
        self.assertFalse(
            os.stat(giddir).st_mode & stat.S_ISVTX, "no sticky bit %s" % giddir
        )
        # setting stciky bit works
        stickydir = os.path.join(barfoodir, "sticky")
        check_mkdir(stickydir, sticky=True)
        self.assertFalse(
            os.stat(stickydir).st_mode & stat.S_ISGID, "no gid bit %s" % stickydir
        )
        self.assertTrue(
            os.stat(stickydir).st_mode & stat.S_ISVTX, "sticky bit set %s" % stickydir
        )
    ...
    def test_symlink_resolve_path(self):
        ...
        self.assertTrue(
            os.path.samefile(os.path.join(self.test_prefix, "test", "test.txt"), link)
        )

        # test resolve_path
        self.assertEqual(test_dir, ft.resolve_path(link_dir))
        self.assertEqual(
            os.path.join(os.path.realpath(self.test_prefix), "test", "test.txt"),
            ft.resolve_path(link),
        )
        self.assertEqual(ft.read_file(link), "test123")
        self.assertErrorRegex(
            EasyBuildError, "Resolving path .* failed", ft.resolve_path, None
        )
    ...
    def test_read_write_file(self):
        ...
        backup2 = [
            x for x in test_files if x != backup1 and os.path.basename(x) != "test.txt"
        ][0]
        self.assertEqual(ft.read_file(backup1), txt + txt2)
        ...
        self.mock_stdout(False)
        regex = re.compile(
            "^== Backup of .*/test.txt created at .*/test.txt.bak_[0-9]*"
        )
        self.assertTrue(
        ...
        self.assertErrorRegex(
            EasyBuildError, error, ft.write_file, fp, "blah", always_overwrite=False
        )
        self.assertErrorRegex(
            EasyBuildError,
            error,
        ...
        ft.write_file(
            fp, "overwrittenbyforcewithbackup", always_overwrite=False, backup=True
        )
    ...
    def test_det_patched_files(self):
        ...
        pf = os.path.join(
            os.path.dirname(__file__), "sandbox", "sources", "toy", toy_patch_fn
        )
        self.assertEqual(ft.det_patched_files(pf), ["b/toy-0.0/toy.source"])
        self.assertEqual(
            ft.det_patched_files(pf, omit_ab_prefix=True), ["toy-0.0/toy.source"]
        )
    ...
    def test_guess_patch_level(self):
        ...
        for patched_file, correct_patch_level in [
            ...
            self.assertEqual(
                ft.guess_patch_level([patched_file], self.test_buildpath),
                correct_patch_level,
            )
    ...
    def test_back_up_file(self):
        ...
        self.assertTrue(
            os.path.samefile(res, os.path.join(self.test_prefix, "sandbox", new_file))
        )
        ...
        self.assertEqual(
            ft.read_file(os.path.join(os.path.dirname(fp), new_file)), new_txt
        )
    ...
    def test_move_logs(self):
        ...
        self.assertEqual(
            ft.read_file(os.path.join(self.test_prefix, "foo.log.1")), "moarfoobar"
        )
        ...
        self.assertEqual(
            len(logs), 7, "Found exactly 7 log files: %d (%s)" % (len(logs), logs)
        )
        ...
        other_bar = [
            x
            for x in logs
            if x.startswith("bar") and x not in ["bar.log", "bar.log.1", "bar.log_1"]
        ][0]
    ...
    def test_multidiff(self):
        ...
        expected = (
            "8 %(red)s- toolchain = {'name': 'gompi', 'version': '2018a'}%(endcol)s"
        )
        expected = expected % {"endcol": endcol, "green": green, "red": red}
        ...
        self.assertTrue(
            any(l.startswith(expected) for l in lines),
        ...
        self.assertEqual(
            lines[0],
            "Comparing toy-0.0.eb with toy-0.0-deps.eb, toy-0.0-gompi-2018a-test.eb",
        )
        self.assertEqual(lines[1], "=====")

        # different versionsuffix
        self.assertTrue(
            lines[2].startswith("3 - versionsuffix = '-deps' (1/2) toy-0.0-")
        )
        self.assertTrue(
            lines[3].startswith("3 - versionsuffix = '-test' (1/2) toy-0.0-")
        )
    ...
    def test_weld_paths(self):
        ...
        self.assertEqual(
            ft.weld_paths("/foo/bar", "foobar/baz"), "/foo/bar/foobar/baz/"
        )
    ...
    def test_expand_glob_paths(self):
        ...
        self.assertEqual(
            ft.expand_glob_paths(["~/*.txt"]), [os.path.join(new_home, "test.txt")]
        )
    ...
    def test_adjust_permissions(self):
        ...
        self.assertErrorRegex(
            EasyBuildError, err_msg, ft.adjust_permissions, nosuchdir, stat.S_IWOTH
        )
    ...
    def test_apply_regex_substitutions(self):
        ...
        expected_testtxt = "\n".join(
            ["CC = ${CC}", "CFLAGS = -O2", "FC = ${FC}", "FFLAGS = -O2 -ffixed-form",]
        )
        ...
        self.assertErrorRegex(
            EasyBuildError, error_pat, ft.apply_regex_substitutions, path, regex_subs
        )
    ...
    def test_find_flexlm_license(self):
        ...
        self.assertEqual(
            ft.find_flexlm_license(lic_specs=[lic_file1]), ([lic_file1], None)
        )
        self.assertEqual(
        ...
        os.environ[
            "LM_LICENSE_FILE"
        ] = "/no/such/file/unless/you/aim/to/break/this/check"
        self.assertEqual(ft.find_flexlm_license(), ([], None))
        ...
        self.assertEqual(
            ft.find_flexlm_license(), ([lic_file1, lic_file2], "LM_LICENSE_FILE")
        )
        ...
        os.environ["LM_LICENSE_FILE"] = lic_file2
        self.assertEqual(
            ft.find_flexlm_license(lic_specs=[lic_server]),
        ...
        self.assertEqual(
            ft.find_flexlm_license(custom_env_vars="INTEL_LICENSE_FILE"), expected
        )
        self.assertEqual(
            ft.find_flexlm_license(custom_env_vars=["INTEL_LICENSE_FILE"]), expected
        )
        ...
        os.environ["INTEL_LICENSE_FILE"] = (
            "/not/a/valid/license/path:%s:/another/bogus/license/file" % lic_file2
        )
        ...
        os.environ[
            "INTEL_LICENSE_FILE"
        ] = "1234@lic1.test:4567@lic2.test:7890@lic3.test"
        expected = (
            ["1234@lic1.test", "4567@lic2.test", "7890@lic3.test"],
            "INTEL_LICENSE_FILE",
        )
    ...
    def test_is_patch_file(self):
        ...
        self.assertFalse(
            ft.is_patch_file(
                os.path.join(
                    testdir, "easyconfigs", "test_ecs", "t", "toy", "toy-0.0.eb"
                )
            )
        )
        toy_patch_fn = "toy-0.0_fix-silly-typo-in-printf-statement.patch"
        self.assertTrue(
            ft.is_patch_file(
                os.path.join(testdir, "sandbox", "sources", "toy", toy_patch_fn)
            )
        )
    ...
    def test_is_alt_pypi_url(self):
        ...
        url = (
            "https://pypi.python.org/packages/source/e/easybuild/easybuild-2.7.0.tar.gz"
        )
        self.assertFalse(ft.is_alt_pypi_url(url))

        url = url.replace(
            "source/e/easybuild",
            "5b/03/e135b19fadeb9b1ccb45eac9f60ca2dc3afe72d099f6bd84e03cb131f9bf",
        )
    ...
    def test_pypi_source_urls(self):
        ...
        eb340_url += (
            "93/41/574d01f352671fbc8589a436167e15a7f3e27ac0aa635d208eb29ee8fd4e/"
        )
        eb340_url += "easybuild-3.4.0.tar.gz#md5=267a056a77a8f77fccfbf56354364045"
        self.assertTrue(eb340_url, res)
        pattern = (
            "^https://pypi.python.org/packages/[a-f0-9]{2}/[a-f0-9]{2}/[a-f0-9]{60}/"
        )
        ...
        for url in res:
            error_msg = "Pattern '%s' or '%s' matches for '%s'" % (
                regex_md5.pattern,
                regex_sha256.pattern,
                url,
            )
    ...
    def test_derive_alt_pypi_url(self):
        ...
        alturl = url.replace(
            "source/t/tornado",
            "df/42/a180ee540e12e2ec1007ac82a42b09dd92e5461e09c98bf465e98646d187",
        )
        self.assertEqual(ft.derive_alt_pypi_url(url), alturl)

        # no crash on non-existing version
        url = (
            "https://pypi.python.org/packages/source/e/easybuild/easybuild-0.0.0.tar.gz"
        )
    ...
    def test_apply_patch(self):
        ...
        path = ft.extract_file(
            os.path.join(testdir, "sandbox", "sources", "toy", "toy-0.0.tar.gz"), tmpdir
        )
        toy_patch_fn = "toy-0.0_fix-silly-typo-in-printf-statement.patch"
        ...
        toy_patch_gz = os.path.join(
            testdir, "sandbox", "sources", "toy", "toy-0.0_gzip.patch.gz"
        )
        ...
        self.assertErrorRegex(
            EasyBuildError, "Couldn't apply patch file", ft.apply_patch, toy_patch, path
        )

    def test_copy_file(self):
        """Test copy_file function."""
        testdir = os.path.dirname(os.path.abspath(__file__))
        to_copy = os.path.join(
            testdir, "easyconfigs", "test_ecs", "t", "toy", "toy-0.0.eb"
        )
        ...
        self.assertErrorRegex(
            EasyBuildError,
            "Failed to copy file.*Is a directory",
            ft.copy_file,
            src,
            target,
        )
        ...
        test_file_to_overwrite = os.path.join(
            "/tmp", "file_to_overwrite_for_easybuild_test_copy_file.txt"
        )
        if os.path.exists(test_file_to_overwrite):
            ...
            print(
                "Skipping overwrite-file-owned-by-other-user copy_file test (%s is missing)",
                test_file_to_overwrite,
            )
    ...
    def test_copy_files(self):
        ...
        self.assertErrorRegex(
            EasyBuildError, error_pattern, ft.copy_files, [bzip2_ec], copied_toy_ec
        )
    ...
    def test_copy_dir(self):
        ...
        src, target = (
            os.path.join(to_copy, "GCC-4.6.3.eb"),
            os.path.join(self.test_prefix, "GCC-4.6.3.eb"),
        )
        self.assertErrorRegex(
            EasyBuildError,
            "Failed to copy directory.*Not a directory",
            ft.copy_dir,
            src,
            target,
        )
    ...
    def test_copy(self):
        ...
        self.assertTrue(
            os.path.isfile(os.path.join(self.test_prefix, "foo", "toy-0.0.eb"))
        )
        ...
        self.assertTrue(
            re.search("^copied file .*/GCC-4.6.3.eb to .*/GCC-4.6.3.eb", txt, re.M)
        )
        ...
        self.assertTrue(
            os.path.isfile(os.path.join(self.test_prefix, "toy", "toy-0.0.eb"))
        )
    ...
    def test_change_dir(self):
        ...
        foo = os.path.join(self.test_prefix, "foo")
        self.assertErrorRegex(
            EasyBuildError, "Failed to change from .* to %s" % foo, ft.change_dir, foo
        )

    def test_extract_file(self):
        """Test extract_file"""
        testdir = os.path.dirname(os.path.abspath(__file__))
        toy_tarball = os.path.join(
            testdir, "sandbox", "sources", "toy", "toy-0.0.tar.gz"
        )

        self.assertFalse(
            os.path.exists(os.path.join(self.test_prefix, "toy-0.0", "toy.source"))
        )
    ...
    def test_remove(self):
        ...
        ft.mkdir(test_dir)
        ft.adjust_permissions(
            self.test_prefix, stat.S_IWUSR | stat.S_IWGRP | stat.S_IWOTH, add=False
        )
        self.assertErrorRegex(
            EasyBuildError, "Failed to remove", ft.remove_file, testfile
        )
        self.assertErrorRegex(EasyBuildError, "Failed to remove", ft.remove, testfile)
        self.assertErrorRegex(
            EasyBuildError, "Failed to remove", ft.remove_dir, test_dir
        )
    ...
    def test_search_file(self):
        ...
        var_defs, hits = ft.search_file(
            [test_ecs], "HWLOC", silent=True, filename_only=True
        )
        self.assertEqual(var_defs, [])
        self.assertEqual(
            hits,
        ...
        var_defs, hits = ft.search_file(
            [test_ecs], "HWLOC", silent=True, ignore_dirs=["hwloc"]
        )
        ...
        var_defs, hits = ft.search_file(
            [test_ecs], "HWLOC", terse=True, filename_only=True
        )
        self.assertEqual(var_defs, [])
        self.assertEqual(
            hits,
        ...
        for pattern in ["netCDF-C++", "foo.*bar", "foo|bar"]:
            var_defs, hits = ft.search_file(
                [test_ecs], pattern, terse=True, filename_only=True
            )
            self.assertEqual(var_defs, [])
        ...
        for pattern in ["netCDF-C++", "CDF", "C++", "^netCDF"]:
            var_defs, hits = ft.search_file(
                [self.test_prefix], pattern, terse=True, filename_only=True
            )
            self.assertEqual(var_defs, [])
    ...
    def test_find_eb_script(self):
        ...
        ft.write_file(
            os.path.join(self.test_prefix, "bin", "eb"), '#!/bin/bash\necho "fake eb"'
        )
        ...
        justatest = os.path.join(
            self.test_prefix, "easybuild", "scripts", "justatest.sh"
        )
    ...
    def test_find_backup_name_candidate(self):
        ...
        self.assertTrue(
            regex.match(fn), "'%s' matches pattern '%s'" % (fn, regex.pattern)
        )
    ...
    def test_diff_files(self):
        ...
        bar = os.path.join(self.test_prefix, "bar")
        ft.write_file(bar, "\n".join(["zero", "1", "two", "tree", "four", "five",]))
        expected = "\n".join(
            [
                "@@ -1,5 +1,6 @@",
                "-one",
        ...
        self.assertTrue(
            regex.search(res), "Pattern '%s' found in: %s" % (regex.pattern, res)
        )
    ...
    def test_get_source_tarball_from_git(self):
        ...
        try:
            ...
            self.assertEqual(
                sorted(os.listdir(target_dir)), ["test.tar.gz", "test2.tar.gz"]
            )

        except EasyBuildError as err:
            if "Network is down" in str(err):
                print(
                    "Ignoring download error in test_get_source_tarball_from_git, working offline?"
                )
            else:
                raise err
        ...
        for key in ["repo_name", "url", "tag"]:
            ...
            self.assertErrorRegex(
                EasyBuildError, error_pattern, ft.get_source_tarball_from_git, *args
            )
        ...
        error_pattern = (
            "Found one or more unexpected keys in 'git_config' specification"
        )
        self.assertErrorRegex(
            EasyBuildError, error_pattern, ft.get_source_tarball_from_git, *args
        )
    ...
    def test_is_sha256_checksum(self):
        ...
        for not_a_sha256_checksum in [
            ...
            [
                a_sha256_checksum
            ],  # False for a list of whatever, even with only a single SHA256 in it
            True,
            12345,
            "",
            (a_sha256_checksum,),
    ...
    def test_fake_vsc(self):
        ...
        test_python_mod = os.path.join(
            self.test_prefix, "test_fake_vsc", "import_vsc.py"
        )
        ...
        ft.move_file(
            test_python_mod,
            os.path.join(os.path.dirname(test_python_mod), "pkgutil.py"),
        )
