...
from easybuild.tools.package.utilities import (
    ActivePNS,
...
def mock_fpm(tmpdir):
    ...
    write_file(
        fpm, MOCKED_FPM % {"fpm_output_file": os.path.join(tmpdir, FPM_OUTPUT_FILE)}
    )
    ...
    write_file(
        rpmbuild, "#!/bin/bash"
    )  # only needs to be there, doesn't need to actually do something...
...
class PackageTest(EnhancedTestCase):
    ...
    def test_avail_package_naming_schemes(self):
        ...
        self.assertEqual(
            sorted(avail_package_naming_schemes().keys()), ["EasyBuildPNS"]
        )
    ...
    def test_check_pkg_support(self):
        ...
        self.assertErrorRegex(
            EasyBuildError, "Selected packaging tool 'fpm' not found", check_pkg_support
        )
    ...
    def test_active_pns(self):
        ...
        test_ec = os.path.join(
            test_easyconfigs, "o", "OpenMPI", "OpenMPI-2.1.2-GCC-6.4.0-2.28.eb"
        )
    ...
    def test_package(self):
        ...
        pkgfile = os.path.join(
            pkgdir, "toy-0.0-gompi-2018a-test-eb-%s.1.rpm" % EASYBUILD_VERSION
        )
        ...
        self.assertTrue(
            regex.search(fpm_output),
            "Pattern '%s' found in: %s" % (regex.pattern, fpm_output),
        )

        pkgtxt = read_file(pkgfile)
        pkgtxt_regex = re.compile(
            "STARTCONTENTS of installdir %s" % easyblock.installdir
        )
        self.assertTrue(
        ...
        no_logfiles_regex = re.compile(
            r"STARTCONTENTS.*\.(log|md)$.*ENDCONTENTS", re.DOTALL | re.MULTILINE
        )
        res = no_logfiles_regex.search(pkgtxt)
        self.assertFalse(
            res, "Pattern not '%s' found in: %s" % (no_logfiles_regex.pattern, pkgtxt)
        )
        ...
        replace_str = (
            '''description = """Toy C program, 100% toy. Now with `backticks'\n'''
        )
        ...
        self.assertTrue(
            regex.search(toy_txt),
            "Pattern '%s' found in: %s" % (regex.pattern, toy_txt),
        )
        ...
        self.assertTrue(
            regex_pkg.search(pkgtxt),
            "Pattern '%s' not found in: %s" % (regex_pkg.pattern, pkgtxt),
        )
