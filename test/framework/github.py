...
class GithubTest(EnhancedTestCase):
    ...
    def test_fetch_easyconfigs_from_pr(self):
        ...
        try:
            ...
            tmpdir = os.path.join(self.test_prefix, 'pr2481')
            ec_files = gh.fetch_easyconfigs_from_pr(2481, path=tmpdir, github_user=GITHUB_TEST_ACCOUNT)
            self.assertEqual(all_ecs_pr2481, sorted([os.path.basename(f) for f in ec_files]))
            ...
            tmpdir = os.path.join(self.test_prefix, 'pr6587')
            ec_files = gh.fetch_easyconfigs_from_pr(6587, path=tmpdir, github_user=GITHUB_TEST_ACCOUNT)
            self.assertEqual(all_ecs_pr6587, sorted([os.path.basename(f) for f in ec_files]))
