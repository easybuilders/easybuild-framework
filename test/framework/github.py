...
class GithubTest(EnhancedTestCase):
    ...
    def setUp(self):
        ...
        if self.github_token is None:
            self.ghfs = gh.Githubfs(
                GITHUB_USER, GITHUB_REPO, GITHUB_BRANCH, None, None, None
            )
        else:
            self.ghfs = gh.Githubfs(
                GITHUB_USER,
        ...
        self.skip_github_tests = (
            self.github_token is None and os.getenv("FORCE_EB_GITHUB_TESTS") is None
        )
    ...
    def test_fetch_pr_data(self):
        ...
        pr_data, pr_url = gh.fetch_pr_data(
            1, GITHUB_USER, GITHUB_REPO, GITHUB_TEST_ACCOUNT
        )
        ...
        self.assertFalse(
            any(
                key in pr_data
                for key in ["issue_comments", "review", "status_last_commit"]
            )
        )

        pr_data, pr_url = gh.fetch_pr_data(
            2, GITHUB_USER, GITHUB_REPO, GITHUB_TEST_ACCOUNT, full=True
        )
    ...
    def test_reasons_for_closing(self):
        ...
        pr_data, _ = gh.fetch_pr_data(
            1844, repo_owner, repo_name, GITHUB_TEST_ACCOUNT, full=True
        )
        ...
        self.assertEqual(
            stderr.strip(), "WARNING: Using easyconfigs from closed PR #1844"
        )
        patterns = [
        ...
        for pattern in patterns:
            self.assertTrue(
                pattern in stdout, "Pattern '%s' found in: %s" % (pattern, stdout)
            )
    ...
    def test_close_pr(self):
        ...
        patterns = [
            "hpcugent/testrepository PR #2 was submitted by migueldiascosta",
            "[DRY RUN] Adding comment to testrepository issue #2: '"
            + "@migueldiascosta, this PR is being closed for the following reason(s): %s"
            % retest_msg,
            "[DRY RUN] Closed hpcugent/testrepository PR #2",
    ...
    def test_fetch_easyconfigs_from_pr(self):
        ...
        init_config(
            build_options={"pr_target_account": gh.GITHUB_EB_MAIN,}
        )
        ...
        for pr, all_ecs in [
            ...
            try:
                ...
                self.assertEqual(
                    sorted(all_ecs), sorted([os.path.basename(f) for f in ec_files])
                )
    ...
    def test_fetch_latest_commit_sha(self):
        ...
        sha = gh.fetch_latest_commit_sha(
            "easybuild-framework", "easybuilders", github_user=GITHUB_TEST_ACCOUNT
        )
    ...
    def test_download_repo(self):
        ...
        repodir = os.path.join(
            self.test_prefix, "easybuilders", "easybuild-easyconfigs-master"
        )
        ...
        self.assertTrue(
            os.path.exists(
                os.path.join(
                    repodir, "easybuild", "easyconfigs", "f", "foss", "foss-2019b.eb"
                )
            )
        )
        ...
        self.assertTrue(
            os.path.exists(
                os.path.join(repodir, "easybuild", "easyblocks", "__init__.py")
            )
        )
    ...
    def test_validate_github_token(self):
        ...
        self.assertTrue(
            gh.validate_github_token(self.github_token, GITHUB_TEST_ACCOUNT)
        )
    ...
    def test_find_easybuild_easyconfig(self):
        ...
        expected = os.path.join(
            "e", "EasyBuild", "EasyBuild-[1-9]+\.[0-9]+\.[0-9]+\.eb"
        )
        regex = re.compile(expected)
        self.assertTrue(
            regex.search(path), "Pattern '%s' found in '%s'" % (regex.pattern, path)
        )
    ...
    def test_find_patches(self):
        ...
        self.mock_stdout(True)
        ec = gh.find_software_name_for_patch(
            "toy-0.0_fix-silly-typo-in-printf-statement.patch", [ec_path]
        )
        txt = self.get_stdout()
        self.mock_stdout(False)
    ...
    def test_check_pr_eligible_to_merge(self):
        ...
        def run_check(expected_result=False):
            ...
            self.assertTrue(
                expected_warning in stderr,
                "Found '%s' in: %s" % (expected_warning, stderr),
            )
            return stderr
        ...
        test_result_warning_template = (
            "* test suite passes: %s => not eligible for merging!"
        )
    ...
    def test_det_patch_specs(self):
        ...
        patch_paths = [
            os.path.join(self.test_prefix, p) for p in ["1.patch", "2.patch", "3.patch"]
        ]
        file_info = {
            "ecs": [
        ...
        error_pattern = (
            "Failed to determine software name to which patch file .*/2.patch relates"
        )
        self.mock_stdout(True)
        ...
        file_info["ecs"][-1] = {
            ...
            "exts_list": [
                "foo",
                ("bar", "1.2.3"),
                ("patched", "4.5.6", {"patches": [("2.patch", 1), "3.patch"],}),
            ],
        }
    ...
    def test_restclient(self):
        ...
        client = RestClient(
            ...
            username=GITHUB_TEST_ACCOUNT,
            token=self.github_token,
        )

        status, body = (
            client.repos["hpcugent"]["testrepository"]
            .contents.a_directory["a_file.txt"]
            .get()
        )
        ...
        self.assertEqual(
            body["content"].strip(),
            base64.b64encode(b"this is a line of text\n").decode(),
        )
        ...
        try:
            ...
            self.assertTrue(
                False, "posting to unauthorized endpoint did not throw a http error"
            )
        except HTTPError:
            httperror_hit = True
        ...
        try:
            ...
            self.assertTrue(
                False, "deleting to unauthorized endpoint did not throw a http error"
            )
        except HTTPError:
            httperror_hit = True
        self.assertTrue(httperror_hit, "expected HTTPError not encountered")
    ...
    def test_create_delete_gist(self):
        ...
        gh.delete_gist(
            gist_id, github_user=GITHUB_TEST_ACCOUNT, github_token=self.github_token
        )
    ...
    def test_det_account_branch_for_pr(self):
        ...
        self.mock_stdout(True)
        account, branch = gh.det_account_branch_for_pr(
            9149, github_user=GITHUB_TEST_ACCOUNT
        )
        self.mock_stdout(False)
        ...
        self.mock_stdout(True)
        account, branch = gh.det_account_branch_for_pr(
            3069, github_user=GITHUB_TEST_ACCOUNT
        )
        self.mock_stdout(False)
    ...
    def test_push_branch_to_github(self):
        ...
        pattern = (
            ...
            + "\n".join(
                [
                    r"== fetching branch 'master' from https://github.com/%s\.\.\."
                    % github_path,
                    r"== pushing branch 'test123' to remote 'github_.*' \(git@github.com:%s\) \[DRY RUN\]"
                    % github_path,
                ]
            )
            + r"$"
        ...
        self.assertTrue(
            regex.match(stdout.strip()),
            "Pattern '%s' doesn't match: %s" % (regex.pattern, stdout),
        )
