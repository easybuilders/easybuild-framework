...
class ParallelBuildTest(EnhancedTestCase):
    ...
    def test_build_easyconfigs_in_parallel_pbs_python(self):
        ...
        ec_file = os.path.join(
            topdir, "easyconfigs", "test_ecs", "g", "gzip", "gzip-1.5-foss-2018a.eb"
        )
        easyconfigs = process_easyconfig(ec_file)
        ordered_ecs = resolve_dependencies(easyconfigs, self.modtool)
        jobs = build_easyconfigs_in_parallel(
            "echo '%(spec)s'", ordered_ecs, prepare_first=False
        )
        ...
        ordered_ecs = resolve_dependencies(
            process_easyconfig(ec_file), self.modtool, retain_all_deps=True
        )
    ...
    def test_submit_jobs(self):
        ...
        args = [
            ...
            "--robot",
            self.test_prefix,  # should be excluded in job script
        ...
        for regex in regexs:
            ...
            self.assertTrue(
                regex.search(cmd), "Pattern '%s' found in: %s" % (regex.pattern, cmd)
            )
        ...
        for regex in [
            ...
            self.assertFalse(
                regex.search(cmd),
                "Pattern '%s' *not* found in: %s" % (regex.pattern, cmd),
            )
    ...
    def test_build_easyconfigs_in_parallel_slurm(self):
        ...
        foss_ec = os.path.join(
            topdir, "easyconfigs", "test_ecs", "f", "foss", "foss-2018a.eb"
        )
