...
class ModulesToolTest(EnhancedTestCase):
    ...
    def test_module_mismatch(self):
        ...
        os.environ[
            "module"
        ] = "() {  eval `/tmp/Modules/$MODULE_VERSION/bin/modulecmd bash $*`\n}"
        error_regex = ".*pattern .* not found in defined 'module' function"
        self.assertErrorRegex(
            EasyBuildError, error_regex, MockModulesTool, testing=True
        )
    ...
    def test_lmod_specific(self):
        ...
        if lmod_abspath is not None:
            ...
            self.assertTrue(
                lmod.update(), "Updated local Lmod spider cache is non-empty"
            )
