...
class TweakTest(EnhancedTestCase):
    ...
    def test_pick_version(self):
        ...
        self.assertEqual(
            ("1.12", "1.10"),
            pick_version("1.12", ["1.5", "1.20", "1.1", "1.50", "1.10", "1.9", "1.8"]),
        )

        # if no older versions are available, oldest available version is returned
        self.assertEqual(
            ("0.8", "1.1"), pick_version("0.8", ["1.5", "1.1", "1.10", "1.8"])
        )
    ...
    def test_find_matching_easyconfigs(self):
        ...
        for (name, installver) in [("GCC", "4.8.2"), ("gzip", "1.5-foss-2018a")]:
            ...
            self.assertTrue(
                len(ecs) == 1 and ecs[0].endswith("/%s-%s.eb" % (name, installver))
            )
        ...
        for gccver in gccvers:
            ...
            self.assertTrue(
                gcc_ec in ecs_basename, "%s is included in %s" % (gcc_ec, ecs_basename)
            )
    ...
    def test_obtain_ec_for(self):
        ...
        self.assertFalse(generated)
        self.assertEqual(
            os.path.basename(ec_file), "ScaLAPACK-2.0.2-gompi-2018a-OpenBLAS-0.2.20.eb"
        )
        ...
        self.assertFalse(generated)
        self.assertEqual(
            os.path.basename(ec_file), "ifort-2016.1.150-GCC-4.9.3-2.25.eb"
        )
    ...
    def test_tweak_one_version(self):
        ...
        for key in [
            k for k in toy_ec_parsed.keys() if k not in ["checksums", "version"]
        ]:
            ...
            self.assertEqual(
                val,
                tweaked_val,
                "Different value for %s parameter: %s vs %s" % (key, val, tweaked_val),
            )
    ...
    def test_check_capability_mapping(self):
        ...
        self.assertFalse(
            check_capability_mapping(foss_hierarchy[1], iimpi_hierarchy[1]),
            "golf requires math libs",
        )
        # gompi <-> iimpi
    ...
    def test_map_toolchain_hierarchies(self):
        ...
        self.assertEqual(
            map_toolchain_hierarchies(iimpi_tc, foss_tc, self.modtool), expected
        )
        ...
        self.assertEqual(
            map_toolchain_hierarchies(gompi_tc, iimpi_tc, self.modtool), expected
        )
        ...
        self.assertEqual(
            map_toolchain_hierarchies(
                gcc_binutils_tc, iccifort_binutils_tc, self.modtool
            ),
            expected,
        )
    ...
    def test_map_easyconfig_to_target_tc_hierarchy(self):
        ...
        tc_mapping = map_toolchain_hierarchies(
            gcc_binutils_tc, iccifort_binutils_tc, self.modtool
        )
        ec_spec = os.path.join(
            test_easyconfigs, "h", "hwloc", "hwloc-1.6.2-GCC-4.9.3-2.26.eb"
        )
