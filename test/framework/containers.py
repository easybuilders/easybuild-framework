...
class ContainersTest(EnhancedTestCase):
    ...
    def test_end2end_singularity_recipe_config(self):
        ...
        test_container_recipe = os.path.join(
            self.test_prefix, "containers", "Singularity.toy-0.0"
        )
        ...
        self.assertErrorRegex(
            EasyBuildError, error_pattern, self.run_main, args, raise_error=True
        )
        ...
        for (bootstrap, from_spec) in test_cases:
            ...
            txt = read_file(test_container_recipe)
            expected = "\n".join(
                ["Bootstrap: %s" % bootstrap, "From: %s" % from_spec, "",]
            )
            self.assertTrue(
                txt.startswith(expected),
                "Container recipe starts with '%s':\n\n%s" % (expected, txt),
        ...
        for pattern in pip_patterns:
            ...
            self.assertFalse(
                regex.search(txt),
                "Pattern '%s' should not be found in: %s" % (regex.pattern, txt),
            )
    ...
    def test_end2end_singularity_image(self):
        ...
        if which("singularity") is None:
            error_pattern = (
                "singularity with version 2.4 or higher not found on your system."
            )
            self.assertErrorRegex(
        ...
        os.environ["PATH"] = os.path.pathsep.join(
            [os.path.join(self.test_prefix, "bin"), os.getenv("PATH")]
        )
        ...
        for (version, ext) in [("2.4.0", "simg"), ("3.1.0", "sif")]:
            ...
            self.assertTrue(
                os.path.exists(os.path.join(containerpath, "toy-0.0.%s" % ext))
            )
        ...
        args.extend(
            ["--container-image-format=ext3", "--container-image-name=foo-bar",]
        )
        stdout, stderr = self.run_main(args)
        self.assertFalse(stderr)
        regexs = [
        ...
        self.assertFalse(stderr)
        regexs[-3] = (
            "^== Running 'sudo\s*SINGULARITY_TMPDIR=%s \S*/singularity build .*"
            % self.test_prefix
        )
        self.check_regexs(regexs, stdout)
    ...
    def test_end2end_dockerfile(self):
        ...
        error_pattern = (
            "Container recipe at %s/containers/Dockerfile.toy-0.0 already exists, "
        ...
        for pattern in [r"^\s+", r"\s+$"]:
            ...
            self.assertFalse(
                ...
                "Pattern '%s' should *not* be found in: %s" % (pattern, def_file),
            )
    ...
    def test_container_config_template_recipe(self):
        ...
        self.assertFalse(stderr)
        regex = re.compile(
            "^== Singularity definition file created at .*/containers/Singularity.toy-0.0$"
        )
        self.assertTrue(
            regex.match(stdout),
            "Stdout matches pattern '%s': %s" % (regex.pattern, stdout),
        )
        ...
        cont_recipe = read_file(
            os.path.join(self.test_prefix, "containers", "Singularity.toy-0.0")
        )
