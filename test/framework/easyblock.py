...
class EasyBlockTest(EnhancedTestCase):
    ...
    def test_empty(self):
        ...
        self.assertErrorRegex(
            EasyBuildError, "Value of incorrect type passed", EasyBlock, ""
        )
    ...
    def test_load_module(self):
        ...
        copy_dir(
            os.path.join(test_dir, "modules", "OpenMPI"),
            os.path.join(tmp_modules, "OpenMPI"),
        )
        ...
        orig_tmpdir = os.path.join(
            self.test_prefix, "verylongdirectorythatmaycauseproblemswithopenmpi2"
        )
        ...
        self.assertTrue(
            stderr.strip().startswith(
                "WARNING: Long $TMPDIR path may cause problems with OpenMPI 2.x"
            )
        )
    ...
    def test_fake_module_load(self):
        ...
        if get_module_syntax() == "Lua":
            ...
            dot_version_txt = read_file(
                os.path.join(fake_mod_data[0], "pi", ".version")
            )
    ...
    def test_make_module_extend_modpath(self):
        ...
        if get_module_syntax() == "Tcl":
            regexs = [
                r'^module use ".*/modules/funky/Compiler/pi/3.14/%s"$' % c
                for c in modclasses
            ]
            home = r"\$::env\(HOME\)"
            ...
            regexs.extend(
                [
                    ...
                    r"if { \[ file isdirectory \[ file join %s \[ %s \] \] \] } {$"
                    % (home, fj_usermodsdir),
                    # no per-moduleclass extension for user modules
                    r"^\s+module use \[ file join %s \[ %s \] \]$"
                    % (home, fj_usermodsdir),
                ]
            )
        elif get_module_syntax() == "Lua":
            regexs = [
                r'^prepend_path\("MODULEPATH", ".*/modules/funky/Compiler/pi/3.14/%s"\)$'
                % c
                for c in modclasses
            ]
            home = r'os.getenv\("HOME"\)'
            pj_usermodsdir = (
                'pathJoin\("%s", "funky", "Compiler/pi/3.14"\)' % usermodsdir
            )
            regexs.extend(
                [
                    # extension for user modules is guarded
                    ...
                    # no per-moduleclass extension for user modules
                    r'\s+prepend_path\("MODULEPATH", pathJoin\(%s, %s\)\)'
                    % (home, pj_usermodsdir),
                ]
            )
        else:
    ...
    def test_make_module_req(self):
        ...
        if get_module_syntax() == "Tcl":
            self.assertTrue(
                re.search(r"^prepend-path\s+CLASSPATH\s+\$root/bla.jar$", guess, re.M)
            )
            self.assertTrue(
                re.search(r"^prepend-path\s+CLASSPATH\s+\$root/foo.jar$", guess, re.M)
            )
            self.assertTrue(
                re.search(r"^prepend-path\s+MANPATH\s+\$root/share/man$", guess, re.M)
            )
            # bin/ is not added to $PATH if it doesn't include files
            self.assertFalse(
                re.search(r"^prepend-path\s+PATH\s+\$root/bin$", guess, re.M)
            )
            self.assertFalse(
                re.search(r"^prepend-path\s+PATH\s+\$root/sbin$", guess, re.M)
            )
            ...
            self.assertTrue(
                re.search(
                    r'^prepend_path\("CLASSPATH", pathJoin\(root, "bla.jar"\)\)$',
                    guess,
                    re.M,
                )
            )
            self.assertTrue(
                re.search(
                    r'^prepend_path\("CLASSPATH", pathJoin\(root, "foo.jar"\)\)$',
                    guess,
                    re.M,
                )
            )
            self.assertTrue(
                re.search(
                    r'^prepend_path\("MANPATH", pathJoin\(root, "share/man"\)\)$',
                    guess,
                    re.M,
                )
            )
            ...
            self.assertFalse(
                re.search(
                    r'^prepend_path\("PATH", pathJoin\(root, "bin"\)\)$', guess, re.M
                )
            )
            self.assertFalse(
                re.search(
                    r'^prepend_path\("PATH", pathJoin\(root, "sbin"\)\)$', guess, re.M
                )
            )
            # no include/ subdirectory, so no $CPATH update statement
    ...
    def test_make_module_extra(self):
        ...
        self.assertTrue(
            expected_default.match(defaulttxt),
            "Pattern %s found in %s" % (expected_default.pattern, defaulttxt),
        )

        alttxt = eb.make_module_extra(
            altroot="/opt/software/tau/6.28", altversion="6.28"
        ).strip()
        self.assertTrue(
            expected_alt.match(alttxt),
        ...
        error_pattern = (
            "Absolute path .* passed to update_paths which only expects relative paths"
        )
    ...
    def test_handle_iterate_opts(self):
        ...
        self.assertEqual(
            eb.cfg["configopts"], ["--opt1 --anotheropt", "--opt2", "--opt3 --optbis"]
        )
    ...
    def test_extensions_step(self):
        ...
        self.assertErrorRegex(
            EasyBuildError,
            "No default extension class set",
            eb.extensions_step,
            fetch=True,
        )
    ...
    def test_make_module_step(self):
        ...
        if get_module_syntax() == "Tcl":
            ...
            self.assertTrue(
                ebroot_regex.search(txt), "%s in %s" % (ebroot_regex.pattern, txt)
            )
            self.assertTrue(
                re.search(
            ...
            self.assertTrue(
                re.search(
                    r'^setenv\("EBVERSION%s", "%s"\)$' % (name.upper(), version),
                    txt,
                    re.M,
                )
            )
        ...
        for (key, val) in modextrapaths.items():
            if get_module_syntax() == "Tcl":
                ...
                regex = re.compile(
                    r'^prepend_path\("%s", pathJoin\(root, "%s"\)\)$' % (key, val), re.M
                )
            else:
                self.assertTrue(
                    False, "Unknown module syntax: %s" % get_module_syntax()
        ...
        if get_module_syntax() == "Lua":
            ...
            for descr in ["test [==[", "]==] foo"]:
                ...
                write_file(
                    self.eb_file,
                    re.sub("description.*", 'description = "%s"' % descr, ectxt),
                )
                ec = EasyConfig(self.eb_file)
                eb = EasyBlock(ec)
                eb.installdir = os.path.join(config.install_path(), "pi", "3.14")
                eb.check_readiness_step()
                self.assertErrorRegex(
                    EasyBuildError, error_pattern, eb.make_module_step
                )
    ...
    def test_gen_dirs(self):
        ...
        self.assertTrue(
            eb.cfg["cleanupoldbuild"] or eb.cfg.get("cleanupoldbuild", True)
        )
    ...
    def test_make_builddir(self):
        ...
        self.assertEqual(
            os.listdir(os.path.join(eb.builddir, "test123")), ["foobar.txt"]
        )
    ...
    def test_get_easyblock_instance(self):
        ...
        ec = process_easyconfig(
            os.path.join(testdir, "easyconfigs", "test_ecs", "t", "toy", "toy-0.0.eb")
        )[0]
        eb = get_easyblock_instance(ec)
    ...
    def test_fetch_sources(self):
        ...
        orig_toy_extra_txt = read_file(
            os.path.join(os.path.dirname(toy_source), "toy-extra.txt")
        )
        ...
        self.assertErrorRegex(
            EasyBuildError, error_pattern, eb.fetch_sources, sources, checksums=[]
        )
    ...
    def test_obtain_file(self):
        ...
        init_config(
            args=["--sourcepath=%s:/no/such/dir:%s" % (sandbox_sources, tmpdir)]
        )
        ...
        self.mock_stdout(True)
        res = eb.obtain_file(
            toy_tarball, urls=["file://%s" % tmpdir_subdir], force_download=True
        )
        stderr = self.get_stderr()
        stdout = self.get_stdout()
        self.mock_stderr(False)
        ...
        msg = (
            "WARNING: Found file toy-0.0.tar.gz at %s, but re-downloading it anyway..."
            % toy_tarball_path
        )
        ...
        error_regex = (
            "Couldn't find file %s anywhere, and downloading it didn't work either" % fn
        )
        self.assertErrorRegex(
            EasyBuildError,
            error_regex,
        ...
        init_config(
            args=["--sourcepath=%s:/no/such/dir:%s" % (tmpdir, sandbox_sources)]
        )
        urls = [
        ...
        for file_url in urls:
            ...
            if res is not None:
                ...
                self.assertTrue(
                    os.path.exists(loc), "%s file is found at %s" % (fn, loc)
                )
                txt = open(loc, "r").read()
                eb_regex = re.compile("EasyBuild: building software with ease")
                self.assertTrue(
                    eb_regex.search(txt),
                    "Pattern '%s' found in: %s" % (eb_regex.pattern, txt),
                )
            else:
                print(
                    "ignoring failure to download %s in test_obtain_file, testing offline?"
                    % file_url
                )
    ...
    def test_check_readiness(self):
        ...
        ec_path = os.path.join(
            topdir, "easyconfigs", "test_ecs", "o", "OpenMPI", ec_file
        )
        ...
        try:
            ...
            err_regex = re.compile(
                "Missing modules dependencies .*: nosuchsoftware/1.2.3-GCC-6.4.0-2.28"
            )
            self.assertTrue(
    ...
    def test_exclude_path_to_top_of_module_tree(self):
        ...
        impi_modfile_path = os.path.join(
            "Compiler", "intel", intel_ver, "impi", "5.1.2.150"
        )
        imkl_modfile_path = os.path.join(
            "MPI", "intel", intel_ver, "impi", "5.1.2.150", "imkl", "11.3.1.150"
        )
        if get_module_syntax() == "Lua":
        ...
        tests = [
            (
                "i/impi/impi-5.1.2.150-iccifort-2016.1.150-GCC-4.9.3-2.25.eb",
                impi_modfile_path,
                iccifort_mods,
            ),
            (
                "i/imkl/imkl-11.3.1.150-iimpi-2016.01.eb",
                imkl_modfile_path,
                iccifort_mods + ["iimpi", "impi"],
            ),
        ]
        for ec_file, modfile_path, excluded_deps in tests:
            ...
            for dep in excluded_deps:
                ...
                failmsg = (
                    "No 'module load' statement found for '%s' not found in module %s: %s"
                    % tup
                )
                if get_module_syntax() == "Tcl":
        ...
        expected = {
            ...
            impi_mod: [
                os.path.join(modpath, "MPI", "intel", intel_ver, "impi", "5.1.2.150")
            ],
        }
        self.assertEqual(
    ...
    def test_patch_step(self):
        ...
        self.assertEqual(
            sorted(os.listdir(toydir)),
            ["toy-extra.txt", "toy.source", "toy.source.orig"],
        )
        self.assertTrue(
            "and very proud of it" in read_file(os.path.join(toydir, "toy.source"))
        )
    ...
    def test_extensions_sanity_check(self):
        ...
        toy_ec_fn = os.path.join(
            test_ecs_dir, "t", "toy", "toy-0.0-gompi-2018a-test.eb"
        )
        ...
        toy_ec.update(
            ...
            [("%(installdir)s/bin/toy && rm %(installdir)s/bin/toy", "")],
        )
        eb = EB_toy(toy_ec)
        eb.silent = True
    ...
    def test_parallel(self):
        ...
        self.assertTrue(
            isinstance(test_eb.cfg["parallel"], int) and test_eb.cfg["parallel"] > 0
        )
    ...
    def test_guess_start_dir(self):
        ...
        err_pattern = (
            "Specified start dir .*/toy-0.0/thisstartdirisnotthere does not exist"
        )
    ...
    def test_prepare_step(self):
        ...
        mkdir(
            os.path.join(self.test_buildpath, "toy", "0.0", "system-system"),
            parents=True,
        )
        eb = EasyBlock(ec["ec"])
        eb.silent = True
    ...
    def test_checksum_step(self):
        ...
        ectxt = ectxt.replace(
            "be662daa971a640e40be5c804d9d7d10", "00112233445566778899aabbccddeeff"
        )
        ...
        self.assertEqual(
            stderr.strip(),
            "WARNING: Ignoring failing checksum verification for bar-0.0.tar.gz",
        )
    ...
    def test_check_checksums(self):
        ...
        for ext in ["bar", "barbar", "toy"]:
            expected = (
                "Checksums missing for one or more sources/patches of extension %s in "
                % ext
            )
    ...
    def test_avail_easyblocks(self):
        ...
        self.assertTrue(
            all(key.startswith("easybuild.easyblocks") for key in easyblocks)
        )
        ...
        self.assertTrue(
            bar["loc"].endswith("sandbox/easybuild/easyblocks/generic/bar.py")
        )
