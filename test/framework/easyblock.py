    ...
    def test_make_module_extend_modpath(self):
        ...
        if get_module_syntax() == 'Tcl':
            ...
            regexs.extend([
                ...
                r'^\s+module use \[ file join %s "funky/%s/Compiler/pi/3.14"\ ]$' % (home, usermodsdir),
            ])
            ...
            regexs.extend([
                # extension for user modules is guarded
                r'if isDir\(pathJoin\(%s, "funky/%s/Compiler/pi/3.14"\)\) then' % (home, usermodsdir),
                # no per-moduleclass extension for user modules
                r'\s+prepend_path\("MODULEPATH", pathJoin\(%s, "funky/%s/Compiler/pi/3.14"\)\)' % (home, usermodsdir),
            ])
        else:
