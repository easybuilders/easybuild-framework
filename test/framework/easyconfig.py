...
class EasyConfigTest(EnhancedTestCase):
    ...
    def test_new_cat_copy_edit(self):
        """Test combining --new with --cat, --copy and --edit."""

        args = ['toy', '1.2.3', 'foss/2018b']
        ec_fp = os.path.join(self.test_prefix, 'toy-1.2.3-foss-2018b.eb')

        # contents of easyconfig are printed when using --cat, file still is created into current directory
        (stdout, stderr) = self.run_eb_new(args + ['--cat'])
        self.assertTrue(os.path.exists(ec_fp))
        ...
        for copy_arg in copy_args:
            ...
            # combination of --copy and --cat also works
            (stdout, stderr) = self.run_eb_new(args + ['--cat', '--copy', copy_arg])
            self.assertFalse(os.path.exists(ec_fp))
            self.assertTrue(os.path.exists(test_ec), "%s exists" % test_ec)
