...
from easybuild.framework.easyconfig.tools import (
...
from easybuild.tools.module_naming_scheme.toolchain import (
...
class EasyConfigTest(EnhancedTestCase):
    ...
    def prep(self):
        ...
        if self.contents is not None:
            fd, self.eb_file = tempfile.mkstemp(
                prefix="easyconfig_test_file_", suffix=".eb"
            )
            os.close(fd)
    ...
    def test_mandatory(self):
        ...
        self.contents = "\n".join(
            ['easyblock = "ConfigureMake"', 'name = "pi"', 'version = "3.14"',]
        )
        self.prep()
    ...
    def test_validation(self):
        ...
        self.assertErrorRegex(
            EasyBuildError, r"\w* provided '\w*' is not valid", ec.validate
        )
        ...
        self.assertErrorRegex(
            EasyBuildError, "OS dependencies were not found", ec.validate
        )
        ...
        self.assertErrorRegex(
            EasyBuildError, "Permission denied", EasyConfig, self.eb_file
        )
        ...
        self.prep()
        error_pattern = (
            "Parsing easyconfig file failed: EOL while scanning string literal"
        )
        ...
        self.contents = self.contents.replace(
            "syntax_error'", "foo = '%(name)s %s' % version"
        )
        self.prep()
        error_pattern = (
            r"Parsing easyconfig file failed: format requires a mapping \(line 8\)"
        )
        self.assertErrorRegex(EasyBuildError, error_pattern, EasyConfig, self.eb_file)
    ...
    def test_shlib_ext(self):
        ...
        eb = EasyConfig(self.eb_file)
        self.assertEqual(
            eb["sanity_check_paths"]["files"][0], "lib/lib.%s" % get_shared_lib_ext()
        )
    ...
    def test_dependency(self):
        ...
        self.assertErrorRegex(
            EasyBuildError,
            "Dependency foo of unsupported type",
            eb._parse_dependency,
            "foo",
        )
        self.assertErrorRegex(EasyBuildError, "without name", eb._parse_dependency, ())
        self.assertErrorRegex(
            EasyBuildError, "without version", eb._parse_dependency, {"name": "test"}
        )
        err_msg = "Incorrect external dependency specification"
        self.assertErrorRegex(
            EasyBuildError, err_msg, eb._parse_dependency, (EXTERNAL_MODULE_MARKER,)
        )
        self.assertErrorRegex(
            EasyBuildError,
            err_msg,
            eb._parse_dependency,
            ("foo", "1.2.3", EXTERNAL_MODULE_MARKER),
        )
    ...
    def test_extra_options(self):
        ...
        eb = EasyConfig(self.eb_file)
        self.assertErrorRegex(
            EasyBuildError, "unknown easyconfig parameter", lambda: eb["custom_key"]
        )

        extra_vars = {
            "custom_key": ["default", "This is a default key", easyconfig.CUSTOM]
        }
        ...
        extra_vars.update(
            {
                "mandatory_key": [
                    "default",
                    "another mandatory key",
                    easyconfig.MANDATORY,
                ]
            }
        )
        self.assertErrorRegex(
            EasyBuildError,
            r"mandatory parameters not provided",
            EasyConfig,
            self.eb_file,
        ...
        self.assertTrue(
            regex.search(ectxt), "Pattern '%s' found in: %s" % (regex.pattern, ectxt)
        )
    ...
    def test_exts_list(self):
        ...
        modfile = os.path.join(
            eb.make_module_step(),
            "pi",
            "3.14" + eb.module_generator.MODULE_FILE_EXTENSION,
        )
        modtxt = read_file(modfile)
        regex = re.compile("EBEXTSLISTPI.*ext1-1.0,ext2-2.0")
        self.assertTrue(
            regex.search(modtxt), "Pattern '%s' found in: %s" % (regex.pattern, modtxt)
        )
    ...
    def test_extensions_templates(self):
        ...
        self.assertEqual(
            toy_ext.cfg["source_urls"],
            ["https://pypi.python.org/packages/source/t/toy"],
        )
        self.assertEqual(toy_ext.cfg["patches"], [toy_patch_fn])
        self.assertEqual(
            toy_ext.cfg["prebuildopts"], "gcc -O2 toy.c -o toy-0.0 && mv toy-0.0 toy #"
        )

        # check whether files expected to be installed for 'toy' extension are in place
        pi_installdir = os.path.join(
            self.test_installpath, "software", "pi", "3.14-test"
        )
    ...
    def test_suggestions(self):
        ...
        self.prep()
        self.assertErrorRegex(
            EasyBuildError, "dependencis -> dependencies", EasyConfig, self.eb_file
        )
        self.assertErrorRegex(
            EasyBuildError, "source_uls -> source_urls", EasyConfig, self.eb_file
        )
        self.assertErrorRegex(
            EasyBuildError, "source_URLs -> source_urls", EasyConfig, self.eb_file
        )
        self.assertErrorRegex(
            EasyBuildError, "sourceURLs -> source_urls", EasyConfig, self.eb_file
        )
    ...
    def test_installversion(self):
        ...
        self.assertEqual(
            installver, "%s%s-%s-%s%s" % (verpref, ver, tcname, tcver, versuff)
        )
    ...
    def test_obtain_easyconfig(self):
        ...
        self.assertErrorRegex(
            EasyBuildError, error_regexp, obtain_ec_for, specs, [self.test_prefix], None
        )
        ...
        ver = "3.12"
        suff = "mysuff"
        specs.update({"name": name, "version": ver, "versionsuffix": suff})
        ...
        specs.update({"foo": "bar123"})
        self.assertErrorRegex(
            EasyBuildError,
            "Unknown easyconfig parameter: foo",
            obtain_ec_for,
            specs,
            [self.test_prefix],
            None,
        ...
        specs.update(
            {"version": "3.15", "toolchain_version": "4.8.3",}
        )
        res = obtain_ec_for(specs, [self.test_prefix], None)
        self.assertEqual(res[0], True)
        ec = EasyConfig(res[1])
        ...
        txt = read_file(res[1])
        pattern = (
            "^toolchain = .*version.*[\"']%s[\"'].*}$" % specs["toolchain_version"]
        )
        ...
        specs.update(
            {
                ...
                "dependencies": [
                    ("foo", "1.2.3"),
                    ("bar", "666", "-bleh", ("gompi", "2018a")),
                ],
                "hiddendependencies": [
                    ("test", "3.2.1"),
                    ("testbuildonly", "4.9.3-2.25"),
                ],
            }
        )
        parsed_deps = [
        ...
        self.assertFalse(
            "test/3.2.1-GCC-4.8.3" in [d["full_mod_name"] for d in ec["dependencies"]]
        )
        self.assertTrue(
            "test/.3.2.1-GCC-4.8.3"
            in [d["full_mod_name"] for d in ec["hiddendependencies"]]
        )
        self.assertFalse(
            "testbuildonly/4.9.3-2.25-GCC-4.8.3"
            in [d["full_mod_name"] for d in ec["builddependencies"]]
        )
        self.assertTrue(
            "testbuildonly/.4.9.3-2.25-GCC-4.8.3"
            in [d["full_mod_name"] for d in ec["hiddendependencies"]]
        ...
        def trim_path(path):
            ...
            if len(dirs) > 3 and "site-packages" in dirs:
                if path.endswith(".egg"):
                    path = os.path.join(
                        *dirs[:-4]
                    )  # strip of lib/python2.7/site-packages/*.egg part
                else:
                    path = os.path.join(
                        *dirs[:-3]
                    )  # strip of lib/python2.7/site-packages part
    ...
    def test_templating(self):
        ...
        self.assertEqual(
            eb["source_urls"][1], "https://github.com/easybuilders/PI/archive"
        )
        self.assertEqual(eb["versionsuffix"], "-Python-2.7.10")
        self.assertEqual(eb["sanity_check_paths"]["files"][0], "bin/pi_3_04")
        self.assertEqual(
            eb["sanity_check_paths"]["files"][1], "lib/python2.7/site-packages"
        )
        self.assertEqual(
            eb["sanity_check_paths"]["dirs"][0], "libfoo.%s" % get_shared_lib_ext()
        )
        lib_arch_regex = re.compile(
            "^lib/[a-z0-9_]+$"
        )  # should match lib/x86_64, lib/aarch64, lib/ppc64le, etc.
        dirs1 = eb["sanity_check_paths"]["dirs"][1]
        self.assertTrue(
            lib_arch_regex.match(dirs1),
            "Pattern '%s' matches '%s'" % (lib_arch_regex.pattern, dirs1),
        )
        ...
        self.assertEqual(
            eb["license_file"],
            os.path.join(os.environ["HOME"], "licenses", "PI", "license.txt"),
        )

        # test the escaping insanity here (ie all the crap we allow in easyconfigs)
        eb[
            "description"
        ] = "test easyconfig % %% %s% %%% %(name)s %%(name)s %%%(name)s %%%%(name)s"
        self.assertEqual(
    ...
    def test_build_options(self):
        ...
        self.assertErrorRegex(
            EasyBuildError,
            "Build option lists for iterated build should have same length",
            eb.validate,
        )
    ...
    def test_format_equivalence_basic(self):
        ...
        for eb_file1, eb_file2, specs in [
            ...
            (
                ...
                "gzip.eb",
                {"version": "1.4", "toolchain": {"name": "GCC", "version": "4.6.3"}},
            ),
            (
            ...
            (
                "gzip-1.5-intel-2018a.eb",
                "gzip.eb",
                {"version": "1.5", "toolchain": {"name": "intel", "version": "2018a"}},
            ),
        ]:
            eb_file1 = glob.glob(
                os.path.join(easyconfigs_path, "v1.0", "*", "*", eb_file1)
            )[0]
            ec1 = EasyConfig(eb_file1, validate=False)
            ec2 = EasyConfig(
                os.path.join(easyconfigs_path, "v2.0", eb_file2),
                validate=False,
                build_specs=specs,
            )
            ...
            self.assertEqual(
                ec1.asdict(),
                ec2_dict,
                "Parsed %s is equivalent with %s" % (eb_file1, eb_file2),
            )
    ...
    def test_fetch_parameters_from_easyconfig(self):
        ...
        test_ecs_dir = os.path.join(
            os.path.abspath(os.path.dirname(__file__)), "easyconfigs", "test_ecs"
        )
        ...
        for ec_file, correct_name, correct_easyblock in [
            ...
            (
                os.path.join(test_ecs_dir, "f", "foss", "foss-2018a.eb"),
                "foss",
                "Toolchain",
            ),
        ]:
            name, easyblock = fetch_parameters_from_easyconfig(
                read_file(ec_file), ["name", "easyblock"]
            )
        ...
        self.assertEqual(
            fetch_parameters_from_easyconfig(read_file(toy_ec_file), ["description"])[
                0
            ],
            expected,
        )

        res = fetch_parameters_from_easyconfig(
            "easyblock = 'ConfigureMake'  # test comment", ["easyblock"]
        )
    ...
    def test_get_easyblock_class(self):
        ...
        self.assertErrorRegex(
            EasyBuildError, error_pattern, get_easyblock_class, None, name="gzip"
        )
        self.assertEqual(
            get_easyblock_class(None, name="gzip", error_on_missing_easyblock=False),
            None,
        )
        self.assertEqual(get_easyblock_class(None, name="toy"), EB_toy)
        self.assertErrorRegex(
            EasyBuildError,
            "Failed to import EB_TOY",
            get_easyblock_class,
            None,
            name="TOY",
        )
        self.assertEqual(
            get_easyblock_class(None, name="TOY", error_on_failed_import=False), None
        )
        ...
        self.assertEqual(
            get_easyblock_class(None, name="gzip", default_fallback=False), None
        )
    ...
    def test_toolchain_inspection(self):
        ...
        tc_compilers = [
            "/".join([x["name"], x["version"]]) for x in det_toolchain_compilers(ec)
        ]
        ...
        self.assertEqual(
            [
                "/".join([x["name"], x["version"] + x["versionsuffix"]])
                for x in tc_comps
            ],
            expected,
        )
    ...
    def test_filter_deps(self):
        ...
        shutil.copy2(
            os.path.join(
                test_ecs_dir, "o", "OpenMPI", "OpenMPI-2.1.2-GCC-6.4.0-2.28.eb"
            ),
            ec_file,
        )
    ...
    def test_replaced_easyconfig_parameters(self):
        ...
        for key, (newkey, ver) in replaced_parameters.items():
            error_regex = "NO LONGER SUPPORTED since v%s.*'%s' is replaced by '%s'" % (
                ver,
                key,
                newkey,
            )
    ...
    def test_deprecated_easyconfig_parameters(self):
        ...
        orig_deprecated_parameters = copy.deepcopy(
            easyconfig.parser.DEPRECATED_PARAMETERS
        )
        ...
        for key, (newkey, depr_ver) in easyconfig.parser.DEPRECATED_PARAMETERS.items():
            if LooseVersion(depr_ver) <= easybuild.tools.build_log.CURRENT_VERSION:
                # deprecation error
                error_regex = (
                    "DEPRECATED.*since v%s.*'%s' is deprecated.*use '%s' instead"
                    % (depr_ver, key, newkey)
                )
                self.assertErrorRegex(EasyBuildError, error_regex, ec.get, key)
                self.assertErrorRegex(EasyBuildError, error_regex, lambda k: ec[k], key)
    ...
    def test_unknown_easyconfig_parameter(self):
        ...
        self.assertErrorRegex(
            ...
            error_regex,
            lambda k: ec[k],
            "therenosucheasyconfigparameterlikethis",
        )
    ...
    def test_external_dependencies(self):
        ...
        ectxt = read_file(
            os.path.join(
                topdir, "easyconfigs", "test_ecs", "t", "toy", "toy-0.0-deps.eb"
            )
        )
        ...
        self.assertEqual(
            [d["external_module"] for d in deps],
            [True, False, True, True, True, True, True],
        )
        self.assertEqual(
            [d["hidden"] for d in deps],
    ...
    def test_update(self):
        ...
        toy_ebfile = os.path.join(
            topdir, "easyconfigs", "test_ecs", "t", "toy", "toy-0.0.eb"
        )
        ...
        self.assertEqual(
            ec["patches"],
            [toy_patch_fn, ("toy-extra.txt", "toy-0.0"), "foo.patch", "bar.patch"],
        )
    ...
    def test_hide_hidden_deps(self):
        ...
        ec_file = os.path.join(
            test_dir, "easyconfigs", "test_ecs", "g", "gzip", "gzip-1.4-GCC-4.6.3.eb"
        )
        ec = EasyConfig(ec_file)
    ...
    def test_dump(self):
        ...
        for ecfile in ecfiles:
            ...
            for pattern in patterns:
                ...
                self.assertTrue(
                    regex.search(ectxt),
                    "Pattern '%s' found in: %s" % (regex.pattern, ectxt),
                )
    ...
    def test_dump_template(self):
        ...
        self.assertTrue(
            ec.enable_templating
        )  # templating should still be enabled after calling dump()
    ...
    def test_dump_comments(self):
        ...
        self.assertEqual(
            ec.parser._formatter.comments["above"],
            {
        ...
        self.assertEqual(
            ec.parser._formatter.comments["header"],
            ["# #", "# some header comment", "# #"],
        )
        self.assertEqual(
            ec.parser._formatter.comments["inline"],
            {"description": "  # test", "name": "  # name comment",},
        )
        self.assertEqual(ec.parser._formatter.comments["iterabove"], {})
        self.assertEqual(
            ec.parser._formatter.comments["iterinline"],
            {
                "sanity_check_paths": {
                    "    'files': ['files/foobar'],": "  # comment on files"
                },
        ...
        self.assertEqual(
            ec.parser._formatter.comments["inline"],
            {"homepage": "  # may not be actual homepage", "name": "  # just a test",},
        )
        self.assertEqual(
            ec.parser._formatter.comments["iterabove"],
            {
                "dependencies": {
                    "    ('foo', '1.2.3'),": [
                        "# this dependency",
        ...
        self.assertEqual(
            ec.parser._formatter.comments["tail"],
            [
                "#",
                "# trailing comment",
                "",
        ...
        patterns = [
            ...
            "\n".join([r"", r"# the version doesn't matter much here", r"version = ",]),
            "\n".join(
                [
                    r"",
    ...
    def test_to_template_str(self):
        ...
        self.assertEqual(
            to_template_str("test", "template", templ_const, templ_val),
            "TEMPLATE_VALUE",
        )
        self.assertEqual(
            to_template_str("test", "foo/bar/0.0.1/", templ_const, templ_val),
            "%(name)s/bar/%(version)s/",
        )
        self.assertEqual(
            to_template_str("test", "foo-0.0.1", templ_const, templ_val), "NAME_VERSION"
        )
        templ_list = to_template_str(
            "test", "['-test', 'dontreplacenamehere']", templ_const, templ_val
        )
        self.assertEqual(templ_list, "['%(special_char)s', 'dontreplacenamehere']")
        templ_dict = to_template_str(
            "test", "{'a': 'foo', 'b': 'notemplate'}", templ_const, templ_val
        )
        self.assertEqual(templ_dict, "{'a': '%(name)s', 'b': 'notemplate'}")
        templ_tuple = to_template_str(
            "test", "('foo', '0.0.1')", templ_const, templ_val
        )
        ...
        self.assertEqual(
            to_template_str("versionsuffix", test_input, templ_const, templ_val),
            "-Python-%(pyver)s",
        )
        self.assertEqual(
            to_template_str("test", test_input, templ_const, templ_val),
            "%(versionsuffix)s",
        )
        ...
        self.assertEqual(
            to_template_str("sanity_check_paths", test_input, templ_const, templ_val),
            res,
        )
    ...
    def test_ActiveMNS_det_full_module_name(self):
        ...
        self.assertEqual(
            ActiveMNS().det_full_module_name(ec["dependencies"][0]), "intel/2018a"
        )
        self.assertEqual(
            ActiveMNS().det_full_module_name(ec["dependencies"][1]), "GCC/6.4.0-2.28"
        )

        ec_file = os.path.join(
            topdir, "easyconfigs", "test_ecs", "g", "gzip", "gzip-1.4-GCC-4.6.3.eb"
        )
        ec = EasyConfig(ec_file)
        hiddendep = ec["hiddendependencies"][0]
        self.assertEqual(ActiveMNS().det_full_module_name(hiddendep), "toy/.0.0-deps")
        self.assertEqual(
            ActiveMNS().det_full_module_name(hiddendep, force_visible=True),
            "toy/0.0-deps",
        )
    ...
    def test_find_related_easyconfigs(self):
        ...
        res = [
            os.path.basename(x) for x in find_related_easyconfigs(test_easyconfigs, ec)
        ]
    ...
    def test_param_value_type_checking(self):
        ...
        topdir = os.path.dirname(os.path.abspath(__file__))
        ec_file = os.path.join(
            topdir, "easyconfigs", "test_ecs", "g", "gzip", "gzip-1.4-broken.eb"
        )
        # version parameter has values of wrong type in this broken easyconfig
        error_msg_pattern = (
            "Type checking of easyconfig parameter values failed: .*'version'.*"
        )
        self.assertErrorRegex(
            EasyBuildError,
            error_msg_pattern,
            EasyConfig,
            ec_file,
            auto_convert_value_types=False,
        )
    ...
    def test_copy_easyconfigs(self):
        ...
        expected = os.path.join(
            target_dir, "easybuild", "easyconfigs", "g", "GCC", "GCC-6.4.0-2.28.eb"
        )
        ...
        for orig_ec, src_ec in test_ecs:
            ...
            copied_ec = os.path.join(
                ecs_target_dir, orig_ec[0].lower(), orig_ec.split("-")[0], orig_ec
            )
            self.assertTrue(os.path.exists(copied_ec), "File %s exists" % copied_ec)
            self.assertEqual(
                read_file(copied_ec), read_file(os.path.join(self.test_prefix, src_ec))
            )
    ...
    def test_template_constant_dict(self):
        ...
        ec = EasyConfig(
            os.path.join(test_ecs_dir, "g", "gzip", "gzip-1.5-foss-2018a.eb")
        )
        ...
        arch = res.pop("arch")
        self.assertTrue(
            arch_regex.match(arch),
            "'%s' matches with pattern '%s'" % (arch, arch_regex.pattern),
        )
    ...
    def test_parse_deps_templates(self):
        ...
        shutil.copy2(
            os.path.join(test_ecs, "p", "Python", "Python-2.7.10-intel-2018a.eb"), pyec
        )
        write_file(
        ...
        write_file(
            os.path.join(self.test_prefix, "pytest-1.2.3-foss-2018a-Python-2.7.10.eb"),
            pytest_ec_txt,
        )
        ...
        init_config(
            args=["--module-naming-scheme=HierarchicalMNS"], build_options=build_options
        )
        ...
        dep_full_mod_names = [
            d["full_mod_name"] for d in ordered_ecs[-1]["ec"]._config["dependencies"][0]
        ]
    ...
    def test_hidden_toolchain(self):
        ...
        ec_txt = read_file(
            os.path.join(test_ecs_dir, "g", "gzip", "gzip-1.6-GCC-4.9.2.eb")
        )
    ...
    def test_categorize_files_by_type(self):
        ...
        toy_patch_fn = "toy-0.0_fix-silly-typo-in-printf-statement.patch"
        toy_patch = os.path.join(
            os.path.dirname(test_ecs_dir), "sandbox", "sources", "toy", toy_patch_fn
        )
        paths = [
            "bzip2-1.0.6.eb",
    ...
    def test_resolve_template(self):
        ...
        self.assertEqual(
            resolve_template("%(namelower)s-%(version)s", tmpl_dict), "foobar-1.2.3"
        )

        value, expected = (
            ["%(namelower)s-%(version)s", "name:%(name)s"],
            ["foobar-1.2.3", "name:FooBar"],
        )
    ...
    def test_det_subtoolchain_version(self):
        ...
        optional_toolchains = set(
            tc_class.NAME
            for tc_class in all_tc_classes
            if getattr(tc_class, "OPTIONAL", False)
        )
        ...
        versions = [
            det_subtoolchain_version(
                current_tc, subtoolchain_name, optional_toolchains, cands
            )
        ...
        init_config(
            build_options={
                "add_system_to_minimal_toolchains": False,
    ...
    def test_verify_easyconfig_filename(self):
        ...
        specs["versionsuffix"] = ""
        error_pattern = (
            "filename '%s' does not match with expected filename 'toy-0.0-gompi-2018a.eb' "
            % toy_ec_name
        )
        error_pattern += r"\(specs: name: 'toy'; version: '0.0'; versionsuffix: ''; "
        error_pattern += r"toolchain name, version: 'gompi', '2018a'\)"
        self.assertErrorRegex(
            EasyBuildError, error_pattern, verify_easyconfig_filename, toy_ec, specs
        )
        ...
        error_pattern += (
            r"\(specs: name: 'toy'; version: '0.0'; versionsuffix: '-test'; "
        )
        error_pattern += r"toolchain name, version: 'gompi', '2018a'\)"
        self.assertErrorRegex(
            EasyBuildError, error_pattern, verify_easyconfig_filename, toy_ec, specs
        ...
        error_pattern = (
            r"Contents of .*/%s does not match with filename" % os.path.basename(toy_ec)
        )
        toy_txt = toy_txt.replace("versionsuffix = '-test'", "versionsuffix = ''")
        toy_ec = os.path.join(self.test_prefix, "toy-0.0-gompi-2018a-test.eb")
        write_file(toy_ec, toy_txt)
        error_pattern = (
            "Contents of .*/%s does not match with filename" % os.path.basename(toy_ec)
        )
        self.assertErrorRegex(
            EasyBuildError, error_pattern, verify_easyconfig_filename, toy_ec, specs
        )
    ...
    def test_get_paths_for(self):
        ...
        write_file(
            os.path.join(self.test_prefix, "bin", "eb"),
            "#!/bin/bash; echo 'This is a fake eb'",
        )
        ...
        self.assertEqual(
            res[-1], os.path.join(symlinked_prefix, "easybuild", "easyconfigs")
        )
        ...
        self.assertEqual(
            res[0], os.path.join(symlinked_prefix, "easybuild", "easyconfigs")
        )
        ...
        self.assertTrue(
            os.path.samefile(
                res[0], os.path.join(someprefix, "easybuild", "easyconfigs")
            )
        )
    ...
    def test_get_module_path(self):
        ...
        self.assertEqual(
            get_module_path("RPackage"), "easybuild.easyblocks.generic.rpackage"
        )
        self.assertEqual(
            get_module_path("RPackage", generic=True),
            "easybuild.easyblocks.generic.rpackage",
        )
    ...
    def test_not_an_easyconfig(self):
        ...
        not_an_ec = os.path.join(
            os.path.dirname(test_ecs_dir), "sandbox", "not_an_easyconfig.eb"
        )
    ...
    def test_check_sha256_checksums(self):
        ...
        self.assertTrue(
            res[0].startswith(
                "Checksums missing for one or more sources/patches in toy-0.0-fail.eb"
            )
        )
        ...
        self.assertTrue(res)
        self.assertTrue(
            res[-1].startswith("Non-SHA256 checksum(s) found for toy-0.0.tar.gz")
        )
    ...
    def test_filename(self):
        ...
        test_ecs = [
            ...
            os.path.join(
                "s", "ScaLAPACK", "ScaLAPACK-2.0.2-gompic-2018a-OpenBLAS-0.2.20.eb"
            ),
    ...
    def test_get_ref(self):
        ...
        self.assertTrue(
            sanity_check_paths_ref["files"]
            is ec._config["sanity_check_paths"][0]["files"]
        )
    ...
    def test_multi_deps(self):
        ...
        self.assertEqual(
            sorted(bd[0]["version"] for bd in builddeps),
            ["4.6.3", "4.8.3", "7.3.0-2.30"],
        )
        ...
        self.assertEqual(
            sorted(bd[0]["version"] for bd in multi_deps),
            ["4.6.3", "4.8.3", "7.3.0-2.30"],
        )
        ...
        self.assertTrue(
            all(bd[1]["full_mod_name"] == "CMake/3.12.1" for bd in builddeps)
        )
        ...
        error_pattern = (
            "Can't combine multi_deps with builddependencies specified as list of lists"
        )
        self.assertErrorRegex(EasyBuildError, error_pattern, EasyConfig, test_ec)
    ...
    def test_fix_deprecated_easyconfigs(self):
        ...
        test_ectxt = regex.sub(
            r'foo = "--foobar --barfoo --barfoobaz"\nconfigopts = foo\n\n\1', toy_ec_txt
        )
        ...
        unknown_params_error_pattern = (
            "Use of 2 unknown easyconfig parameters detected in test.eb: foo, some_list"
        )

        for tc_str in tc_strs:
            # first check if names of local variables get fixed if 'dummy' toolchain is not used
            init_config(
                build_options={"local_var_naming_check": "error", "silent": True}
            )

            write_file(test_ec, test_ectxt)
            self.assertErrorRegex(
                EasyBuildError, unknown_params_error_pattern, EasyConfig, test_ec
            )
            ...
            self.assertTrue(
                regex.search(test_ec_txt),
                "Pattern '%s' found in: %s" % (regex.pattern, test_ec_txt),
            )
            ...
            init_config(
                build_options={"local_var_naming_check": "warn", "silent": False}
            )
            self.mock_stderr(True)
            self.mock_stdout(True)
            ec = EasyConfig(test_ec)
            ...
            for warning in warnings:
                self.assertTrue(
                    warning in stderr,
                    "Found warning '%s' in stderr output: %s" % (warning, stderr),
                )
            ...
            self.assertFalse(
                regex.search(ectxt),
                "Pattern '%s' *not* found in: %s" % (regex.pattern, ectxt),
            )
            ...
            test_ecs = sorted(
                [f for f in os.listdir(self.test_prefix) if f.startswith("test.eb")]
            )
            ...
            for idx, pattern in enumerate(patterns):
                self.assertTrue(
                    re.match(pattern, stdout[idx]),
                    "Pattern '%s' matches '%s'" % (pattern, stdout[idx]),
                )
    ...
    def test_triage_easyconfig_params(self):
        ...
        ec.update(
            {"x": None, "local_foo": None, "_foo": None, "_": None,}
        )
        error = "Found 4 easyconfig parameters that are considered local variables: _, _foo, local_foo, x"
        self.assertErrorRegex(
            EasyBuildError, error, triage_easyconfig_params, variables, ec
        )
    ...
    def test_local_vars_detection(self):
        ...
        write_file(test_ec, test_ectxt)
        expected_error = (
            "Use of 1 unknown easyconfig parameters detected in test.eb: foobar"
        )
        self.assertErrorRegex(
            EasyBuildError,
            expected_error,
    ...
    def test_arch_specific_dependency(self):
        ...
        dep_str = "[('foo', {'arch=%s': '%s', 'arch=Foo': 'bar'})]" % (
            my_arch,
            expected_version,
        )
    ...
    def test_unexpected_version_keys_caught(self):
        ...
        for dep_str in (
            "[('foo', {'bar=%s': '%s', 'arch=Foo': 'bar'})]"
            % (my_arch, expected_version),
            "[('foo', {'blah': 'bar'})]",
        ):
            test_ec = os.path.join(self.test_prefix, "test.eb")
            test_ectxt = "\n".join(
    ...
    def test_resolve_exts_filter_template(self):
        ...
        self.assertErrorRegex(
            EasyBuildError, error_msg, resolve_exts_filter_template, "[ 1 == 1 ]", {}
        )
        self.assertErrorRegex(
            EasyBuildError, error_msg, resolve_exts_filter_template, ["[ 1 == 1 ]"], {}
        )
        ...
        test_cases = [
            ...
            (["%(ext_name)s", None], {"name": "foo"}, ("foo", None),),
            # Minimal case with input
            (["%(ext_name)s", ">%(ext_name)s"], {"name": "foo"}, ("foo", ">foo"),),
            # All values
            (
                [
                    "%(ext_name)s-%(ext_version)s-%(src)s",
                    ">%(ext_name)s-%(ext_version)s-%(src)s",
                ],
