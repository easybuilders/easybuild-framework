...
class RobotTest(EnhancedTestCase):
    ...
    def test_resolve_dependencies(self):
        ...
        base_easyconfig_dir = find_full_path(
            os.path.join("test", "framework", "easyconfigs", "test_ecs")
        )
        ...
        res = resolve_dependencies(
            [deepcopy(easyconfig_moredeps)], self.modtool, retain_all_deps=True
        )
        self.assertEqual(
            len(res), 4
        )  # hidden dep toy/.0.0-deps (+1) depends on (fake) intel/2018a (+1)
        self.assertEqual("gzip/1.4", res[0]["full_mod_name"])
        self.assertEqual("foo/1.2.3", res[-1]["full_mod_name"])
        full_mod_names = [ec["full_mod_name"] for ec in res]
        ...
        self.assertErrorRegex(
            EasyBuildError, msg, resolve_dependencies, ecs, self.modtool
        )
        ...
        newecs = skip_available(
            ecs, self.modtool
        )  # skip available builds since force is not enabled
        ...
        self.assertEqual(
            "ScaLAPACK/2.0.2-gompi-2018a-OpenBLAS-0.2.20", res[1]["full_mod_name"]
        )
        self.assertEqual("foss/2018a", res[2]["full_mod_name"])
    ...
    def test_resolve_dependencies_minimal(self):
        ...
        easybuild.framework.easyconfig.tools._log.experimental = (
            easybuild.framework.easyconfig.tools._log.warning
        )
        ...
        impi_txt = read_file(
            os.path.join(test_easyconfigs, "i", "impi", "impi-5.1.2.150.eb")
        )
    ...
    def test_resolve_dependencies_missing(self):
        ...
        self.assertErrorRegex(
            EasyBuildError, error, resolve_dependencies, [ec], self.modtool
        )
        ...
        res = resolve_dependencies(
            [ec], self.modtool, retain_all_deps=True, raise_error_missing_ecs=False
        )
        self.assertEqual(len(res), 2)
        self.assertEqual(res[0]["full_mod_name"], "test/123")
    ...
    def test_det_easyconfig_paths(self):
        ...
        shutil.copy2(
            os.path.join(test_ecs_path, "i", "intel", "intel-2018a.eb"),
            self.test_prefix,
        )
        ...
        args = [
            ...
            "--github-user=%s"
            % GITHUB_TEST_ACCOUNT,  # a GitHub token should be available for this user
        ...
        modules = [
            (
                test_ecs_path,
                "toy/0.0",
            ),  # specified easyconfigs, available at given location
            (self.test_prefix, "intel/2018a"),  # dependency, found in robot search path
            (
                self.test_prefix,
                "toy/0.0-deps",
            ),  # specified easyconfig, found in robot search path
        ]
        for path_prefix, module in modules:
            ec_fn = "%s.eb" % "-".join(module.split("/"))
            regex = re.compile(
                r"^ \* \[.\] %s.*%s \(module: %s\)$" % (path_prefix, ec_fn, module),
                re.M,
            )
            self.assertTrue(
                regex.search(outtxt), "Found pattern %s in %s" % (regex.pattern, outtxt)
            )
        ...
        outtxt = self.eb_main(args, logfile=dummylogfn, raise_error=True)
        regex = re.compile(
            r"^ \* \[.\] .*/__archive__/.*/intel-2012a.eb \(module: intel/2012a\)", re.M
        )
        self.assertTrue(
            regex.search(outtxt), "Found pattern %s in %s" % (regex.pattern, outtxt)
        )
    ...
    def test_det_easyconfig_paths_from_pr(self):
        ...
        write_file(
            os.path.join(self.test_prefix, "gompi-2018b-test.eb"), gompi_2018b_txt
        )
        ...
        test_ecs_path = os.path.join(
            ".*", "test", "framework", "easyconfigs", "test_ecs"
        )
        ...
        modules = [
            ...
            (
                test_ecs_path,
                "gompi/2018b",
            ),  # part of PR easyconfigs, found in robot search path
            (
                test_ecs_path,
                "GCC/7.3.0-2.30",
            ),  # dependency for PR easyconfigs, found in robot search path
        ]
        for path_prefix, module in modules:
            ec_fn = "%s.eb" % "-".join(module.split("/"))
            regex = re.compile(
    ...
    def test_get_toolchain_hierarchy(self):
        ...
        fosscuda_hierarchy = get_toolchain_hierarchy(
        ...
        foss_hierarchy = get_toolchain_hierarchy(
            {"name": "foss", "version": "2018a"}, incl_capabilities=True
        )
        expected = [
            {
                "name": "GCC",
        ...
        iimpi_hierarchy = get_toolchain_hierarchy(
            {"name": "iimpi", "version": "2016.01"}, incl_capabilities=True
        )
        expected = [
            {
                "name": "GCCcore",
        ...
        iccifortcuda_hierarchy = get_toolchain_hierarchy(
            {"name": "iccifortcuda", "version": "2016.1.150"}, incl_capabilities=True
        )
        expected = [
            {
                "name": "GCCcore",
        ...
        gcc_hierarchy = get_toolchain_hierarchy(
        ...
        get_toolchain_hierarchy.clear()
        iccifort_hierarchy = get_toolchain_hierarchy(
            {"name": "iccifort", "version": "2016.1.150-GCC-4.9.3-2.25"}
        )
        self.assertEqual(
            iccifort_hierarchy,
        ...
        init_config(build_options=build_options)
        craycce_hierarchy = get_toolchain_hierarchy(
        ...
        gmvapich2_hierarchy = get_toolchain_hierarchy(
        ...
        copy_file(
            os.path.join(test_easyconfigs, "g", "gompi", "gompi-2018a.eb"), broken_gompi
        )
    ...
    def test_find_resolved_modules(self):
        ...
        mods = [
            "foo/2.3.4-GCC-6.4.0-2.28",
            "bar/3.4.5-gompi-2018a",
            "bar/3.4.5-GCC-6.4.0-2.28",
        ]

        ordered_ecs, new_easyconfigs, new_avail_modules = find_resolved_modules(
            ecs, mods, self.modtool
        )
        ...
        self.assertEqual(
            new_easyconfigs[0]["full_mod_name"], "threedeps/9.8.7-foss-2018a"
        )
        self.assertEqual(len(new_easyconfigs[0]["dependencies"]), 1)
        self.assertEqual(new_easyconfigs[0]["dependencies"][0]["name"], "bar")

        self.assertTrue(
            new_avail_modules, mods + ["nodeps/1.2.3", "onedep/3.14-foss-2018a"]
        )
        ...
        self.assertEqual(len(ordered_ecs), 2)
        self.assertEqual(
            [ec["full_mod_name"] for ec in ordered_ecs],
            ["nodeps/1.2.3", "onedep/3.14-foss-2018a"],
        )

        self.assertEqual(len(new_easyconfigs), 1)
        self.assertEqual(len(new_easyconfigs[0]["dependencies"]), 2)
        self.assertEqual(
            [dep["name"] for dep in new_easyconfigs[0]["dependencies"]], ["foo", "bar"]
        )
    ...
    def test_tweak_robotpath(self):
        ...
        tweaked_ecs_paths, pr_path = alt_easyconfig_paths(
            self.test_prefix, tweaked_ecs=True
        )
        robot_path = det_robot_path(
            [test_easyconfigs], tweaked_ecs_paths, pr_path, auto_robot=True
        )
        ...
        untweaked_openmpi = os.path.join(
            test_easyconfigs, "o", "OpenMPI", "OpenMPI-2.1.2-GCC-4.6.4.eb"
        )
        ...
        easyconfigs = tweak(
            easyconfigs, tweak_specs, self.modtool, targetdirs=tweaked_ecs_paths
        )

        # Check that all expected tweaked easyconfigs exists
        tweaked_openmpi = os.path.join(
            tweaked_ecs_paths[0], "OpenMPI-2.1.2-GCC-6.4.0-2.28.eb"
        )
        tweaked_hwloc = os.path.join(
            tweaked_ecs_paths[1], "hwloc-1.11.8-GCC-6.4.0-2.28.eb"
        )
        ...
        untweaked_hwloc = os.path.join(
            test_easyconfigs, "h", "hwloc", "hwloc-1.11.8-GCC-6.4.0-2.28.eb"
        )
    ...
    def test_robot_find_subtoolchain_for_dep(self):
        ...
        get_toolchain_hierarchy.clear()
        new_gzip14_toolchain = robot_find_subtoolchain_for_dep(
            gzip14, self.modtool, parent_tc=gompi_1410
        )
        ...
        init_config(
            build_options={"minimal_toolchains": True, "robot_path": test_easyconfigs,}
        )
        bar = EasyConfig(barec)
        ...
        os.environ[
            "MODULEPATH"
        ] = module_parent  # Add the parent directory to the MODULEPATH
    ...
    def test_robot_find_subtoolchain_for_dep_ecs_vs_mods(self):
        ...
        write_file(
            os.path.join(self.test_prefix, "dummydep-1.2.3-foss-2018a.eb"), ec_txt
        )
    ...
    def test_check_conflicts(self):
        ...
        self.assertTrue(
            "Conflict found for dependencies of foss-2018a: GCC-4.6.4 vs GCC-6.4.0-2.28"
            in stderr
        )
        ...
        ecs, _ = parse_easyconfigs(
            [
                (
                    os.path.join(test_easyconfigs, "g", "GCC", "GCC-6.4.0-2.28.eb"),
                    False,
                ),
                (
                    os.path.join(test_easyconfigs, "g", "GCC", "GCC-4.9.3-2.25.eb"),
                    False,
                ),
            ]
        )
        ...
        self.assertTrue(conflicts)
        self.assertTrue(
            "Conflict between (dependencies of) easyconfigs: GCC-4.9.3-2.25 vs GCC-6.4.0-2.28"
            in stderr
        )
        ...
        ecs, _ = parse_easyconfigs(
            [
                (
                    os.path.join(
                        test_easyconfigs, "b", "bzip2", "bzip2-1.0.6-GCC-4.9.2.eb"
                    ),
                    False,
                ),
                (
        ...
        self.assertTrue(conflicts)
        self.assertTrue(
            "Conflict between (dependencies of) easyconfigs: GCC-4.9.2 vs GCC-6.4.0-2.28"
            in stderr
        )

        # test use of check_inter_ec_conflicts
        self.assertFalse(
            check_conflicts(ecs, self.modtool, check_inter_ec_conflicts=False),
            "No conflicts found",
        )
    ...
    def test_robot_archived_easyconfigs(self):
        ...
        self.assertEqual(
            [ec["full_mod_name"] for ec in res], ["intel/2012a", "gzip/1.5-intel-2012a"]
        )
