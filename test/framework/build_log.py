...
class BuildLogTest(EnhancedTestCase):
    ...
    def test_easybuilderror(self):
        ...
        self.assertTrue(
            log_re.match(logtxt), "%s matches %s" % (log_re.pattern, logtxt)
        )

        # test formatting of message
        self.assertErrorRegex(
            EasyBuildError, "BOOMBAF", raise_easybuilderror, "BOOM%s", "BAF"
        )

        # a '%s' in a value used to template the error message should not print a traceback!
        self.mock_stderr(True)
        self.assertErrorRegex(
            EasyBuildError, "err: msg: %s", raise_easybuilderror, "err: %s", "msg: %s"
        )
    ...
    def test_easybuildlog(self):
        ...
        expected_stderr = (
            "\n\n".join(
                [
                    "\nWARNING: Deprecated functionality, will no longer work in v10000001: anotherwarning; "
                    + more_info,
        ...
        log.deprecated(
            "\nthis is just a test\n",
            newer_ver,
            more_info="(see URLGOESHERE for more information)",
        )
        self.mock_stderr(False)
        logtxt = read_file(tmplog)
        expected_logtxt = "\n".join(
    ...
    def test_log_levels(self):
        ...
        self.mock_stderr(
            True
        )  # avoid that some log statement spit out stuff to stderr while tests are running
        ...
        deprecated_msg = (
            r"%s \[WARNING\] :: Deprecated functionality, .*: almost kaput; see .*"
            % prefix
        )
        error_msg = (
            r"%s \[ERROR\] :: EasyBuild crashed with an error \(at .* in .*\): kaput"
            % prefix
        )
    ...
    def test_print_warning(self):
        ...
        run_check(
            ["You have been %s.", "warned"],
            expected_stderr="\nWARNING: You have been warned.\n\n",
        )
        run_check(
            ["You %s %s %s.", "have", "been", "warned"],
            expected_stderr="\nWARNING: You have been warned.\n\n",
        )
    ...
    def test_print_error(self):
        ...
        run_check(
            ["You have %s.", "failed"], expected_stderr="ERROR: You have failed.\n"
        )
        run_check(
            ["%s %s %s.", "You", "have", "failed"],
            expected_stderr="ERROR: You have failed.\n",
        )
    ...
    def test_print_msg(self):
        ...
        run_check(
            "testing, %s, %s, %s",
            ["1", "2", "3"],
            expected_stdout="== testing, 1, 2, 3\n",
        )
        run_check(
            "testing, 1, 2, 3", [], expected_stdout="== testing, 1, 2, 3", newline=False
        )
        run_check(
            "testing, %s, 2, %s",
            ["1", "3"],
        ...
        run_check(
            "testing, 1, 2, 3", [], expected_stdout="testing, 1, 2, 3\n", prefix=False
        )
        run_check(
            "testing, 1, 2, 3",
            [],
        ...
        run_check(
            "testing, 1, 2, 3", [], expected_stderr="== testing, 1, 2, 3\n", stderr=True
        )
        run_check(
            "testing, 1, 2, 3",
            [],
            expected_stderr="== testing, 1, 2, 3",
            stderr=True,
            newline=False,
        )
        run_check(
    ...
    def test_time_str_since(self):
        ...
        self.assertEqual(
            time_str_since(datetime.now() - timedelta(seconds=1.1)), "00h00m01s"
        )
        self.assertEqual(
            time_str_since(datetime.now() - timedelta(seconds=37.1)), "00h00m37s"
        )
        self.assertEqual(
            time_str_since(datetime.now() - timedelta(seconds=60.1)), "00h01m00s"
        )
        self.assertEqual(
            time_str_since(datetime.now() - timedelta(seconds=81.1)), "00h01m21s"
        )
        self.assertEqual(
            time_str_since(datetime.now() - timedelta(seconds=1358.1)), "00h22m38s"
        )
        self.assertEqual(
            time_str_since(datetime.now() - timedelta(seconds=3600.1)), "01h00m00s"
        )
        self.assertEqual(
            time_str_since(datetime.now() - timedelta(seconds=3960.1)), "01h06m00s"
        )
        self.assertEqual(
            time_str_since(datetime.now() - timedelta(seconds=4500.1)), "01h15m00s"
        )
        self.assertEqual(
            time_str_since(datetime.now() - timedelta(seconds=12305.1)), "03h25m05s"
        )
        self.assertEqual(
            time_str_since(datetime.now() - timedelta(seconds=54321.1)), "15h05m21s"
        )
    ...
    def test_dry_run_warning(self):
        ...
        run_check(
            "test 123", [], expected_stdout="\n!!!\n!!! WARNING: test 123\n!!!\n\n"
        )
        run_check(
            "test %s", ["123"], expected_stdout="\n!!!\n!!! WARNING: test 123\n!!!\n\n"
        )
        run_check("test 123", [], silent=True)
        run_check("test %s", ["123"], silent=True)
    ...
    def test_raise_nosupport(self):
        self.assertErrorRegex(
            EasyBuildError,
            "NO LONGER SUPPORTED since v42: foobar;",
            raise_nosupport,
            "foobar",
            42,
        )
